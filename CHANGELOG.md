...
- **MyLeague Background Automation, Halo Live Activities & Matchday Prediction Market**:
  ...
  - **Club result notifications**: [club-notify.ts](file:///ixwiki/public/projects/ixstats/src/lib/sports/club-notify.ts) delivers each owned club's matchday result two ways — an in-app `Notification` (the bell, deep-linked to `/myclub`) and a durable record message in a per-user "Sports Desk" ThinkShare channel.
