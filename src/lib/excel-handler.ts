...
export class IxStatsExcelHandler {
  ...
  async parseFile(fileBuffer: ArrayBuffer, fileName?: string): Promise<ExcelParseResult> {
    ...
    try {
      ...
      if (fileName && !this.isExcelFile(fileName)) {
        result.errors.push(
          "Only Excel files (.xlsx, .xls) are supported. CSV import has been removed."
        );
        return result;
      }
      ...
      result.success = true;
      console.log(
        `[Excel Handler] Successfully parsed ${result.data.length} countries from ${fileName || "Excel file"}`
      );
    } catch (error) {
      result.errors.push(
        `Excel file parsing failed: ${error instanceof Error ? error.message : "Unknown error"}`
      );
  ...
  private createFieldIndexMap(headers: string[]): Map<keyof BaseCountryData, number> {
    ...
    for (const mapping of FIELD_MAPPINGS) {
      ...
      if (headerIndex !== -1) {
        ...
        console.warn(
          `[Excel Handler] Required field "${mapping.excelHeader}" (or aliases) not found in headers.`
        );
      }
    }
  ...
  private parseRow(
    row: any[],
    fieldMap: Map<keyof BaseCountryData, number>
    ...
    try {
      ...
      const countryData: BaseCountryData = {
        ...
        populationGrowthRate: this.parsePercentage(
          row[fieldMap.get("populationGrowthRate") ?? -1],
          0.01
        ),
        projected2040Population:
          this.parseOptionalNumber(row[fieldMap.get("projected2040Population") ?? -1]) ?? 0,
        projected2040Gdp:
          this.parseOptionalNumber(row[fieldMap.get("projected2040Gdp") ?? -1]) ?? 0,
        projected2040GdpPerCapita:
          this.parseOptionalNumber(row[fieldMap.get("projected2040GdpPerCapita") ?? -1]) ?? 0,
      ...
      if (
        countryData.areaSqMi === null &&
        countryData.landArea !== null &&
        countryData.landArea !== undefined
      ) {
      ...
      if (
        countryData.landArea === null &&
        countryData.areaSqMi !== null &&
        countryData.areaSqMi !== undefined
      ) {
  ...
  private parsePercentage(value: any, defaultValue: number): number {
    ...
    if (asString === "" || asString.toLowerCase() === "#div/0!" || asString.toLowerCase() === "n/a")
      return defaultValue;
  ...
  private validateCountryData(data: BaseCountryData): boolean {
    ...
    if (!isValid) {
      console.warn(
        `[ExcelHandler] Validation failed for ${data.country}: Pop=${data.population}, GDPPC=${data.gdpPerCapita}, ProjPop=${data.projected2040Population}, ProjGDP=${data.projected2040Gdp}, ProjGDPPC=${data.projected2040GdpPerCapita}`
      );
    }
    return isValid;
  }
