...
export class PreviewConfigManager {
  ...
  private static readonly DEFAULT_CONFIG: PreviewConfig = {
    ...
    features: {
      enabledModules: ["countries", "analytics", "modals", "scenarios", "admin", "sdi", "eci"],
  ...
  public static getDemoUser(username: string, password: string): DemoUser | null {
    ...
    return (
      config.demoUsers.find(
        (user) => user.credentials.username === username && user.credentials.password === password
      ) || null
    );
  }
  ...
  private static showPreviewBanner(): void {
    ...
    banner.className =
      "fixed top-0 left-0 right-0 z-50 bg-gradient-to-r from-blue-600 to-purple-600 text-white px-4 py-2 text-center text-sm font-medium";
    banner.innerHTML = `
