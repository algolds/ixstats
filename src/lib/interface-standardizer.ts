 ...
 */
export function standardizePriorityProperties<T extends Record<string, any>>(
  obj: T
): T & {
 ...
 */
export function standardizeCategoryProperties<T extends Record<string, any>>(
  obj: T
): T & {
 ...
 */
export function standardizeTimestampProperties<T extends Record<string, any>>(
  obj: T
): T & {
  timestamp: number;
  createdAt: number;
 ...
 */
export function bulkStandardizeInterface<T extends Record<string, any>>(
...
export function standardizeCountryData(data: any): CountryWithEconomicData {
  ...
  return validateObjectProperties(
    ...
    ],
    {
      ...
      flagUrl:
        typeof data?.flagUrl === "string" && data.flagUrl.trim().length > 0
          ? data.flagUrl
