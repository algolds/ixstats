...
export const DEFENSE_MANUFACTURERS = {
  // United States
  LOCKHEED_MARTIN: { name: 'Lockheed Martin', country: 'USA', specialty: ['aircraft', 'missiles', 'space'] },
  BOEING: { name: 'Boeing', country: 'USA', specialty: ['aircraft', 'helicopters', 'missiles'] },
  NORTHROP_GRUMMAN: { name: 'Northrop Grumman', country: 'USA', specialty: ['aircraft', 'electronics', 'ships'] },
  RAYTHEON: { name: 'Raytheon Technologies', country: 'USA', specialty: ['missiles', 'electronics', 'sensors'] },
  GENERAL_DYNAMICS: { name: 'General Dynamics', country: 'USA', specialty: ['tanks', 'submarines', 'munitions'] },

  // Europe
  BAE_SYSTEMS: { name: 'BAE Systems', country: 'UK', specialty: ['aircraft', 'ships', 'vehicles'] },
  AIRBUS: { name: 'Airbus Defence', country: 'EU', specialty: ['aircraft', 'helicopters', 'space'] },
  THALES: { name: 'Thales Group', country: 'France', specialty: ['electronics', 'missiles', 'radar'] },
  LEONARDO: { name: 'Leonardo', country: 'Italy', specialty: ['helicopters', 'electronics', 'weapons'] },
  RHEINMETALL: { name: 'Rheinmetall', country: 'Germany', specialty: ['vehicles', 'weapons', 'ammunition'] },

  // Russia
  UNITED_AIRCRAFT: { name: 'United Aircraft Corporation', country: 'Russia', specialty: ['aircraft', 'helicopters'] },
  ...
  MITSUBISHI_HEAVY: { name: 'Mitsubishi Heavy Industries', country: 'Japan', specialty: ['aircraft', 'ships'] },
  KAI: { name: 'Korea Aerospace Industries', country: 'South Korea', specialty: ['aircraft', 'helicopters'] },
  HAL: { name: 'Hindustan Aeronautics', country: 'India', specialty: ['aircraft', 'helicopters'] },

  // Israel
  IAI: { name: 'Israel Aerospace Industries', country: 'Israel', specialty: ['aircraft', 'missiles', 'drones'] },
  ELBIT: { name: 'Elbit Systems', country: 'Israel', specialty: ['electronics', 'drones', 'sensors'] },
...
export function getEquipmentByManufacturer(manufacturer: keyof typeof DEFENSE_MANUFACTURERS) {
  const aircraft = Object.entries(MILITARY_AIRCRAFT).filter(([_, v]) => v.manufacturer === manufacturer);
  const ships = Object.entries(MILITARY_SHIPS).filter(([_, v]) => v.manufacturer === manufacturer);
  const vehicles = Object.entries(MILITARY_VEHICLES).filter(([_, v]) => v.manufacturer === manufacturer);
  const weapons = Object.entries(WEAPON_SYSTEMS).filter(([_, v]) => v.manufacturer === manufacturer);
