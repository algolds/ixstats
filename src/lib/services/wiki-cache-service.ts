...
export class WikiCacheService {
   ...
   */
  async getCountryInfobox(countryName: string): Promise<WikiCacheEntry<CountryInfoboxWithDynamicProps | null>> {
    ...
    try {
      ...
      if (dbCache && new Date(dbCache.expiresAt) > new Date()) {
        ...
        if (this.redisClient) {
          const ttlSeconds = Math.floor((new Date(dbCache.expiresAt).getTime() - Date.now()) / 1000);
          if (ttlSeconds > 0) {
   ...
   */
  async getCountryProfile(countryName: string, pageVariants: string[]): Promise<WikiCountryProfile> {
  ...
  async getCacheStats(): Promise<{
    ...
    try {
      ...
      const typeStats = entriesByType.reduce((acc, item) => {
