...
import { type WorldNamingSystem } from "./markov-naming";
...
export interface CountryGenParams {
  ...
  similarity?: number;
  /** Markov naming system — if provided, uses linguistically coherent names */
  namingSystem?: WorldNamingSystem;
  /** Region → language family assignments (country index → familyId) */
  familyAssignments?: Map<number, string>;
}
...
export function generateCountries(params: CountryGenParams): CountryResult {
  ...
  // Step 3: Lloyd relaxation (1 iteration — keeps irregular borders more realistic)
  ...
  // Name generator: use Markov system if provided, otherwise fallback to syllable combiner
  const legacyNameGen = createNameGenerator(seed + 300);
  const nameGen = (countryIndex: number): string => {
    if (params.namingSystem && params.familyAssignments) {
      const familyId = params.familyAssignments.get(countryIndex);
      if (familyId) {
        return params.namingSystem.generateName(familyId, { useSuffix: true });
      }
    }
    return legacyNameGen();
  };
