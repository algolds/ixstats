...
export class IntelligenceWebSocketServer {
        ...
        vitality: {
          ...
          diplomatic: Math.min(
            ...
            Math.max(
              0,
              50 +
                (country.actualGdpGrowth || 0) * 10 +
                ((country as any).currentGdpPerCapita > 25000
                  ? 20
                  : (country as any).currentGdpPerCapita > 10000
                    ? 10
                    : 0)
            )
          ),
  ...
  private updateConnectionStats(): void {
    const stats = {
      ...
      authenticatedConnections: Array.from(this.connectedUsers.values()).filter(
        (conn) => conn.userId
      ).length,
      countrySubscriptions: Array.from(this.connectedUsers.values()).filter(
        (conn) => conn.countryId
      ).length,
  ...
  private startHealthChecks(): void {
    setInterval(() => {
      ...
      for (const [socketId, connection] of this.connectedUsers) {
        if (now - connection.lastSeen > 120000) {
          // 2 minutes
