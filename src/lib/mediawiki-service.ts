...
function runNextRequest() {
  if (currentRequests < MAX_CONCURRENT_REQUESTS && requestQueue.length > 0) {
    ...
    if (next) {
      console.log(
        `[MediaWiki] Executing queued request. Current: ${currentRequests}, Queue: ${requestQueue.length}`
      );
      next();
    }
  }
...
async function throttledRequest<T>(fn: () => Promise<T>): Promise<T> {
  return new Promise<T>((resolve, reject) => {
    const exec = () => {
      currentRequests++;
      console.log(
        `[MediaWiki] Starting request. Current: ${currentRequests}, Queue: ${requestQueue.length}`
      );
      ...
      fn().then(
        (res) => {
          currentRequests--;
          console.log(
            `[MediaWiki] Request completed. Current: ${currentRequests}, Queue: ${requestQueue.length}`
          );
          runNextRequest();
          resolve(res);
        },
        (err) => {
          currentRequests--;
          console.log(
            `[MediaWiki] Request failed. Current: ${currentRequests}, Queue: ${requestQueue.length}`
          );
          runNextRequest();
          reject(err);
        }
    ...
    if (currentRequests < MAX_CONCURRENT_REQUESTS) {
      ...
      console.log(
        `[MediaWiki] Queuing request. Current: ${currentRequests}, Queue: ${requestQueue.length}`
      );
...
export class IxnayWikiService {
  ...
  >(MEDIAWIKI_CONFIG.cache.maxSize);
  private readonly TEMPLATE_CACHE = new LRUCache<string, CacheEntry<string | null>>(
    MEDIAWIKI_CONFIG.cache.maxSize
  );
  private readonly FILE_CACHE = new LRUCache<string, CacheEntry<string | null>>(
    MEDIAWIKI_CONFIG.cache.maxSize
  );
  private readonly WIKITEXT_CACHE = new LRUCache<string, CacheEntry<string | null>>(
    MEDIAWIKI_CONFIG.cache.maxSize
  );
  private readonly RENDERED_HTML_CACHE = new LRUCache<string, CacheEntry<string | null>>(
    MEDIAWIKI_CONFIG.cache.maxSize
  );
  ...
  async parseWikitextToHtml(wikitext: string, title?: string): Promise<string | null> {
    ...
    try {
      ...
      if (!response.ok) {
        console.error(
          `[MediaWiki] Parse API HTTP Error: ${response.status} ${response.statusText}`
        );
  ...
  extractInfoboxTemplate(wikitext: string): string | null {
    try {
      console.log(
        `[MediaWiki] Extracting infobox template from wikitext (${wikitext.length} chars)`
      );
      ...
      for (const pattern of infoboxStartPatterns) {
        ...
        if (match) {
          ...
          console.log(
            `[MediaWiki] Found infobox start pattern at index ${startIndex}: ${match[0]}`
          );
          break;
        }
      }
      ...
      console.log(
        `[MediaWiki] Extracted complete infobox template: ${templateContent.length} characters`
      );
  ...
  async parseInfoboxParameters(templateWikitext: string): Promise<Record<string, string>> {
    ...
    try {
      console.log(
        `[MediaWiki] Two-phase parsing of infobox parameters from template (${templateWikitext.length} chars)`
      );
      ...
      for (const [key, value] of Object.entries(parsedParams)) {
        if (key && value !== undefined) {
          ...
          console.log(
            `[MediaWiki] Parameter: ${key} = ${processedValue.substring(0, 100)}${processedValue.length > 100 ? "..." : ""}`
          );
        }
      }

      console.log(
        `[MediaWiki] Successfully parsed ${Object.keys(parameters).length} total parameters`
      );
  ...
  private findParameterBoundaries(
    content: string
  ): Array<{ name: string; start: number; nameEnd: number }> {
  ...
  private extractParameterValues(
    ...
    for (let i = 0; i < boundaries.length; i++) {
      ...
      console.log(
        `[MediaWiki] Extracted ${current.name}: ${rawValue.substring(0, 150)}${rawValue.length > 150 ? "..." : ""}`
      );
    }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (paramMatch) {
        ...
        if (inMultilineParam && currentParam) {
          ...
          console.log(
            `[MediaWiki] Regex fallback - ${currentParam}: ${currentValue.substring(0, 100)}${currentValue.length > 100 ? "..." : ""}`
          );
        }
  ...
  private extractTemplateContent(templateWikitext: string): string | null {
    ...
    console.log(
      `[MediaWiki] Extracted template content: ${extractedContent.length} characters (from ${contentStart} to ${contentEnd})`
    );

    // Validate that we have a reasonable amount of content
    if (extractedContent.length < 100) {
      console.warn(
        `[MediaWiki] Extracted content seems too short (${extractedContent.length} chars), might be incomplete`
      );
  ...
  async getCountryInfobox(countryName: string): Promise<CountryInfoboxWithDynamicProps | null> {
    ...
    return this.getOrCreateRequest(`infobox_${countryName}`, async () => {
      try {
        ...
        const hasUnprocessedTemplates = Object.entries(infobox).some(
          ([key, value]) =>
            typeof value === "string" &&
            value.includes("{{") &&
            key !== "rawWikitext" &&
        ...
        const hasUnprocessedWikilinks = Object.entries(infobox).some(
          ([key, value]) =>
            typeof value === "string" &&
            value.includes("[[") &&
            key !== "rawWikitext" &&
        ...
        if (!hasUnprocessedTemplates && !hasUnprocessedWikilinks) {
          console.log(
            `[MediaWiki] ✓ ${countryName} - All templates and wikilinks processed successfully`
          );
        }
   ...
   */
  private mapTemplateDataToInfobox(
    ...
    for (const [key, value] of Object.entries(templateData)) {
      if (key && value && value.trim()) {
        ...
        if (value.includes("[[") && !value.includes("{{")) {
          ...
          console.log(
            `[MediaWiki] Pre-processed wikilinks in ${key}: "${value.substring(0, 50)}..." -> "${processedValue.substring(0, 50)}..."`
          );
        }
    ...
    console.log(
      `[MediaWiki] Mapped ALL ${Object.keys(templateData).length} template parameters to infobox`
    );
  }
      ...
      for (const [key, value] of Object.entries(infobox.parsedTemplateData)) {
        if (value && (value.includes("{{") || value.includes("[["))) {
          ...
          if (processedValue && processedValue !== value) {
            ...
            console.log(
              `  Processed: ${processedValue.substring(0, 100)}${processedValue.length > 100 ? "..." : ""}`
            );
          }
        }
      }
  ...
  private enhancedSwitcherProcessing(switcherTemplate: string): string {
    try {
      console.log(
        `[MediaWiki] Enhanced Switcher processing: ${switcherTemplate.substring(0, 200)}...`
      );
      ...
      parts.forEach((part, index) => {
        console.log(
          `[MediaWiki] Switcher part ${index}: ${part.substring(0, 100)}${part.length > 100 ? "..." : ""}`
        );
      });
      ...
      for (const part of parts) {
        ...
        if (
          ...
          console.log(
            `[MediaWiki] Found text content in Switcher: ${cleaned.substring(0, 100)}...`
          );
          return cleaned;
        }
      }
    ...
    try {
      ...
      if (rendered) {
        ...
        if (
          ...
          textContent.length < templateValue.length * 3
        ) {
          return textContent;
        }
      }
    ...
    return this.getOrCreateRequest(`flag_${normName}`, async () => {
      ...
      try {
        ...
        try {
          ...
          console.warn(
            `[MediaWiki] Country_data template method failed for ${countryName}:`,
            error
          );
        }
          ...
          if (infobox) {
            ...
            for (const flagFile of flagFields) {
              if (flagFile?.trim()) {
                ...
                if (typeof flagUrl === "string") {
                  console.log(
                    `[MediaWiki] Successfully got flag URL from infobox: ${flagUrl} for ${countryName}`
                  );
                  this.setCacheValue(this.FLAG_CACHE, normName, flagUrl, this.FLAG_TTL);
                  return flagUrl;
                }
        ...
        try {
          ...
          for (const flagPattern of commonFlagPatterns) {
            ...
            if (typeof flagUrl === "string") {
              console.log(
                `[MediaWiki] Successfully got flag URL from pattern: ${flagUrl} for ${countryName}`
              );
              this.setCacheValue(this.FLAG_CACHE, normName, flagUrl, this.FLAG_TTL);
              return flagUrl;
            }
        ...
        NOT_FOUND_SET.add(normName);
        return {
          error: `Critical exception for ${countryName}: ${error instanceof Error ? error.message : String(error)}`,
        };
      }
    });
  }
  ...
  private extractFlagFromCountryDataTemplate(templateContent: string): string | null {
    try {
      console.log(
        `[MediaWiki] Extracting flag from Country_data template (${templateContent.length} chars)`
      );
      if (process.env.NODE_ENV !== "production") {
        console.log(
          `[MediaWiki] Template content preview: ${templateContent.substring(0, 500)}...`
        );
      }
      ...
      for (const pattern of aliasPatterns) {
        ...
        if (match?.[1]) {
          ...
          if (
            ...
            console.log(
              `[MediaWiki] Found flag filename in Country_data template: ${flagFileName}`
            );
            return flagFileName;
          }
        }
      ...
      for (const line of lines) {
        ...
        if (
          ...
          const fileMatch = /([^|\s\[\]]+(?:Flag[^|\s\[\]]*\.(?:svg|png|jpg|jpeg|gif)))/i.exec(
            trimmedLine
          );
          if (fileMatch?.[1]) {
  ...
  async getFileUrl(fileName: string): Promise<string | { error: string }> {
    ...
    return this.getOrCreateRequest(`file:${normalizedKey}`, async () => {
      try {
        ...
        NOT_FOUND_SET.add(normalizedKey);
        return {
          error: `[MediaWiki] Exception getting file ${cleanFileName}: ${error instanceof Error ? error.message : String(error)}`,
        };
      }
    });
  }
  ...
  async preloadCountryFlags(countryNames: string[]): Promise<void> {
    ...
    try {
      ...
      console.log(
        `[MediaWiki] ${uncachedCountries.length} countries need flag loading (${countryNames.length - uncachedCountries.length} already cached)`
      );
      ...
      for (let i = 0; i < uncachedCountries.length; i += BATCH_SIZE) {
        ...
        console.log(
          `[MediaWiki] Processing batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(uncachedCountries.length / BATCH_SIZE)}: ${batch.join(", ")}`
        );
      ...
      console.log(
        `[MediaWiki] ✓ Completed intelligent flag preloading for ${countryNames.length} countries`
      );
    } finally {
  ...
  getCacheStats() {
    ...
    const validFlags = Array.from(this.FLAG_CACHE.values()).filter(
      (entry) => now - entry.timestamp < entry.ttl
    );
    const validInfoboxes = Array.from(this.INFOBOX_CACHE.values()).filter(
      (entry) => now - entry.timestamp < entry.ttl
    );

    const failedFlags = Array.from(this.FLAG_CACHE.values()).filter((entry) => entry.data === null);

    const totalValid = validFlags.length + validInfoboxes.length;
    const totalCaches =
      this.FLAG_CACHE.size +
      this.INFOBOX_CACHE.size +
      this.TEMPLATE_CACHE.size +
      this.FILE_CACHE.size +
      this.WIKITEXT_CACHE.size +
      this.RENDERED_HTML_CACHE.size;
  ...
  private cleanParameterValue(value: string): string {
    ...
    cleaned = cleaned.replace(
      /\[\[([^\|\]]+)\|([^\]]+)\]\]/g,
      (_match: string, page: string, display: string) => {
  ...
  private processCommonTemplates(text: string): string {
    ...
    while (processed.includes("{{") && iterations < maxIterations) {
      ...
      processed = processed.replace(
        /\{\{convert\|([^|]+)\|([^|]+)[\|][^}]*\}\}/g,
        (_match: string, num: string, unit: string) => `${num.trim()} ${unit.trim()}`
      );
      processed = processed.replace(
        /\{\{convert\|([^|]+)\|([^}]+)\}\}/g,
        (_match: string, num: string, unit: string) => `${num.trim()} ${unit.trim()}`
      ...
      processed = processed.replace(
        /\{\{currency\|([^|]+)\|([^}]+)\}\}/g,
        (_match: string, amount: string, code: string) => `${amount.trim()} ${code.trim()}`
      );
      ...
      processed = processed.replace(
        /\{\{date\|([^|}]+)[\|][^}]*\}\}/g,
        (_match: string, date: string) => date.trim()
      );
      processed = processed.replace(/\{\{date\|([^}]+)\}\}/g, (_match: string, date: string) =>
        date.trim()
      );
      ...
      processed = processed.replace(
        /\{\{color\|[^|]+\|([^}]+)\}\}/g,
        (_match: string, text: string) => text.trim()
      );
    ...
    if (text !== processed) {
      ...
      console.log(
        `  Output: "${processed.substring(0, 150)}${processed.length > 150 ? "..." : ""}"`
      );
    }
