...
import { CulturalScenarioGenerator, CULTURAL_SCENARIO_TEMPLATES, type CulturalScenarioType, type ScenarioContext } from './cultural-scenario-generator';
import { MarkovDiplomacyEngine, type RelationshipState as MarkovRelationshipState } from './diplomatic-markov-engine';
...
export interface RelationshipState {
  ...
  relationship: 'alliance' | 'friendly' | 'cooperative' | 'neutral' | 'cool' | 'strained' | 'hostile';
...
export interface CountryPersonality {
  archetype: 'aggressive' | 'defensive' | 'mercantile' | 'diplomatic' | 'isolationist' | 'expansionist';
  traits: {
...
export interface DiplomaticEvent {
  id: string;
  type: 'trade_dispute' | 'alliance_offer' | 'cultural_exchange_offer' |
        'cultural_exchange_proposal' | 'sanction_threat' | 'crisis_mediation' |
        'treaty_proposal' | 'economic_cooperation' | 'security_pact' | 'border_tension' |
        'intelligence_sharing' | 'joint_venture' | 'resource_conflict';
...
export class DiplomaticResponseAI {
  ...
  static calculateEventProbabilities(
    ...
    const highTradeRelations = worldState.relationships.filter(r =>
      r.tradeVolume && r.tradeVolume > 500000
    );
    if (highTradeRelations.length > 0) {
      const tradeDisputeProbability = Math.min(0.4,
    ...
    const strongRelations = worldState.relationships.filter(r =>
      r.strength > 75 && r.relationship === 'friendly'
    );
    const existingAlliances = worldState.relationships.filter(r =>
      r.relationship === 'alliance'
    ).length;

    if (strongRelations.length > 0 && existingAlliances < 3) {
      const allianceOfferProbability = Math.min(0.35,
    ...
    const embassiesWithoutExchanges = worldState.embassies.filter(e =>
      e.level >= 2 &&
      !worldState.relationships.find(r =>
        r.targetCountry === e.country && r.culturalExchange === 'High'
      )
    );

    if (embassiesWithoutExchanges.length > 0) {
      const culturalExchangeProbability = Math.min(0.45,
    ...
    const culturalExchangePartners = worldState.relationships.filter(r =>
      r.culturalExchange === 'High' && r.strength > 50
    );
    const activeCulturalEmbassies = worldState.embassies.filter(e =>
      e.specialization === 'cultural' && e.level >= 2
    );

    if (culturalExchangePartners.length > 0 || activeCulturalEmbassies.length > 0) {
      const culturalProposalProbability = Math.min(0.50,
        (culturalExchangePartners.length * 0.12) +
        (activeCulturalEmbassies.length * 0.10) +
      ...
      probabilities.push({
        ...
        reasoning: 'Active cultural programs and strong relationships enable rich scenario-based exchanges',
        contextFactors: [
    ...
    const deterioratingRelations = worldState.relationships.filter(r =>
      r.strength < 30 && (r.relationship === 'strained' || r.relationship === 'hostile')
    );

    if (deterioratingRelations.length > 0) {
      const sanctionThreatProbability = Math.min(0.3,
    ...
    if (playerReputation.historicalPatterns.interventionist && context.globalTensions > 50) {
      const mediationProbability = Math.min(0.4,
    ...
    const potentialTreatyPartners = worldState.relationships.filter(r =>
      r.strength > 60 && r.treaties.length < 2
    );

    if (potentialTreatyPartners.length > 0) {
      const treatyProbability = Math.min(0.38,
    ...
    if (playerReputation.historicalPatterns.prefersTrade) {
      const cooperationProbability = Math.min(0.42,
        (worldState.economicData.gdpGrowth / 10) * 0.15 +
        (worldState.relationships.filter(r => r.tradeVolume && r.tradeVolume > 100000).length * 0.08) +
    ...
    const alliancePartners = worldState.relationships.filter(r =>
      r.relationship === 'alliance' && !r.treaties.includes('Defense Pact')
    );

    if (alliancePartners.length > 0 && context.globalTensions > 40) {
      const securityPactProbability = Math.min(0.36,
    ...
    const intelligenceCapableEmbassies = worldState.embassies.filter(e =>
      e.level >= 3 && e.strength > 65
    );

    if (intelligenceCapableEmbassies.length > 0) {
      const intelligenceProbability = Math.min(0.33,
  ...
  static generateContextualEvent(
    ...
    switch (eventType) {
      ...
      case 'alliance_offer':
        event = {
          ...
          potentialConsequences: {
            accept: ['Strategic alliance formed', 'Defense obligations incurred', 'Regional power balance shifts'],
            reject: ['Opportunity lost', 'Relationship cooling possible', 'Independence maintained'],
            negotiate: ['Custom alliance terms', 'Extended negotiation period', 'Flexible commitment level']
          }
        };
        break;
      ...
      case 'cultural_exchange_offer':
        event = {
          ...
          potentialConsequences: {
            accept: ['Cultural ties deepen', 'Soft power increases', 'Long-term relationship benefits'],
            reject: ['Opportunity lost', 'Minimal relationship impact', 'Resources saved'],
            negotiate: ['Balanced program', 'Moderate resource commitment', 'Steady cultural gains']
          }
        };
        break;
      ...
      case 'cultural_exchange_proposal':
        ...
        const scenarioContext: ScenarioContext = {
          ...
          existingExchanges: worldState.relationships.filter(r => r.culturalExchange === 'High').length,
        ...
        const scenarioResponseOptions: EventResponseOption[] = culturalScenario.responseOptions.map(option => ({
          id: option.id,
          label: option.label,
          description: option.description,
          expectedOutcome: option.predictedOutcomes.shortTerm.description,
          risks: option.requirements.map(req => `Requires ${req.skill} skill level ${req.level}`),
          benefits: [
        ...
        event = {
          ...
          potentialConsequences: {
            accept: [
              culturalScenario.responseOptions[0]?.predictedOutcomes.longTerm.description || 'Positive cultural outcome',
      ...
      case 'crisis_mediation':
        event = {
          ...
          potentialConsequences: {
            accept: ['Regional leadership gained', 'Success enhances influence dramatically', 'Failure damages reputation'],
            reject: ['Mediator reputation lost', 'Crisis continues', 'Resources saved'],
            negotiate: ['Moderate engagement', 'Shared mediation responsibility', 'Limited risk and reward']
          }
        };
        break;
    ...
    event.contextualRelevance = this.calculateContextualRelevance(eventType, context, playerReputation);
    event.metadata = {
  ...
  private static buildEventContext(worldState: WorldState): EventGenerationContext {
    ...
    const hostileRelations = worldState.relationships.filter(r =>
      r.relationship === 'hostile' || r.relationship === 'strained'
    ).length;
    const globalTensions = Math.min(100,
      hostileRelations * 20 +
      (worldState.relationships.filter(r => r.strength < 30).length * 10)
    );

    // Calculate economic competition
    const highTradeCompetitors = worldState.relationships.filter(r =>
      r.tradeVolume && r.tradeVolume > 300000 && r.strength < 60
    ).length;
    const economicCompetition = Math.min(100,
      highTradeCompetitors * 25 +
      (worldState.economicData.tradeBalance < 0 ? 20 : 0)
    );
  ...
  private static selectSourceCountry(
    ...
    switch (eventType) {
      case 'trade_dispute':
        candidates = worldState.relationships.filter(r =>
          r.tradeVolume && r.tradeVolume > 500000
        );
        break;
      ...
      case 'cultural_exchange_offer':
        candidates = worldState.relationships.filter(r =>
          r.strength > 60 &&
          worldState.embassies.some(e => e.country === r.targetCountry)
        );
        break;
  ...
  private static calculateContextualRelevance(
    ...
    switch (eventType) {
      ...
      case 'cultural_exchange_proposal':
        ...
        relevance += context.worldState.relationships.filter(r => r.culturalExchange === 'High').length > 2 ? 25 : 0;
        break;
  ...
  static inferCountryPersonality(
    ...
    const avgStrength = relationships.reduce((sum, r) => sum + r.strength, 0) / Math.max(relationships.length, 1);
    const allianceCount = relationships.filter(r => r.relationship === 'alliance').length;
    const hostileCount = relationships.filter(r => r.relationship === 'hostile' || r.relationship === 'strained').length;
    const tradeRelations = relationships.filter(r => r.tradeVolume && r.tradeVolume > 100000).length;

    // Calculate personality traits
    const assertiveness = Math.min(100,
      (hostileCount * 20) +
      (relationships.filter(r => r.strength < 40).length * 10) +
      30 // Base assertiveness
    );

    const cooperativeness = Math.min(100,
      (allianceCount * 15) +
      (avgStrength / 2) +
      (relationships.filter(r => r.relationship === 'friendly' || r.relationship === 'cooperative').length * 8)
    );

    const economicFocus = Math.min(100,
      (tradeRelations * 15) +
      (relationships.filter(r => r.treaties?.includes('Trade Agreement')).length * 12) +
      20 // Base economic focus
    );

    const culturalOpenness = Math.min(100,
      (relationships.filter(r => r.culturalExchange === 'High').length * 20) +
      40 // Base openness
    );

    const riskTolerance = Math.min(100,
      (hostileCount * 15) +
      (relationships.filter(r => r.strength < 30).length * 10) +
      50 // Base tolerance
    );
    ...
    return {
      ...
      historicalBehavior: DiplomaticChoiceTracker.getCumulativeEffects(countryId, historicalActions)
    };
  }
   ...
   */
  private static mapToMarkovState(relationship: RelationshipState['relationship']): MarkovRelationshipState {
