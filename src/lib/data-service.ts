...
export class IxStatsDataService {
  ...
  initializeCountries(baseData: BaseCountryData[]): CountryStats[] {
    ...
    console.log(
      `[DataService] Countries initialized with baseline: ${IxTime.formatIxTime(IxTime.getInGameEpoch())}`
    );
    return initialized;
  }
  ...
  validateEpochAlignment(countries: BaseCountryData[]): {
    ...
    const avgPopulation =
      countries.length > 0
        ? countries.reduce((sum, c) => sum + c.population, 0) / countries.length
        : 0;
    const avgGdpPerCapita =
      countries.length > 0
        ? countries.reduce((sum, c) => sum + c.gdpPerCapita, 0) / countries.length
        : 0;

    if (avgPopulation > 500000000) {
      warnings.push(
        "Average population seems very high - check if data is from correct time period"
      );
    }
    if (avgGdpPerCapita > 100000) {
      warnings.push(
        "Average GDP per capita seems very high - verify data represents 2028 baseline"
      );
    }

    const countriesWithAreaButNoDensity = countries.filter(
      (c) => c.landArea && c.landArea > 0
    ).length;
    if (countriesWithAreaButNoDensity > 0) {
      console.log(
        `[DataService] ${countriesWithAreaButNoDensity} countries have land area data for density calculations`
      );
    }
