...
describe("computeForeignPolicyImpact — free trade asymmetry (Keaor)", () => {
  it("a richer partner gives you more GDP than a poorer one", () => {
    const vsRich = computeForeignPolicyImpact({ initiator: mid, target: rich, actionType: "free_trade" });
    const vsPoor = computeForeignPolicyImpact({ initiator: mid, target: poor, actionType: "free_trade" });
  ...
  it("a bigger imbalance gives a bigger relations boost", () => {
    const vsEqual = computeForeignPolicyImpact({ initiator: mid, target: mid, actionType: "free_trade" });
    const vsPoor = computeForeignPolicyImpact({ initiator: mid, target: poor, actionType: "free_trade" });
  ...
  it("the poorer side gains more GDP from the deal than the richer side", () => {
    const r = computeForeignPolicyImpact({ initiator: rich, target: poor, actionType: "free_trade" });
...
describe("computeForeignPolicyImpact — coercive tools", () => {
  it("embargo/blockade hurt both sides and tank relations", () => {
    ...
    expect(computeForeignPolicyImpact({ initiator: mid, target: mid, actionType: "blockade" }).category).toBe("military");
  });

  it("severity scales magnitude", () => {
    const light = computeForeignPolicyImpact({ initiator: mid, target: mid, actionType: "sanction", severity: "light" });
    const severe = computeForeignPolicyImpact({ initiator: mid, target: mid, actionType: "sanction", severity: "severe" });
