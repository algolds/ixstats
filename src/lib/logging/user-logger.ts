...
import { logger, LogLevel, LogCategory } from "~/lib/system/logger";
...
export class UserLogger {
   ...
   */
  static async logUserAction(context: UserLogContext, action: UserAction): Promise<void> {
    try {
  ...
  static async getUserActivitySummary(
    ...
    try {
      ...
      summary.peakActivityHour = Object.entries(hourCounts).reduce(
