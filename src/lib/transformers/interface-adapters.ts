...
export const adaptLegacyExecutiveAction = (
...
export const adaptExecutiveToQuick = (
  action: Record<string, unknown> | ExecutiveAction
): QuickAction => {
  // First convert legacy format to unified format if needed
  const unifiedAction = action.type
    ? (action as ExecutiveAction)
    : adaptLegacyExecutiveAction(action as Record<string, unknown>);
...
const calculateEstimatedTime = (action: ExecutiveAction): string => {
  ...
  const urgencyMultiplier =
    action.urgency === "critical" ? 0.5 : action.urgency === "high" ? 0.75 : 1;
...
export const unifyIntelligenceItem = (
  ...
  affectedRegions: isStringArray(item.affectedRegions)
    ? item.affectedRegions
    : typeof item.affectedCountries === "string"
      ? item.affectedCountries.split(",")
      : [],
  ...
  content: isString(item.description)
    ? item.description
    : isString(item.message)
      ? item.message
      : isString(item.content)
        ? item.content
        : "",
  relatedCountries: isStringArray(item.affectedRegions)
    ? item.affectedRegions
    : typeof item.affectedCountries === "string"
      ? item.affectedCountries.split(",")
      : [],
...
export const adaptCountryForSocialProfile = (
