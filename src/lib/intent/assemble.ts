...
export function forbiddenFieldsUsed(pkgs: IntentPackage[]): string[] {
  ...
  for (const p of pkgs)
    for (const c of p.consequences)
      if (FORBIDDEN_FIELDS.has(c.targetField)) hits.push(c.targetField);
  return hits;
}
...
export function weightAcceptance(
  ...
  if (opts.brokerUnlocked)
    i = Math.min(2, i + 1); // acting in a broker's domain pleases them
...
const RECIPES: Record<Category, Recipe> = {
  ...
  economy: {
    budgetLine: "Commerce & Industry",
    policyName: "Industrial Strategy",
  ...
  social: {
    ...
    policyName: "Public Services Reform",
  ...
  infrastructure: {
    budgetLine: "Public Works",
    policyName: "Infrastructure Program",
...
function buildPackage(tier: Tier, cat: Category, goal: string, target?: string): IntentPackage {
  ...
  if (tier === "measured") {
    ...
    changes.push({
      kind: "policy",
      label: `Enact "${r.policyName}"`,
      detail: tier === "extreme" ? "an aggressive mandate" : "a targeted mandate",
    });
