...
import { EnhancedEconomicCalculator, type ComprehensiveEconomicAnalysis } from './enhanced-economic-calculations';
import { EconomicCalculationGroups, type GroupedAnalysisResult } from './economic-calculation-groups';
...
export class IntuitiveEconomicAnalysis {
  ...
  async analyzeEconomy(
    ...
    const summary = this.generateHealthSummary(countryStats, economyData, detailedAnalysis, groupedAnalysis);
    const insights = this.generateActionableInsights(countryStats, economyData, detailedAnalysis, groupedAnalysis);
    const story = this.generateEconomicStory(countryStats, economyData, historicalData, detailedAnalysis);
  ...
  quickHealthCheck(countryStats: CountryStats, economyData: EconomyData): EconomicHealthSummary {
    ...
    return {
      ...
      healthIndicators: {
        ...
        stability: debt <= 60 && inflation <= 0.05 ? 'high' : debt <= 90 && inflation <= 0.08 ? 'medium' : 'low',
  ...
  private runGroupedAnalysis(
    ...
    const growthDynamics = this.groupCalculator.calculateGrowthDynamics(countryStats, economyData, historicalData);
    const financialHealth = this.groupCalculator.calculateFinancialHealth(countryStats, economyData, historicalData);
    const humanDevelopment = this.groupCalculator.calculateHumanDevelopment(countryStats, economyData);
    const economicStructure = this.groupCalculator.calculateEconomicStructure(countryStats, economyData);
    const externalRelations = this.groupCalculator.calculateExternalRelations(countryStats, economyData);
    ...
    const overallScore = (
      ...
      externalRelations.overallScore * 0.10
    );
    ...
    return {
      ...
      strengths: this.identifyGroupStrengths([growthDynamics, financialHealth, humanDevelopment, economicStructure, externalRelations]),
      challenges: this.identifyGroupChallenges([growthDynamics, financialHealth, humanDevelopment, economicStructure, externalRelations]),
      priorityActions: this.generateGroupPriorityActions([growthDynamics, financialHealth, humanDevelopment, economicStructure, externalRelations])
    };
  }
  ...
  private generateActionableInsights(
    ...
    if (economyData.fiscal.totalDebtGDPRatio > 80) {
      watchAreas.push({
        ...
        earlyWarningSignals: ['Rising interest payments', 'Credit rating concerns', 'Market volatility'],
  ...
  private generateEconomicStory(
    ...
    currentSituation += unemployment < 6 ? 'labor markets are tight' : unemployment > 10 ? 'job creation remains a challenge' : 'employment levels are moderate';
    currentSituation += inflation < 4 ? ' while price stability is maintained.' : inflation > 8 ? ' amid concerning price pressures.' : ' with manageable price dynamics.';
    ...
    if (growth > 0.04) {
      ...
      recentProgress += 'has been challenging, with economic contraction requiring policy intervention.';
    }
    ...
    if (economicTier === 'Impoverished' || economicTier === 'Developing') {
      futurePotential = 'The economy has significant catch-up potential through infrastructure development, human capital investment, and institutional strengthening.';
    } else if (economicTier === 'Developed' || economicTier === 'Healthy') {
      futurePotential = 'Future growth depends on productivity gains, innovation capacity, and successful adaptation to global economic shifts.';
    } else {
      futurePotential = 'As an advanced economy, future prosperity relies on maintaining competitiveness through innovation, sustainable development, and quality institutions.';
    }

    // Comparative perspective
    const comparativePerspective = `Compared to similar economies, ${countryStats.name} ${
      detailedAnalysis.overallRating.score > 80 ? 'performs strongly across key metrics' :
      detailedAnalysis.overallRating.score > 60 ? 'shows mixed performance with areas for improvement' :
    ...
    const peerComparisons = [
      {
        ...
        gap: this.calculateGap(countryStats.currentGdpPerCapita, this.getPeerAverage(countryStats.economicTier, 'gdpPerCapita'))
      },
      {
        ...
        gap: this.calculateGap(economyData.labor.unemploymentRate, this.getPeerAverage(countryStats.economicTier, 'unemployment'))
      }
    ];
    ...
    const policyScenarios = [
      {
        ...
        assumptions: ['2% of GDP annual investment', 'Productivity gains from better infrastructure', 'Short-term fiscal expansion'],
        impacts: {
          gdpGrowthChange: 0.8,
      ...
      },
      {
        ...
        assumptions: ['Technology adoption', 'Skills retraining programs', 'Regulatory modernization'],
        impacts: {
          gdpGrowthChange: 1.2,
      ...
      },
      {
        ...
        assumptions: ['Higher education spending', 'Vocational training expansion', 'University partnerships'],
        impacts: {
          gdpGrowthChange: 0.6,
    ...
    const riskScenarios = [
      {
        ...
        preparedness: currentDebt < 80 ? 'Well prepared' as const : 'Somewhat prepared' as const
      },
      {
        name: 'Debt Crisis',
  ...
  private determineTrend(countryStats: CountryStats, economyData: EconomyData): EconomicHealthSummary['trend'] {
  ...
  private generateKeyMessage(score: number, countryStats: CountryStats, economyData: EconomyData): string {
    if (score >= 85) {
  ...
  private assessSustainability(countryStats: CountryStats, economyData: EconomyData): 'excellent' | 'good' | 'concerning' {
    const debt = economyData.fiscal.totalDebtGDPRatio;
    const inflation = economyData.core.inflationRate;
    const growth = countryStats.adjustedGdpGrowth;
  ...
  private calculateGap(userValue: number, peerAverage: number): string {
    ...
    return gap > 0 ? `${gap.toFixed(0)}% above peer average` : `${Math.abs(gap).toFixed(0)}% below peer average`;
  }
  ...
  private estimateCompetitivenessRank(countryStats: CountryStats, economyData: EconomyData): number {
  ...
  private identifyGroupStrengths(groups: Array<{ overallScore: number }>): string[] {
    return groups
      .map((group, index) => ({ 
        name: ['Growth Dynamics', 'Financial Health', 'Human Development', 'Economic Structure', 'External Relations'][index], 
