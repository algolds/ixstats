...
export const TAX_EFFECTIVENESS_MODIFIERS: Partial<
  Record<
    ComponentType,
    {
...
> = {
  ...
  [ComponentType.REGIONAL_DEVELOPMENT]: {
    ...
    auditCapacity: 0.88,
    description:
      "Regional development focus may complicate central tax collection but builds local compliance",
  },
} as const;
...
export function calculateAtomicTaxEffectiveness(
  ...
  for (let i = 0; i < components.length; i++) {
    for (let j = i + 1; j < components.length; j++) {
      const synergyKey =
        `${components[i]}+${components[j]}` as keyof typeof TAX_COMPONENT_SYNERGIES;
      const synergy = TAX_COMPONENT_SYNERGIES[synergyKey];

      if (synergy) {
        if ((synergy as any).collectionEfficiency)
          collectionMultiplier *= (synergy as any).collectionEfficiency;
        if ((synergy as any).complianceRate)
          complianceMultiplier *= (synergy as any).complianceRate;
      ...
      // Check for conflicts
      const conflictKey =
        `${components[i]}+${components[j]}` as keyof typeof TAX_COMPONENT_CONFLICTS;
      const conflict = TAX_COMPONENT_CONFLICTS[conflictKey];

      if (conflict) {
        if ((conflict as any).collectionEfficiency)
          collectionMultiplier *= (conflict as any).collectionEfficiency;
        if ((conflict as any).complianceRate)
          complianceMultiplier *= (conflict as any).complianceRate;
  ...
  const finalCollectionEfficiency = Math.min(
    100,
    baseTaxSystem.collectionEfficiency * collectionMultiplier
  );
 ...
 */
export function getAtomicTaxRecommendations(components: ComponentType[]): {
  ...
  if (
    components.includes(ComponentType.ELECTORAL_LEGITIMACY) ||
