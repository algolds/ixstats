...
import { getIxWikiPool, isMysqlAvailable, markMysqlOffline, markMysqlOnline } from "./mysql-pool";
import {
  type WikiArticle,
...
export async function ixwikiRecentChanges(limit: number = 20): Promise<WikiRecentChange[]> {
  ...
  if (isMysqlAvailable()) {
    try {
      ...
      markMysqlOffline();
      console.warn("[WikiBridge] MySQL recent changes error, falling back to HTTP:", err);
    }
  }
  ...
  try {
    ...
    if (res.ok) {
      ...
      const rcList = data?.query?.recentchanges || [];
      const results: WikiRecentChange[] = rcList.map((rc: any) => ({
        title: rc.title,
        user: rc.user || "Unknown",
    ...
    console.error("[WikiBridge] HTTP recent changes fallback error:", err);
  }
...
export async function ixwikiGetUserInfo(username: string): Promise<{
  ...
  try {
    ...
    if (!userRows || userRows.length === 0) {
      return { exists: false, userId: null, username, editCount: 0, registration: null, groups: [] };
    }
