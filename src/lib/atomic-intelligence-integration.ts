...
const STABILITY_MODIFIERS: Partial<
  Record<ComponentType, { stability: number; legitimacy: number; capacity: number; policy: number }>
> = {
  // Power Distribution
...
export function calculateAtomicGovernmentStability(
  ...
  const stabilityTrend: "improving" | "stable" | "declining" =
    strengths.length > riskFactors.length
      ? "improving"
      : riskFactors.length > strengths.length
        ? "declining"
        : "stable";
...
export async function generateAtomicIntelligence(
  ...
  if (components.length === 0) {
    intelligence.push({
      ...
      description:
        "Country lacks defined atomic government components, limiting governance analysis.",
      category: "governance",
      timestamp: new Date(),
      ...
      actionable: true,
      recommendations: [
        "Define government power distribution structure",
        "Establish decision-making processes",
        "Set up legitimacy sources",
      ],
    });
    return intelligence;
  }
  ...
  const economic = calculateAtomicEconomicImpact(
    components,
    economicData.gdpPerCapita || 15000,
    (economicData as any).taxRevenue || 0.2
  );
  ...
  if ((economic as any).overallScore < 60) {
    intelligence.push({
      ...
      actionable: true,
      recommendations:
        (economic as any).synergies.length > 0
    ...
    for (let j = i + 1; j < components.length; j++) {
      ...
      if (synergy) {
        intelligence.push({
          ...
          actionable: true,
          recommendations: [
            "Leverage this synergy for enhanced effectiveness",
            "Build upon this institutional strength",
          ],
        });
      }
    }
  ...
  if (
    ...
    intelligence.push({
      ...
      actionable: false,
      recommendations: [
        "Capitalize on implementation advantages",
        "Consider ambitious reform agenda",
      ],
      metrics: {
