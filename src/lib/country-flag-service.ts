...
class CountryFlagService {
  ...
  private async searchWikimediaFiles(query: string): Promise<FlagSearchResult[]> {
    try {
      ...
      for (const result of searchResults) {
        if (result.title?.startsWith("File:") && /\.(svg|png|jpg|jpeg)$/i.test(result.title)) {
   ...
   */
  private findBestFlagMatch(
    results: FlagSearchResult[],
    countryName: string
  ): FlagSearchResult | null {
    ...
    const scoredResults = results.map((result) => {
      ...
      if (
        filename.includes(`flag_of_${normalizedCountry}`) ||
  ...
  getCacheStats() {
    ...
    const successful = Array.from(this.flagCache.values()).filter(
      (flag) => flag.flagUrl !== null
    ).length;
...
export const countryFlagServiceCommonsOnly = new CountryFlagService({
  enableIiwikiFallback: false,
}); // Commons only
