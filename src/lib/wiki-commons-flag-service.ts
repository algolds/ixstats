...
class WikiCommonsFlagService {
  ...
  async getFlagUrl(countryName: string): Promise<string | null> {
    ...
    try {
      ...
      if (typeof flagResult === 'string') {
        console.log(`[WikiCommonsFlagService] Successfully fetched flag for ${countryName}: ${flagResult}`);
  ...
  async getCoatOfArmsUrl(countryName: string): Promise<string | null> {
    ...
    try {
        ...
        for (const field of coatOfArmsFields) {
          if (typeof field === 'string' && field.trim()) {
            ...
            if (typeof fileUrl === 'string') {
              console.log(`[WikiCommonsFlagService] Found coat of arms for ${countryName}: ${fileUrl}`);
              return fileUrl;
            }
          }
        }
      ...
      console.error(`[WikiCommonsFlagService] Error getting coat of arms for ${countryName}:`, error);
      return null;
    }
  }
