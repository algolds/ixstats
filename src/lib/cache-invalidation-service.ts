...
export class CacheInvalidationService {
  ...
  removeInvalidationRule(pattern: string | RegExp): boolean {
    ...
    this.rules = this.rules.filter(rule => 
      ...
      (typeof pattern === 'string' ? rule.pattern.toString() !== pattern : rule.pattern !== pattern)
    );
  ...
  getStats(): CacheInvalidationStats {
    const avgTime = this.invalidationTimes.length > 0 
  ...
  getCacheHealthMetrics(): {
    ...
    healthScore += Math.min(100, (1000 / Math.max(1, invalidationStats.averageInvalidationTime)) * 10); // 40% weight on invalidation speed
