 ...
 */
export function generateCoreEconomicIndicators(
  ...
  return {
    ...
    regionalEmployment: {
      urban: {
        ...
        averageIncome:
          getAverageIncome(profile.gdpPerCapita) * getUrbanIncomeMultiplier(profile.economicTier),
      },
      rural: {
        ...
        averageIncome:
          getAverageIncome(profile.gdpPerCapita) * getRuralIncomeMultiplier(profile.economicTier),
      },
    },
 ...
 */
export function generateIncomeWealthDistribution(
  profile: CountryProfile
): IncomeWealthDistributionData {
...
export function generateGovernmentSpendingData(profile: CountryProfile): GovernmentSpendingData {
  ...
  const education =
    spendingCategories.find((c) => c.category.toLowerCase().includes("education"))?.amount || 0;
  const healthcare =
    spendingCategories.find((c) => c.category.toLowerCase().includes("health"))?.amount || 0;
  const socialSafety =
    spendingCategories.find((c) => c.category.toLowerCase().includes("social"))?.amount || 0;
...
function getDefaultTaxRates(tier: string) {
  const baseRate = tier === "Developing" ? 0.15 : tier === "Emerging" ? 0.2 : 0.25;
  ...
  return {
    ...
    exciseTaxRates: [
      { type: "Tobacco", rate: 0.5 },
      { type: "Alcohol", rate: 0.3 },
      { type: "Fuel", rate: 0.2 },
    ],
