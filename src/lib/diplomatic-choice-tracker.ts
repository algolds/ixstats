...
export interface DiplomaticChoice {
  id: string;
  countryId: string;
  type: 'establish_embassy' | 'upgrade_embassy' | 'close_embassy' |
        'sign_treaty' | 'break_treaty' | 'cultural_exchange' |
        'diplomatic_message' | 'share_intelligence' | 'trade_agreement' |
        'alliance_formation' | 'sanctions' | 'mediation' |
        'cultural_exchange_initiated' | 'cultural_exchange_joined' |
        'cultural_scenario_response' | 'cultural_exchange_success' |
        'cultural_exchange_failure' | 'create_cultural_exchange' |
        'join_cultural_exchange' | 'complete_cultural_exchange' |
        'cancel_cultural_exchange' | 'generate_cultural_scenario' |
        'respond_to_cultural_scenario' | 'vote_on_cultural_exchange' |
...
export class DiplomaticChoiceTracker {
  ...
  static getCumulativeEffects(
    ...
    const relationshipChoices = choices.filter(c =>
      ['establish_embassy', 'sign_treaty', 'cultural_exchange', 'alliance_formation',
       'create_cultural_exchange', 'join_cultural_exchange', 'complete_cultural_exchange'].includes(c.type)
    );
    ...
    const cooperativeChoices = choices.filter(c =>
      ['cultural_exchange', 'share_intelligence', 'mediation', 'trade_agreement',
       'create_cultural_exchange', 'join_cultural_exchange', 'complete_cultural_exchange',
       'generate_cultural_scenario', 'respond_to_cultural_scenario', 'vote_on_cultural_exchange',
       'upload_cultural_artifact'].includes(c.type)
    );
    cooperativeness = Math.min((cooperativeChoices.length / Math.max(choices.length, 1)) * 100, 100);
  ...
  static previewRippleEffects(
    ...
    switch (choice.type) {
      ...
      case 'sign_treaty':
        ...
        if (treatyType === 'trade') {
          ...
          const rivals = currentNetwork.relationships.filter(
            r => r.relationship === 'tension'
          );
          if (rivals.length > 0) {
            secondOrderEffects.push({
              type: 'security',
    ...
    const riskLevel: RippleEffectPreview['riskLevel'] =
      totalNegativeImpact > 100 ? 'critical' :
      totalNegativeImpact > 60 ? 'high' :
      totalNegativeImpact > 30 ? 'medium' : 'low';
  ...
  private static groupChoicesByType(choices: DiplomaticChoice[]): Record<string, DiplomaticChoice[]> {
  ...
  private static calculateCulturalMetrics(
    ...
    const varietyBonus = Object.keys(choicesByType).filter(type =>
      type.includes('cultural')
    ).length * 8; // +8 per different cultural action type

    const culturalDiplomacyScore = Math.min(culturalFrequency + varietyBonus, 100);

    // Calculate exchange success rate (0-100)
    const successfulExchanges = (choicesByType['cultural_exchange_success'] || []).length +
                                (choicesByType['complete_cultural_exchange'] || []).length;
    const failedExchanges = (choicesByType['cultural_exchange_failure'] || []).length +
                           (choicesByType['cancel_cultural_exchange'] || []).length;
    const totalCompleted = successfulExchanges + failedExchanges;

    const exchangeSuccessRate = totalCompleted > 0
      ? (successfulExchanges / totalCompleted) * 100
      : 50; // Default neutral if no completed exchanges
  ...
  private static identifyPatterns(
    ...
    const createdExchanges = (choicesByType['create_cultural_exchange'] || []).length +
    ...
    return {
      ...
      multilateral: choices.filter(c =>
        c.details.participantCount && c.details.participantCount > 2
      ).length / total > 0.15,
    };
  }
