...
function scoreImageRelevance(candidate: string, rawQuery: string): number {
  ...
  const allPresent = qTokens.every((qt) =>
...
async function searchWithCategoryFilter(
  ...
  try {
    ...
    const allCategoryMembers = await getAllCategoryMembersWithSubcategories(
      categoryFilter,
      config,
      site
    );
    ...
    console.log(
      `[WikiSearch] Found ${allCategoryMembers.length} total members in Category:${categoryFilter}`
    );
    ...
    if (filteredMembers.length > 50) {
      console.log(
        `[WikiSearch] Too many results (${filteredMembers.length}), doing targeted search`
      );
...
async function getAllCategoryMembers(categoryFilter: string, config: WikiConfig): Promise<any[]> {
  ...
  do {
    ...
    console.log(
      `[WikiSearch] Retrieved ${members.length} members (total: ${allMembers.length}, continue: ${!!cmcontinue})`
    );
...
async function getCategorySubcategories(
  ...
  do {
    ...
    console.log(
      `[WikiSearch] Found ${categoryNames.length} subcategories in ${categoryFilter} (total: ${subcategories.length})`
    );
  } while (cmcontinue && iterations < maxIterations);
 ...
 */
async function getAllCategoryMembersWithSubcategories(
  categoryFilter: string,
  config: WikiConfig,
  site: "ixwiki" | "iiwiki" | "althistory"
): Promise<any[]> {
  ...
  console.log(
    `[WikiSearch] Found ${subcategories.length} subcategories:`,
  ...
  console.log(
    `[WikiSearch] Total members: ${directMembers.length} direct + ${subcategoryMembers.length} from subcategories`
  );
...
async function performTargetedSearch(
  ...
  try {
    ...
    return searchResults.filter((result: any) => candidateSet.has(result.title.toLowerCase()));
  } catch (error) {
...
export async function parseCountryInfobox(
  ...
  try {
    ...
    throw new Error(
      `Failed to parse country data: ${error instanceof Error ? error.message : "Unknown error"}`
    );
  }
}
 ...
 */
function parseInfoboxParameters(
 ...
 */
function findParameterBoundaries(
  content: string
 ...
 */
async function extractEconomicData(
  infobox: CountryInfoboxWithDynamicProps,
  site: "ixwiki" | "iiwiki" | "althistory"
): Promise<Partial<ParsedCountryData>> {
  ...
  // Extract area
  const areaFields = [infobox.area_km2, infobox.area, infobox.area_total];
  ...
  if (data.flag) {
    try {
      ...
      console.warn(
        `Failed to get flag from unified service for ${infobox.name}, falling back to direct image URL`,
        error
      );
  ...
  if (cleaned.toLowerCase().startsWith("file:")) {
    ...
    if (fileMatch) {
      ...
      const htmlMatch = cleaned.match(
        /href=["'][^"']*\/([^/"']+\.(png|jpg|jpeg|gif|svg|webp))["']/i
      );
      if (htmlMatch) {
...
async function getImageUrl(
  ...
  try {
    ...
    const data = await response.json();
    console.log(
      `[WikiImageSearch] Image info response for ${filename}:`,
...
export async function searchWikiImagesWithPagination(
  ...
  try {
    ...
    console.log(
      `[WikiImageSearch] Found ${allImages.length} images with prefix "${prefixQuery}", continuation: ${continuationToken || "none"}`
    );
    ...
    for (const image of allImages) {
      ...
      const hasValidExtension = validExtensions.some((ext) => imageNameLower.endsWith(`.${ext}`));
    ...
    const uniqueByName = new Map<
      string,
      { name: string; path: string; url?: string; description?: string }
    >();
    ...
    console.log(
      `[WikiImageSearch] Returning ${finalResults.length} image results (ranked), hasMore: ${hasMore}, nextCursor: ${continuationToken || "none"}`
    );
