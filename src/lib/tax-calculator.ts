...
export class TaxCalculatorEngine {
    ...
    for (const category of this.categories) {
      if (category.calculationMethod === "progressive") {
        ...
        if (brackets.length === 0) {
          warnings.push(
            `Progressive tax category "${category.categoryName}" has no brackets defined`
          );
        }
      }
    }
  ...
  generateOptimizationSuggestions(
    ...
    // Unused deductions
    const availableDeductions = this.deductions.filter(
      (d) => !request.deductions?.find((rd) => rd.deductionId === d.id)
    );
    ...
    // Unused exemptions
    const availableExemptions = this.exemptions.filter(
      (e) => !request.exemptions?.find((re) => re.exemptionId === e.id)
    );
