 ...
 */
const DEPARTMENT_CATEGORY_MAP: Record<
  DepartmentCategory,
  {
...
export function convertGovernmentStructureToSpending(
  ...
  const totalBudget =
    "structure" in governmentData
      ? governmentData.structure.totalBudget
      : governmentData.totalBudget;

  const departments =
    "departments" in governmentData && Array.isArray(governmentData.departments)
      ? governmentData.departments
      : [];

  const budgetAllocations =
    "budgetAllocations" in governmentData && Array.isArray(governmentData.budgetAllocations)
  ...
  if (budgetAllocations.length > 0) {
    budgetAllocations.forEach((allocation, index) => {
      const department = departments.find(
        (d, dIndex) => ("id" in d ? d.id === allocation.departmentId : false) || dIndex === index
      );
 ...
 */
export function shouldUseGovernmentStructureData(
