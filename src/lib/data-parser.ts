...
function processRawDataWithHeaderRecognition(
  rawData: any[][],
  isExcel: boolean
): BaseCountryData[] {
  ...
  if (rawData.length < 2) {
    console.warn(
      `Spreadsheet (${isExcel ? "Excel" : "CSV"}) has insufficient data (less than 2 rows).`
    );
    return countries;
  }
  ...
  const headerMap = {
    ...
    population: findHeaderIndex([
      "Population",
      "Current Population",
      "Pop",
      "Population (current)",
    ]),
    gdpPerCapita: findHeaderIndex([
      "GDP PC",
      "GDP per Capita",
      "GDPPC",
      "GDPperCap",
    ...
    populationGrowthRate: findHeaderIndex([
      "Pop Growth Rate",
      "Population Growth",
      "popgrowthrate",
    ]),
  ...
  if (headerMap.country === -1 || headerMap.population === -1 || headerMap.gdpPerCapita === -1) {
    ...
    if (headerMap.country === -1) {
      ...
      headerMap.country = headers.findIndex(
        (h) =>
          h.includes("country") ||
          h.includes("nation") ||
          h.includes("name") ||
    ...
    console.log(
      `  population: ${headerMap.population} (${headers[headerMap.population] || "none"})`
    );
    console.log(
      `  gdpPerCapita: ${headerMap.gdpPerCapita} (${headers[headerMap.gdpPerCapita] || "none"})`
    );

    if (headerMap.country === -1 || headerMap.population === -1 || headerMap.gdpPerCapita === -1) {
      throw new Error(
        `Required headers (Country, Population, GDP per Capita) not found. Found headers: ${headers.join(", ")}`
      );
    }
  }
  ...
  for (let i = headerRowIndex + 1; i < rawData.length; i++) {
    ...
    try {
      ...
      const countryData: BaseCountryData = {
        ...
        continent:
          headerMap.continent !== -1 ? parseStringOptional(row[headerMap.continent]) : null,
        region: headerMap.region !== -1 ? parseStringOptional(row[headerMap.region]) : null,
        governmentType:
          headerMap.governmentType !== -1
            ? parseStringOptional(row[headerMap.governmentType])
            : null,
      ...
      if (
        countryData.projected2040Population === 0 &&
        yearsToTarget > 0 &&
        countryData.population > 0
      ) {
        countryData.projected2040Population =
          countryData.population * Math.pow(1 + countryData.populationGrowthRate, yearsToTarget);
      }
      if (
        countryData.projected2040GdpPerCapita === 0 &&
        yearsToTarget > 0 &&
        countryData.gdpPerCapita > 0
      ) {
        countryData.projected2040GdpPerCapita =
          countryData.gdpPerCapita * Math.pow(1 + countryData.adjustedGdpGrowth, yearsToTarget);
      }
      if (
        countryData.projected2040Gdp === 0 &&
        countryData.projected2040Population > 0 &&
        countryData.projected2040GdpPerCapita > 0
      ) {
        countryData.projected2040Gdp =
          countryData.projected2040Population * countryData.projected2040GdpPerCapita;
      }
      if (countryData.actualGdpGrowth === 0) {
        countryData.actualGdpGrowth =
          (1 + countryData.populationGrowthRate) * (1 + countryData.adjustedGdpGrowth) - 1;
      }
      ...
      if (validateCountryData(countryData)) {
        ...
        console.warn(
          `[DataParser] Invalid or incomplete data for country: ${countryData.country}. Skipping.`
        );
      }
    } catch (error) {
  ...
  console.log(
    `[DataParser] Processed ${countries.length} valid countries from ${isExcel ? "Excel" : "CSV"} file`
  );
  return countries;
}
...
function parseCsvData(csvString: string, headerSkipLines = 0): BaseCountryData[] {
  ...
  if (dataLines.length < 2) {
    console.warn(
      `CSV has insufficient data (less than 2 effective rows after skipping ${headerSkipLines} lines).`
    );
    return [];
  }
