  ...
  for (let z = 0; z <= thresholds.length; z++) {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        // Chaikin smoothing (2 iterations)
        wgs84Ring = chaikinSmooth(wgs84Ring, 2);

        // Douglas-Peucker simplification
        const tolerance = (1 / width) * 360 * 0.3;
        ...
        wgs84Ring = noisyRing(wgs84Ring, featureCount * 97 + regionSize * 31, 2, 0.08);
        if (wgs84Ring.length < 4) continue;
  ...
  for (let ct = 0; ct < typeNames.length; ct++) {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        wgs84Ring = chaikinSmooth(wgs84Ring, 2);
        const tolerance = (1 / width) * 360 * 0.4;
        wgs84Ring = douglasPeuckerSimplify(wgs84Ring, tolerance);
        if (wgs84Ring.length < 4) continue;
        ...
        wgs84Ring = noisyRing(wgs84Ring, featureCount * 53 + ct * 17, 2, 0.06);
        if (wgs84Ring.length < 4) continue;
