...
export const DEFAULT_PARAMS: WorldGenParams = {
  ...
  hasRivers: true,
  hasLakes: true,
  gridResolution: 256, // Lower default for fast generation (~1-2s)
  similarity: 0,
  profileName: "IxWorld",
...
function generateWorldTectonic(params: WorldGenParams, startTime: number): GeneratedWorld {
  ...
  const plateCount = Math.max(6, params.continentCount * 2 + 4);
  const erosionIntensity = params.erosionIntensity ?? 0.5;
  ...
  if (params.hasRivers) {
    const drainage = extractDrainageNetwork(
      tecResult.elevation,
      tecResult.isOcean,
      width,
      height,
