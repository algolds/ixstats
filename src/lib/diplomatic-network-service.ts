...
export class DiplomaticNetworkService {
  ...
  static getSynergyBonuses(networkData: {
    ...
    const researchTreaties = relationships.filter(
      (r: any) => r.treaties?.includes('Research Cooperation')
    ).length;
    const culturalPrograms = exchanges.filter(
      (e: any) => e.type === 'technology' || e.type === 'education'
    ).length;

    const researchSpeedBonus = Math.min(50,
      researchTreaties * 5 +
      culturalPrograms * 3 +
      (networkPower / 10)
    );
    ...
    const culturalEmbassies = embassies.filter(
      (e: any) => e.specialization === 'cultural'
    ).length;
    const activeExchanges = exchanges.filter(
      (e: any) => e.status === 'active'
    ).length;
    const totalParticipants = exchanges.reduce(
      (sum: number, e: any) => sum + (e.metrics?.participants || 0), 0
    );

    const culturalInfluenceBonus = Math.min(100,
      culturalEmbassies * 10 +
      activeExchanges * 5 +
    ...
    const intelligenceGatheringBonus = Math.min(30,
      securityEmbassies * 4 +
      allianceRelationships * 5 +
      (embassies.length * 1.5) +
      (networkPower / 15)
    );
  ...
  static getStrategicOpportunities(context: {
    ...
    if (embassies.length < 8) {
      ...
      if (strongRelationsWithoutEmbassy.length > 0) {
        ...
        opportunities.push({
          ...
          prerequisites: ['Diplomatic approval', 'Budget allocation ($50,000+)', 'Ambassador appointment'],
    ...
    const upgradeableEmbassies = embassies.filter(
      (emb: any) => emb.strength > 75 && emb.level < 3
    );
    if (upgradeableEmbassies.length > 0) {
      ...
      opportunities.push({
        ...
        prerequisites: [`Embassy Level ${best.level}`, 'Budget ($25,000+)', 'Performance threshold met'],
    ...
    const strongRelationsWithoutTrade = relationships.filter((rel: any) => {
      const hasTradeTreaty = rel.treaties?.some(
        (t: string) => t.toLowerCase().includes('trade')
      );
    ...
    const allianceCandidates = relationships.filter((rel: any) => {
      return (
        ...
        rel.strength > 85 &&
        rel.treaties?.length >= 2
      );
    });

    if (allianceCandidates.length > 0 && relationships.filter((r: any) => r.relationship === 'alliance').length < 3) {
      ...
      opportunities.push({
        ...
        expectedBenefit: '+35 network power, mutual defense pact, shared intelligence, research cooperation',
        difficulty: 'hard',
        recommendedAction: `Propose formal alliance with ${target.targetCountry}`,
        targetCountry: target.targetCountry,
        estimatedTimeframe: '8-12 weeks',
        prerequisites: ['Friendly status', 'Strength 85+', '2+ existing treaties', 'Political approval'],
    ...
    if (weakRelationships.length > 2) {
      opportunities.push({
        ...
        expectedBenefit: 'Prevent relationship decay, unlock future cooperation, improve network stability',
        difficulty: 'medium',
