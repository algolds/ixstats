...
export function detectNumberFormat(value: number, context?: string): NumberFormat {
  ...
  if (
    contextLower.includes("percent") ||
    contextLower.includes("rate") ||
  ...
  if (
    contextLower.includes("population") ||
    contextLower.includes("people") ||
    contextLower.includes("citizens")
  ) {
    return "population";
  }
...
export function createNumberConfig(
  ...
  return {
    ...
    decimalPlaces:
      options.decimalPlaces ?? processed.decimalPlaces ?? baseConfig.decimalPlaces ?? 0,
    trend: options.trend,
