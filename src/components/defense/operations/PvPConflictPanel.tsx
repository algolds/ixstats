...
export function PvPConflictPanel({ countryId }: PvPConflictPanelProps) {
  ...
  const pendingForMe = conflicts?.filter(
  ...
  return (
    <div className="space-y-4">
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Automatically resolve a military engagement against an NPC nation.
                  Outcome is calculated based on relative military strength.
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-4">
                <div>
        ...
        <div className="space-y-2">
          ...
          {pendingForMe.map((c) => (
            <div key={c.id} className="rounded-lg border border-yellow-500/30 bg-yellow-500/5 p-3">
              <div className="flex items-center justify-between">
                <div>
                  <span className="font-medium text-sm">{c.initiator.name}</span>
                  <span className="text-muted-foreground text-xs ml-2">challenges you</span>
                  {c.reason && (
              ...
              return (
                <div key={c.id} className="rounded-lg border border-red-500/20 p-3 text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <span>vs <span className="font-medium">{opponent.name}</span></span>
                    <Badge variant="outline" className="text-[10px]">{c.type.toUpperCase()}</Badge>
      ...
      {resolvedConflicts.length > 0 && (
        <div className="space-y-2">
          <h4 className="text-xs font-semibold text-muted-foreground">Past Conflicts</h4>
              ...
              <div key={c.id} className="rounded-lg border p-3 text-sm opacity-70">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {draw ? (c.winner === "declined" ? "Declined" : "Draw") : won ? "Victory" : "Defeat"}
                    </Badge>
                  </div>
