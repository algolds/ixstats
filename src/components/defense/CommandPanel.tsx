...
const BUDGET_CATEGORIES = [
  { key: 'personnelCosts', label: 'Personnel', icon: Users, color: 'text-blue-600', defaultPercent: 40 },
  { key: 'operationsMaintenance', label: 'Operations & Maintenance', icon: Wrench, color: 'text-green-600', defaultPercent: 30 },
  { key: 'procurement', label: 'Procurement', icon: Package, color: 'text-purple-600', defaultPercent: 15 },
  { key: 'rdteCosts', label: 'Research & Development', icon: Microscope, color: 'text-orange-600', defaultPercent: 10 },
  { key: 'militaryConstruction', label: 'Military Construction', icon: Building, color: 'text-cyan-600', defaultPercent: 5 },
] as const;
...
export function CommandPanel({ countryId }: CommandPanelProps) {
  ...
  const handleTotalBudgetChange = (value: number) => {
    ...
    setBudgetData({
      ...
      gdpPercent: (country && country.calculatedStats.currentTotalGdp) ? (value / country.calculatedStats.currentTotalGdp) * 100 : budgetData.gdpPercent,
    });
  };
  ...
  const totalAllocated = budgetData.personnelCosts + budgetData.operationsMaintenance +
                        budgetData.procurement + budgetData.rdteCosts + budgetData.militaryConstruction;

  const allocationPercent = budgetData.totalBudget > 0 ? (totalAllocated / budgetData.totalBudget) * 100 : 0;

  // Calculate readiness metrics
  const averageReadiness = branches && branches.length > 0
    ? branches.reduce((sum, b) => sum + b.readinessLevel, 0) / branches.length
    : 0;

  const averageTechnology = branches && branches.length > 0
    ? branches.reduce((sum, b) => sum + b.technologyLevel, 0) / branches.length
    : 0;

  const averageMorale = branches && branches.length > 0
                    ...
                    <div className="space-y-4 text-sm">
                      <div>
                        ...
                        <p className="text-muted-foreground">
                          Your defense budget should total 100% allocated across all categories. The system will warn you if you're over or under budget.
                        </p>
                      </div>
                      <div>
                        <h4 className="font-semibold mb-2">Budget Categories Explained</h4>
                        <ul className="list-disc list-inside space-y-1 text-muted-foreground">
                          <li><strong>Personnel (typically 35-45%):</strong> Salaries, benefits, pensions for military and civilian staff</li>
                          <li><strong>Operations & Maintenance (25-35%):</strong> Day-to-day operations, training exercises, facility upkeep</li>
                          <li><strong>Procurement (10-20%):</strong> Purchase of new equipment, vehicles, ships, aircraft, and weapons</li>
                          <li><strong>R&D (5-15%):</strong> Research and development of next-generation military technology</li>
                          <li><strong>Military Construction (3-8%):</strong> Building and upgrading bases, installations, and infrastructure</li>
                        </ul>
                      </div>
                      <div>
                        <h4 className="font-semibold mb-2">GDP Percentage</h4>
                        <p className="text-muted-foreground">
                          Typical defense spending ranges from 1-4% of GDP. Higher percentages indicate a strong military focus, while lower percentages suggest prioritizing other sectors.
                        </p>
                      </div>
                      <div>
                        <h4 className="font-semibold mb-2">Tips</h4>
                        <ul className="list-disc list-inside space-y-1 text-muted-foreground">
                          <li>Balance current needs (personnel, operations) with future capabilities (procurement, R&D)</li>
                          <li>Changing total budget maintains proportional allocations automatically</li>
                          <li>Monitor branch budgets to ensure they sum to your total defense budget</li>
                        </ul>
                      </div>
                    </div>
                  </DialogContent>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="p-4 rounded-lg bg-muted/50 border">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label className="text-sm text-muted-foreground">Total Defense Budget</Label>
                {editingBudget ? (
                  <Input
                    type="number"
              ...
              <div>
                <Label className="text-sm text-muted-foreground">% of GDP</Label>
                <div className="text-2xl font-bold mt-1">
                  <NumberFlowDisplay value={budgetData.gdpPercent} format="percentage" decimalPlaces={2} />
                </div>
              </div>
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <span className={cn(
                'text-sm font-medium',
                allocationPercent > 100 ? 'text-red-600' : allocationPercent < 95 ? 'text-orange-600' : 'text-green-600'
              )}>
                <NumberFlowDisplay value={allocationPercent} format="percentage" decimalPlaces={1} /> Allocated
              </span>
            </div>
            <Progress value={Math.min(allocationPercent, 100)} className="h-2" />
            {allocationPercent > 100 && (
              <p className="text-xs text-red-600 mt-1">Over budget! Reduce allocations.</p>
            )}
          </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {editingBudget ? (
                        <Input
                          type="number"
                          value={value}
                          onChange={(e) => handleCategoryChange(category.key, parseFloat(e.target.value) || 0)}
      ...
      <Card className="glass-hierarchy-child">
              ...
              <DialogContent className="max-w-2xl">
                ...
                <div className="space-y-4 text-sm">
                  <div>
                    ...
                    <p className="text-muted-foreground">
                      Measures the ability of your forces to deploy and conduct operations immediately. Factors include equipment availability, personnel training, and supply stockpiles.
                    </p>
                  </div>
                  <div>
                    <h4 className="font-semibold mb-2">Technology Level</h4>
                    <p className="text-muted-foreground">
                      Reflects the sophistication of your military equipment and systems. Higher technology levels provide tactical advantages but require more maintenance and training.
                    </p>
                  </div>
                  <div>
                    <h4 className="font-semibold mb-2">Force Morale</h4>
                    <p className="text-muted-foreground">
                      Indicates the motivation and esprit de corps of your military personnel. High morale improves combat effectiveness and reduces desertion rates.
                    </p>
                  </div>
                  <div>
                    <h4 className="font-semibold mb-2">Improving Readiness</h4>
                    <ul className="list-disc list-inside space-y-1 text-muted-foreground">
        ...
        <CardContent>
          ...
          {branches && branches.length > 0 && (
            <div>
              <h4 className="font-semibold text-sm mb-3">Branch Status</h4>
              <div className="space-y-2">
                {branches.map((branch) => (
                  <div key={branch.id} className="flex items-center justify-between p-2 rounded-lg hover:bg-accent/50">
                    ...
                    <div className="flex items-center gap-4 text-xs">
                      ...
                      <span className="text-muted-foreground">
                        Budget: <span className="font-medium">${(branch.annualBudget / 1000000).toFixed(0)}M</span>
                      </span>
                    </div>
                  </div>
                ))}
