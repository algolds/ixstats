...
import type { SummaryMetric, EconomicChartDataPoint, EconomicHealthIndicator } from '~/lib/analytics-data-transformers';
...
export const OverviewSection = React.memo<OverviewSectionProps>(({
  ...
  const healthTableData = economicHealthIndicators.map((indicator, index) => ({
    ...
    status: indicator.value >= 80 ? 'Excellent' : indicator.value >= 60 ? 'Good' : indicator.value >= 40 ? 'Fair' : 'Poor'
  }));
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <EconomicOverviewChart
          ...
          onExportCSV={() => exportToCSV(economicChartData, 'gdp-per-capita-trend', {
            date: 'Date',
            gdp: 'Total GDP',
        ...
        <EconomicHealthRadar
          ...
          GlassTooltip={GlassTooltip}
          onExportCSV={() => exportToCSV(economicHealthIndicators, 'economic-health-indicators', {
            indicator: 'Indicator',
            value: 'Score'
          })}
