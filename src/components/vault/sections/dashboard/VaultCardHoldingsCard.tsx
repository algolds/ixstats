    ...
    >
      ...
      <div className="relative z-10 mb-4 flex items-center justify-between border-b border-white/10 pb-3">
        ...
        {featuredCards.length > 0 && (
          <button
            ...
            className="flex items-center gap-1 text-[11px] font-bold text-purple-400 transition-transform hover:underline active:scale-95"
          >
      ...
      {topCardsLoading ? (
        <div className="flex justify-center py-8">
          <Skeleton className="h-64 w-44 rounded-2xl bg-white/5" />
        </div>
      ) : featuredCards.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-8 text-center">
          <Layers className="mb-3 h-10 w-10 text-slate-500/40" />
          ...
          <Button
            ...
            className="h-8 rounded-full border-rose-500/30 bg-rose-500/10 text-xs font-bold text-rose-400 hover:bg-rose-500/20 active:scale-95"
          >
          ...
          {featuredCards.length > 1 && (
            <div className="space-y-2 border-t border-white/10 pt-3">
              {featuredCards.slice(1, 3).map((card) => (
                <div
                  key={card.id}
                  className="flex items-center justify-between rounded-xl border border-white/5 bg-white/5 px-3 py-2 text-xs backdrop-blur-md transition-all hover:bg-white/10 active:scale-[0.985] cursor-pointer"
                >
                  <span className="max-w-[130px] truncate font-semibold text-slate-200">
                    {card.title}
                  </span>
                  <span className="flex items-center gap-0.5 font-mono text-[11px] font-bold text-purple-400">
