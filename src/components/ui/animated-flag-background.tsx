...
export const AnimatedFlagBackground: React.FC<AnimatedFlagBackgroundProps> = ({
      ...
      const animate = () => {
        ...
        for (let x = 0; x < segments; x++) {
          for (let y = 0; y < segments; y++) {
            ...
            // Wave calculation
            const waveX = Math.sin((x / segments) * Math.PI * waveFrequency + time * waveSpeed) * waveAmplitude * rect.width;
            const waveY = Math.sin((y / segments) * Math.PI * waveFrequency * 0.5 + time * waveSpeed * 0.7) * waveAmplitude * rect.height * 0.3;
  ...
  const renderStaticFallback = () => (
    <motion.div
      ...
      style={{
        backgroundImage: flagUrl && flagLoaded 
          ? `url(${flagUrl})` 
  ...
  return (
    <div className={cn("absolute inset-0 overflow-hidden", className)}>
      ...
      {countryName && (
        ...
        >
          <div className="text-white text-xs font-medium px-2 py-1 bg-black/30 rounded backdrop-blur-sm">
            {countryName}
          </div>
        </motion.div>
      )}
