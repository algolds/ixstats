...
export const LampContainer = ({
  ...
  if (className?.includes("sdi-lamp-header")) {
    return (
      <div
        className={cn(
          "relative z-0 flex h-20 w-full items-end justify-start overflow-visible",
          className
        )}
      >
        ...
        <motion.div
          ...
          style={{
            backgroundImage: `radial-gradient(ellipse at 40% 60%, rgba(34,211,238,0.25) 0%, rgba(59,130,246,0.12) 60%, transparent 100%)`,
          }}
          className="pointer-events-none absolute top-0 left-0 h-16 w-64 blur-2xl"
        />
        <div className="relative z-10 px-2 pt-2">{children}</div>
      </div>
    );
  }
  // Default (full-page) lamp effect
  return (
    <div
      className={cn(
