...
export function ColorPicker({
  ...
  useEffect(() => {
    ...
    if (normalized !== lastEmittedRef.current) {
      syncFromExternal(color)
    }
  }, [color])

  const syncFromExternal = (newColor: string) => {
    ...
    setHsl([h, s, l])
    lastEmittedRef.current = normalizedColor
  }
  ...
  const handleColorChange = (newColor: string) => {
    ...
    setHsl([h, s, l])
    const emitted = `hsl(${h.toFixed(1)}, ${s.toFixed(1)}%, ${l.toFixed(1)}%)`
