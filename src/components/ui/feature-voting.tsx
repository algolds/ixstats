...
const FeatureVotingContext = createContext<FeatureVotingContextValue | null>(null);

function useFeatureVotingContext() {
  const context = useContext(FeatureVotingContext);
  if (!context) {
    throw new Error("FeatureVoting components must be used within FeatureVoting.Root");
  }
  return context;
}
...
const FeatureVotingItemContext = createContext<FeatureVotingItemContextValue | null>(null);

function useFeatureVotingItemContext() {
  const context = useContext(FeatureVotingItemContext);
  if (!context) {
    throw new Error("FeatureVoting.Item sub-components must be used within FeatureVoting.Item");
  }
  return context;
}
...
function FeatureVotingRoot({
  ...
  const [votedFeaturesArray, setVotedFeaturesArray] = useControllableState({
    prop: controlledVotedFeatures ? Array.from(controlledVotedFeatures) : undefined,
    defaultProp: defaultVotedFeatures ? Array.from(defaultVotedFeatures) : [],
    onChange: (arr) => onVotedFeaturesChange?.(new Set(arr)),
  });

  const votedFeatures = useMemo(() => new Set(votedFeaturesArray), [votedFeaturesArray]);
  ...
  const unvote = useCallback(
    (featureId: string) => {
      ...
      setVotedFeaturesArray((prev) => (prev ?? []).filter((id) => id !== featureId));
    },
    [disabled, votedFeatures, setVotes, setVotedFeaturesArray]
  );
  ...
  const getVoteCount = useCallback((featureId: string) => votes?.[featureId] ?? 0, [votes]);
  ...
  const contextValue = useMemo(
    ...
    [votes, votedFeatures, disabled, vote, unvote, toggleVote, getVoteCount, hasVoted]
  );

  return (
    <FeatureVotingContext.Provider value={contextValue}>
      <ul aria-label="Feature voting list" data-disabled={disabled ? true : undefined} {...props}>
        {children}
      </ul>
...
function FeatureVotingGroup({ sortBy = "none", children, ...props }: FeatureVotingGroupProps) {
...
function FeatureVotingItem({
  ...
  const { disabled: rootDisabled, hasVoted, getVoteCount } = useFeatureVotingContext();
  const disabled = rootDisabled || itemDisabled;
  const voted = hasVoted(value);
  const voteCount = getVoteCount(value);

  const itemContextValue = useMemo(() => ({ featureId: value, disabled }), [value, disabled]);
...
function FeatureVotingTrigger({ children, onClick, ...props }: FeatureVotingTriggerProps) {
  const { toggleVote, hasVoted, disabled: rootDisabled } = useFeatureVotingContext();
...
function FeatureVotingDescription({ children, ...props }: FeatureVotingDescriptionProps) {
  return (
