...
interface NumberFlowDisplayProps {
  ...
  format?: 'default' | 'currency' | 'population' | 'percentage' | 'financial' | 'compact' | 'decimal';
  duration?: number;
...
export function NumberFlowDisplay({ 
  ...
  const autoDecimalPlaces = decimalPlaces ?? (
    format === 'currency' ? 2 :
    format === 'financial' ? 2 :
    format === 'decimal' ? 2 :
    format === 'percentage' ? 1 :
    format === 'population' || format === 'compact' ? 1 :
    0
  );
  ...
  return (
    <span className={cn(
      ...
      {(format === 'currency' && (safeValue < 1e6)) || format === 'population' || format === 'compact' || format === 'percentage' ? (
        ...
        <>
          ...
          <NumberFlow 
            value={displayValue}
            format={{
              minimumFractionDigits: format === 'currency' && safeValue >= 1e6 ? Math.max(autoDecimalPlaces, 1) : autoDecimalPlaces,
              maximumFractionDigits: format === 'currency' && safeValue >= 1e6 ? Math.max(autoDecimalPlaces, 1) : autoDecimalPlaces,
