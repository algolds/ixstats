...
import { useMap, MapMarker, MarkerContent, MapPopup, MarkerLabel } from "~/components/ui/map";
...
function useFlightMapTheme(): FlightMapTheme {
  const [theme, setTheme] = useState<FlightMapTheme>(() => getDocumentTheme() ?? getSystemTheme());
...
>();
const airportMarkerDedupListeners = new WeakMap<MapLibreGL.Map, Set<() => void>>();
...
function subscribeAirportMarkerDedup(map: MapLibreGL.Map | null, listener: () => void) {
  if (!map) return () => {};
...
function getAirportMarkerOwnerId(map: MapLibreGL.Map | null, dedupeKey: string) {
  if (!map) return null;
  return getAirportMarkerDedupStore(map).get(dedupeKey)?.ownerId ?? null;
}

function claimAirportMarker(map: MapLibreGL.Map | null, dedupeKey: string, markerId: string) {
  if (!map) return () => {};
...
function useAirportMarkerVisibility(map: MapLibreGL.Map | null, dedupeKey?: string): boolean {
...
function getProjectionType(map: MapLibreGL.Map | null | undefined): string | null {
...
type FlightAirportProps = {
  ...
  onClick?: (airport: AirportInfo | { longitude: number; latitude: number }) => void;
      ...
      <MarkerContent className={className}>
        ...
        {showLabel && displayName && (
          <MarkerLabel
            ...
            className={cn(
              flightMapTheme === "dark" ? "text-neutral-100" : "text-neutral-950",
              labelClassName
            )}
          >
            {displayName}
...
function resolveDashArray(lineStyle?: "solid" | "dash" | "dot"): [number, number] | undefined {
  switch (lineStyle) {
    case "dash":
      return [4, 3];
    case "dot":
...
function FlightRoute({
  ...
  const resolvedRouteColor =
    color ?? (flightMapTheme === "dark" ? FLIGHT_ROUTE_COLOR_DARK : FLIGHT_ROUTE_COLOR_LIGHT);
  const autoId = useId();
  ...
  const animateConfig = useMemo<FlightRouteAnimateConfig | null>(() => {
    ...
    const base: FlightRouteAnimateConfig = animate === true ? {} : { ...animate };
  ...
  }, [isLoaded, map, layerId, resolvedRouteColor, width, opacity, resolvedDash]);
  ...
  useEffect(() => {
    ...
    const handleMouseEnter = (e: MapLibreGL.MapLayerMouseEvent & object) => {
      ...
      if (hoverEffectRef.current && info) {
        tooltip.setLngLat(e.lngLat).setHTML(buildRouteTooltipHTML(info)).addTo(map);
      }
...
function FlightRoutes({
  ...
  const uniqueAirports = useMemo(() => {
    ...
    for (const route of routes) {
      const fromKey = typeof route.from === "string" ? route.from : route.from.join(",");
      const toKey = typeof route.to === "string" ? route.to : route.to.join(",");
    ...
    <>
      {routes.map((route, index) => (
        <FlightRoute
          ...
          onClick={route.onClick ?? (onClick ? () => onClick(index, route) : undefined)}
          onMouseEnter={
            route.onMouseEnter ?? (onMouseEnter ? () => onMouseEnter(index, route) : undefined)
          }
          onMouseLeave={
            route.onMouseLeave ?? (onMouseLeave ? () => onMouseLeave(index, route) : undefined)
          }
          showAirports={false}
        />
      ))}
...
function interpolatePosition(coords: [number, number][], t: number): [number, number] {
