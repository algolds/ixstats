...
function generateFlagSets(countries: Array<{ id: string; name: string }>): FlagSet[] {
  if (countries.length < TOTAL_FLAGS) {
    ...
    while (repeatedCountries.length < TOTAL_FLAGS) {
      repeatedCountries.push(
        ...countries.slice(0, Math.min(countries.length, TOTAL_FLAGS - repeatedCountries.length))
      );
    }
...
function getSessionFlagData(countries: Array<{ id: string; name: string }>): SessionFlagData {
  ...
  try {
    ...
    if (stored) {
      ...
      if (
        data.allSets &&
        data.allSets.length === SETS_COUNT &&
...
async function preloadFlagSet(countries: Array<{ id: string; name: string }>): Promise<void> {
  ...
  try {
    ...
    await Promise.allSettled(countryNames.map((name) => unifiedFlagService.getFlagUrl(name)));
  } catch (err) {
...
export function AnimatedFlagsBackground({
  ...
  const FlagComponent = useCallback(
    ({ flag, index, setKey }: { flag: FlagItem; index: number; setKey: string }) => {
      ...
      return (
        ...
        >
          <motion.div
            className="h-full w-full overflow-hidden rounded-sm border border-white/10 bg-white/5 backdrop-blur-[1px] transition-all duration-1000 hover:bg-white/10"
  ...
  return (
    <div className={`pointer-events-none absolute inset-0 overflow-hidden ${className}`}>
      {flagItems.map((flag, index) => (
        <FlagComponent
          key={`flag-${flag.id}-${sessionData.currentSetIndex}`}
          flag={flag}
          index={index}
          setKey={setKey}
        />
