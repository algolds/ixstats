...
export const RandomFlagGrid: React.FC<RandomFlagGridProps> = ({
  ...
  return (
    <div 
      className={cn("w-full h-full relative overflow-hidden cursor-pointer", className)}
    ...
    >
        ...
        <AnimatePresence mode="wait">
          {flagGrid.map((item) => {
            ...
            return (
              ...
              <div className="absolute inset-0 bg-gradient-to-br from-background/5 via-transparent to-background/10 pointer-events-none" />
              
              {/* Hover overlay with country info */}
              <motion.div
                className="absolute inset-0 bg-black/70 flex flex-col justify-center items-center opacity-0 hover:opacity-100 transition-opacity duration-300 pointer-events-none"
                initial={{ opacity: 0 }}
        ...
        >
          ...
          <div className="flex flex-col">
            <span className="text-white/80">
              {currentlyChanging !== null ? 'Changing flag...' : `${hoverDuration}s - Next in ${1 - (hoverDuration % 1)}s`}
            </span>
            {hoverDuration > 0 && (
      ...
      {!isHovered && (
        <motion.div
          className="absolute top-2 right-2 text-white/50 text-xs backdrop-blur-sm bg-black/20 px-2 py-1 rounded"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          style={{
