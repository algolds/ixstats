...
function ToastContainer() {
  ...
  return (
    <div className="pointer-events-none fixed top-4 right-4 z-50 w-full max-w-sm space-y-2">
      <AnimatePresence>
...
function ToastItem({ toast, onRemove }: { toast: Toast; onRemove: () => void }) {
  ...
  return (
    <motion.div
      ...
      className={`relative flex w-full items-center space-x-4 rounded-md border ${getBorderColor()} pointer-events-auto bg-white p-4 shadow-lg dark:bg-gray-800`}
    >
      ...
      <div className="flex-1 space-y-1">
        ...
        {toast.description && (
          <div className="text-sm text-gray-600 dark:text-gray-400">{toast.description}</div>
        )}
      </div>
