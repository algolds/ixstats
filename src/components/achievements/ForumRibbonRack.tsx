...
export function ForumRibbonRack({ unlockedCount, username }: ForumRibbonRackProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-3xl border border-amber-500/25 bg-gradient-to-b from-slate-950/90 via-black/80 to-slate-950/90 p-5 shadow-2xl backdrop-blur-2xl transition-all border-t-amber-400/35">
                  ...
                  {ribbon ? (
                    ...
                    >
                      ...
                      >
                        ...
                        <span className="relative z-10 font-mono text-[9px] font-black tracking-widest text-slate-950 uppercase drop-shadow">
                          {ribbon.badgeLabel}
                        </span>
                        ...
                        <div className="relative z-10 flex h-8 w-8 items-center justify-center rounded-full border border-amber-300/40 bg-amber-400/20 shadow-md backdrop-blur-sm">
                          <Star className="h-4.5 w-4.5 text-amber-300 fill-amber-300 drop-shadow" />
                        </div>

                        {/* Pin Toggle Button */}
                        <button
                          onClick={(e) => togglePin(ribbon.id, e)}
                          className="relative z-10 flex h-5 w-5 items-center justify-center rounded-full border border-slate-950/40 bg-slate-950/30 text-slate-950 hover:bg-slate-950/60 transition-all active:scale-95"
                          title="Unpin Ribbon"
                        >
                      ...
                      {/* Title Below */}
                      <span className="mt-2 max-w-[100px] text-center text-[10px] font-bold text-amber-200 line-clamp-1">
                        {ribbon.title}
                      </span>
                    </motion.div>
                  ) : (
                    <div className="flex flex-col items-center">
                      ...
                      <div className="flex h-28 w-16 items-center justify-center rounded-sm border border-dashed border-white/15 bg-white/5 text-slate-500">
                        <span className="text-[9px] font-extrabold text-slate-500 uppercase rotate-90">
                          EMPTY
                        </span>
                      </div>
                      <div className="h-2.5 w-16 rounded-b-sm bg-slate-800" />
                      <span className="mt-2 text-[10px] text-slate-500 italic">Slot {slotIdx + 1}</span>
                    </div>
                  )}
                </div>
              );
      ...
      <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-slate-950/70 p-5 shadow-2xl backdrop-blur-2xl transition-all border-t-white/20 dark:bg-black/60 dark:border-white/12">
        <TextureOverlay texture="dots" opacity={0.03} />
        ...
        <div className="relative z-10 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1.5 rounded-full border border-amber-500/30 bg-amber-500/15 px-3 py-1 font-mono text-xs font-bold text-amber-400 backdrop-blur-md">
              ...
              <span>{Math.min(unlockedCount, FORUM_RIBBONS.length)} / {FORUM_RIBBONS.length} Honors</span>
            </div>
          </div>

          {/* Vertical Military Ribbon Rack Grid */}
          <TooltipProvider>
            <div className="flex flex-wrap items-center justify-center gap-6 sm:justify-start pt-2">
                ...
                return (
                  <Tooltip key={ribbon.id}>
                    <TooltipTrigger asChild>
                      ...
                      >
                        ...
                        >
                          ...
                          {/* Top Ribbon Badge Label */}
                          <span className="relative z-10 font-mono text-[8px] font-black tracking-widest text-slate-950 uppercase drop-shadow">
                            {ribbon.badgeLabel}
                          </span>
                        ...
                        {/* Title Below */}
                        <span className="mt-1.5 max-w-[85px] text-center text-[9px] font-bold text-slate-300 line-clamp-1">
                          {ribbon.title}
                        </span>
                      </motion.div>
                    </TooltipTrigger>
                    <TooltipContent className="border-white/10 bg-slate-950/90 text-xs text-slate-200 backdrop-blur-xl">
                      ...
                      <p className="text-[10px] text-slate-400">{ribbon.category} Out-of-Character Service Honor</p>
