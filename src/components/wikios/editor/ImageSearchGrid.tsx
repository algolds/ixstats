...
import {
  Search, Loader2, X, ExternalLink, Copy, Check,
  Image as ImageIcon, Globe, Database, ZoomIn,
} from "lucide-react";
...
export function ImageSearchGrid({ onSelect, selectedImage, compact }: ImageSearchGridProps) {
  ...
  const results: ImageResult[] = tab === "ixwiki"
    ? (ixwikiQuery.data ?? []).map((f: any) => ({
        title: f.title ?? f.name ?? "",
        url: f.url ?? `https://ixwiki.com/wiki/Special:FilePath/${encodeURIComponent(f.title ?? f.name ?? "")}`,
        thumbUrl: f.thumbUrl ?? f.url, width: f.width, height: f.height, mime: f.mime,
      }))
    : (commonsQuery.data ?? []).map((f: any) => ({
        title: f.title ?? "", url: f.url ?? "", thumbUrl: f.thumbUrl ?? f.url,
        width: f.width, height: f.height, mime: f.mime,
      }));
    ...
    <div className={cn("wikios-imgs", compact && "wikios-imgs-compact")}>
      ...
      <div className="wikios-imgs-header">
        <div className="wikios-imgs-tabs">
          <button onClick={() => { setTab("ixwiki"); setExpandedImage(null); }} className={cn("wikios-imgs-tab", tab === "ixwiki" && "wikios-imgs-tab-active")}>
            <Database size={13} /> IxWiki
          </button>
          <button onClick={() => { setTab("commons"); setExpandedImage(null); }} className={cn("wikios-imgs-tab", tab === "commons" && "wikios-imgs-tab-active")}>
        ...
        <div className="wikios-imgs-search">
          ...
          />
          {query && (
            <button onClick={() => { setQuery(""); setDebouncedQuery(""); }} className="wikios-imgs-search-clear">
              <X size={12} />
            </button>
          )}
        </div>
      ...
      {debouncedQuery.length < 2 && (
        <div className="wikios-imgs-state wikios-imgs-empty-state">
          <div className="wikios-imgs-empty-icon"><ImageIcon size={36} /></div>
          <h3>Search for images</h3>
          <p>Find images from {tab === "ixwiki" ? "IxWiki's local uploads" : "Wikimedia Commons' free media library"}</p>
        </div>
      )}
      ...
      <div className="wikios-imgs-content">
        <div className={cn("wikios-imgs-grid", compact ? "wikios-imgs-grid-sm" : "wikios-imgs-grid-lg")}>
          {results.map((img) => {
            ...
            return (
              <div
                ...
                className={cn("wikios-imgs-card", isSelected && "wikios-imgs-card-selected", isExpanded && "wikios-imgs-card-expanded")}
                onClick={() => {
                  onSelect?.(img);
              ...
              >
                <div className="wikios-imgs-card-img">
                  <img src={img.thumbUrl ?? img.url} alt={img.title} loading="lazy" referrerPolicy="no-referrer" />
                  <div className="wikios-imgs-card-overlay">
                    <ZoomIn size={16} />
                  </div>
                </div>
                <div className="wikios-imgs-card-footer">
                  <span className="wikios-imgs-card-name">{img.title.replace(/^File:/, "").replace(/_/g, " ")}</span>
                  <div className="wikios-imgs-card-actions">
                    <button
                      ...
                      title="Copy wikitext"
                    >
                      {copiedId === img.url ? <Check size={11} className="text-green-400" /> : <Copy size={11} />}
                    </button>
                    <a
        ...
        {!compact && expandedImage && (
          <div className="wikios-imgs-detail">
            <button onClick={() => setExpandedImage(null)} className="wikios-imgs-detail-close"><X size={14} /></button>
            ...
            <div className="wikios-imgs-detail-info">
              ...
              <div className="wikios-imgs-detail-meta">
                {expandedImage.width && expandedImage.height && (
                  <span>{expandedImage.width} × {expandedImage.height} px</span>
                )}
                {expandedImage.mime && <span>{expandedImage.mime}</span>}
              </div>
              <div className="wikios-imgs-detail-actions">
                <button onClick={() => handleCopy(expandedImage)} className="wikios-imgs-detail-btn">
                  {copiedId === expandedImage.url ? <><Check size={12} /> Copied!</> : <><Copy size={12} /> Copy wikitext</>}
                </button>
                <a href={expandedImage.url} target="_blank" rel="noopener noreferrer" className="wikios-imgs-detail-btn wikios-imgs-detail-btn-secondary">
