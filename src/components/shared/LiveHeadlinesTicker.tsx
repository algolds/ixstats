 ...
 */
export function LiveHeadlinesTicker({
  limit = 30,
  className,
}: {
  limit?: number;
...
const TickerBar = React.memo(function TickerBar({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "glass-hierarchy-child relative overflow-hidden rounded-xl border border-white/10",
        className
      )}
          ...
          >
            {headlines.map((headline) => {
              ...
              const inner = (
                <div
                  className={cn(
                    "flex shrink-0 items-center gap-2 px-1 whitespace-nowrap",
                    isCritical && "font-semibold",
                    headline.url ? "cursor-pointer hover:opacity-80" : "cursor-default"
                  )}
                >
                  {isCritical && (
                    <span className="rounded bg-red-500/20 px-1 py-0.5 text-[0.6rem] font-bold tracking-wider text-red-400 uppercase">
                      Breaking
                    </span>
                  )}
                  {isHigh && !isCritical && (
                    <span className="rounded bg-amber-500/15 px-1 py-0.5 text-[0.6rem] font-bold tracking-wider text-amber-400 uppercase">
                      Alert
                    </span>
                  )}
