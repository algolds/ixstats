        ...
        <AnimatePresence>
          {Object.entries(groupedMessages).map(([severity, msgs]) => {
            ...
            return (
              ...
              >
                <Alert className={cn('py-3 px-4', config.bg)}>
        ...
        {messages.map((msg, index) => {
          ...
          return (
            <motion.div
              ...
              transition={{ duration: 0.2 }}
            >
              <Alert className={cn('py-2 px-3', config.bg)}>
                <div className="flex items-start gap-2">
                  <Icon className={cn('h-4 w-4 mt-0.5 flex-shrink-0', config.color)} />
...
export function ValidationSummary({
  ...
  return (
    <div className={cn('flex items-center gap-2 flex-wrap', className)}>
      ...
      {counts.warning > 0 && (
        <Badge className="text-xs bg-yellow-500/10 text-yellow-700 dark:text-yellow-400 border-yellow-500/20">
