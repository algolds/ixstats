...
function InterfaceSwitcherContent({
  currentInterface,
  countryId,
}: {
  currentInterface: "sdi" | "eci";
  countryId?: string;
}) {
  const router = useRouter();
  const { user } = useUser();
  const { data: profile } = api.users.getProfile.useQuery(undefined, { enabled: !!user?.id });
  ...
  return (
    <div className="flex gap-2">
      ...
      {currentInterface === "sdi" && permissions.canAccessECI && (
        <Button
          onClick={switchToECI}
          className="border-orange-500/30 bg-orange-600/20 text-orange-300 hover:bg-orange-600/30"
        >
