...
export function ValidatedInput({
  ...
  const [validationMessages, setValidationMessages] = useState<Array<{ message: string; severity: 'error' | 'warning' | 'info' }>>([]);
  ...
  const showSuccess = touched && isValid && validationMessages.length === 0 && value.trim() && successMessage;
        ...
        <>
          {validationMessages.map((msg, idx) => (
            <Alert
              ...
              className={cn(
                'py-2 px-3',
                msg.severity === 'warning' && 'border-yellow-500/20 bg-yellow-500/5 text-yellow-700 dark:text-yellow-400',
                msg.severity === 'info' && 'border-blue-500/20 bg-blue-500/5 text-blue-700 dark:text-blue-400'
              )}
            >
...
export const ValidationRules = {
  ...
  custom: (validator: (value: string) => boolean, message: string, severity: 'error' | 'warning' | 'info' = 'error'): ValidationRule => ({
    validate: validator,
    message,
    severity
  })
