...
import {
  Zap, TrendingUp, AlertTriangle, Handshake, Shield, Rss,
  Landmark, Trophy, BookOpen, MessageCircle, ExternalLink, Clock,
} from "lucide-react";
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((item) => {
              ...
              return (
                ...
                >
                  <div className={cn("mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full", config.bg, config.text)}>
                    <Icon className="h-3 w-3" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5">
                      {isCritical && (
                        <Badge variant="destructive" className="text-[8px] px-1 py-0 shrink-0">BREAKING</Badge>
                      )}
                      {isHigh && !isCritical && (
                        <Badge variant="outline" className="text-[8px] px-1 py-0 shrink-0 text-amber-600 border-amber-500/30">ALERT</Badge>
                      )}
                      <span className={cn("text-xs font-medium leading-snug", isCritical && "text-red-400")}>
                        {item.text}
                      </span>
