...
export default function PreviewPanel() {
  ...
  const handleChargeSvgLoaded = (id: string, svg: string) => {
    setCustomChargeSvgs((prev) => {
      if (prev[id] === svg) return prev;
      return { ...prev, [id]: svg };
    });
  };

  // Find unique custom charges
  const customChargeIds = Array.from(
    new Set((composition.shield.charges ?? []).map((c) => c.chargeId))
  ).filter((id) => !["star", "cross", "fleur-de-lis", "lion", "eagle"].includes(id));

  return (
    <div className="flex h-[450px] flex-col rounded-xl border border-white/5 bg-zinc-900/60 p-4 backdrop-blur-md">
