      ...
      <div className="relative flex h-full w-full items-center justify-center overflow-hidden">
        ...
        >
            ...
            {particles.map((particle) => (
              <motion.div
                ...
                animate={{
                  ...
                  rotate: [
                    0,
                    180 * Math.sign(particle.velocityX),
                    360 * Math.sign(particle.velocityX),
                  ],
                }}
                transition={{
                  duration: 0.9,
          ...
          {Array.from({ length: isMobile ? 12 : 18 }).map((_, i) => {
            ...
            const burstColor =
              rarityParticleConfig.colors[i % rarityParticleConfig.colors.length] || "#ffffff";
          ...
          <motion.div
            className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-white/80"
            style={{
              width: "60px",
              height: "60px",
          ...
          {["EPIC", "LEGENDARY", "MYTHIC"].includes(highestRarity) &&
            Array.from({ length: isMobile ? 8 : 16 }).map((_, i) => {
              ...
              const fragmentColor =
                rarityParticleConfig.colors[i % rarityParticleConfig.colors.length];

              return (
                <motion.div
                  key={`fragment-${i}`}
                  className="absolute top-1/2 left-1/2 h-4 w-1 rounded-full"
                  style={{
