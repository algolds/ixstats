...
import type { ExchangeType, ExchangeStatus, ExchangeTypeConfig, StatusConfig } from "./cultural-exchange-types";
      ...
      <div className="flex items-center gap-2">
        ...
        >
          ...
          {Object.entries(exchangeTypes).map(([type, config]) => (
            <option key={type} value={type} className="bg-background text-foreground">{config.label}</option>
          ))}
        </select>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        >
          ...
          {Object.entries(statusStyles).map(([status, config]) => (
            <option key={status} value={status} className="bg-background text-foreground">{config.label}</option>
          ))}
        </select>
      </div>
    </div>
