      ...
      (emoji: string | { url: string; name: string }) => {
        if (editorRef.current) {
          if (typeof emoji === "string") {
            ...
            execCommand(
              "insertHTML",
              `<img src="${emoji.url}" alt=":${emoji.name}:" class="inline-block h-5 w-5" title=":${emoji.name}:" />`
            );
          }
      ...
      <div className={`bg-background rounded-lg border ${className}`}>
          ...
          <div className="flex items-center gap-1 border-b p-2">
            ...
            <Popover open={isLinkPopoverOpen} onOpenChange={setIsLinkPopoverOpen}>
              <PopoverTrigger
                disabled={disabled}
                title="Insert Link"
                className="ring-offset-background focus-visible:ring-ring hover:bg-accent hover:text-accent-foreground inline-flex h-8 w-8 items-center justify-center rounded-md p-0 text-sm font-medium transition-colors focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50"
              >
                    ...
                    <div className="grid max-h-64 grid-cols-8 gap-1 overflow-y-auto">
                      {commonEmojis.map((emoji, index) => (
                        <button
                          ...
                          className="hover:bg-accent rounded p-2 text-lg transition-colors"
                          title={emoji}
                        >
                          {emoji}
                        </button>
        ...
        <div className="relative">
          <div
            ...
            className={`resize-none overflow-y-auto p-3 outline-none ${disabled ? "cursor-not-allowed opacity-50" : ""} `}
            style={{
