...
export function GdpDetailsModal({ isOpen, onClose, countryId, countryName }: GdpDetailsModalProps) {
  ...
  // Get historical data
  const { data: historicalData, isLoading: historicalLoading, refetch } = 
  ...
  const gdpStats = React.useMemo(() => {
    ...
    const growth = previous && current?.totalGdp && previous.totalGdp ? ((current.totalGdp - previous.totalGdp) / previous.totalGdp) * 100 : 0;
    const totalGrowth = current?.totalGdp && firstPoint?.totalGdp ? ((current.totalGdp - firstPoint.totalGdp) / firstPoint.totalGdp) * 100 : 0;
    ...
    const volatility = values.length > 1 ? 
      Math.sqrt(values.reduce((acc, val, i, arr) => {
        if (i === 0) return 0;
  ...
  const getTrendIcon = (value: number) => {
    return value > 0 ? <TrendingUp className="h-4 w-4 text-green-500" /> : 
           value < 0 ? <TrendingDown className="h-4 w-4 text-red-500" /> :
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <Card>
                      <div className="p-6">
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="text-sm font-medium text-muted-foreground">Economic Tier</p>
                  ...
                  <Card>
                    <div className="p-6 pb-4">
                      ...
                      <p className="text-sm text-muted-foreground mt-1">Key performance indicators and trends</p>
                    </div>
                    <div className="p-6">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        ...
                        <div className="text-center">
                          ...
                          <div className="text-sm text-muted-foreground">GDP Volatility</div>
                        </div>
                        <div className="text-center">
                  ...
                  {globalStats && (
                    <Card>
                      <div className="p-6 pb-4">
                        ...
                        <p className="text-sm text-muted-foreground mt-1">How this country compares to global averages</p>
                      </div>
                      <div className="p-6">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                          <div className="text-center p-4 bg-blue-50 dark:bg-blue-950/20 rounded-lg">
                            ...
                            <div className="text-2xl font-bold">
                              {countryData?.currentGdpPerCapita && globalStats && typeof globalStats === 'object' && globalStats !== null && 'averageGdpPerCapita' in globalStats && typeof (globalStats as any).averageGdpPerCapita === 'number' ? 
                                `${((countryData.currentGdpPerCapita / (globalStats as any).averageGdpPerCapita - 1) * 100).toFixed(1)}%` : 
                                'N/A'
                              }
                            </div>
                            <div className="text-sm text-muted-foreground">
                              Global Avg: {globalStats && typeof globalStats === 'object' && globalStats !== null && 'averageGdpPerCapita' in globalStats ? formatCurrency((globalStats as any).averageGdpPerCapita) : 'N/A'}
                            </div>
                          </div>
                          <div className="text-center p-4 bg-green-50 dark:bg-green-950/20 rounded-lg">
                            ...
                            <div className="text-2xl font-bold">
                              {tierInfo?.allTiers ? 
                                (tierInfo.allTiers.findIndex(t => t.name === countryData?.economicTier) + 1) || 0 
                                : 0}/7
                            </div>
                            <div className="text-sm text-muted-foreground">
                              Among global tiers
                          ...
                          <div className="text-center p-4 bg-purple-50 dark:bg-purple-950/20 rounded-lg">
                            ...
                            <div className="text-2xl font-bold">
                              {countryData?.currentTotalGdp && globalStats && typeof globalStats === 'object' && globalStats !== null && 'totalGdp' in globalStats && typeof (globalStats as any).totalGdp === 'number' ? 
                                `${((countryData.currentTotalGdp / (globalStats as any).totalGdp) * 100).toFixed(3)}%` : 
                                'N/A'
                              }
                            </div>
                            <div className="text-sm text-muted-foreground">
                              Global GDP: {globalStats && typeof globalStats === 'object' && globalStats !== null && 'totalGdp' in globalStats ? formatCurrency((globalStats as any).totalGdp / 1e12) : 'N/A'}T
                            </div>
                          </div>
                        </div>
                      </div>
                ...
                <CardContent>
                  {processedData.length > 0 ? (
                    ...
                    <div className="text-center py-8 text-muted-foreground">
                      ...
                      <p className="text-sm">Data points will appear as the economic system generates history</p>
                    </div>
                  )}
                </CardContent>
              </Card>
            ...
            <div className="space-y-6">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Understanding your economic classification and growth potential</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        <h4 className="font-semibold mb-3">Current Economic Tier</h4>
                        <div className="p-4 border rounded-lg">
                          ...
                          <p className="text-sm text-muted-foreground">
                            Range: {tierInfo?.currentTier ? 
                              `${formatCurrency(tierInfo.currentTier.min)} - ${
                                tierInfo.currentTier.max === Infinity ? '∞' : formatCurrency(tierInfo.currentTier.max)
                              }` : 'N/A'
                            }
                          </p>
                        </div>
                      ...
                      <div>
                        <h4 className="font-semibold mb-3">Next Tier Target</h4>
                        <div className="p-4 border rounded-lg">
                          {tierInfo?.currentTier && tierInfo.allTiers ? (() => {
                            const currentIndex = tierInfo.allTiers.findIndex(t => t.name === tierInfo.currentTier?.name);
                            ...
                            if (nextTier) {
                              ...
                              return (
                                <div className="text-center text-green-600">
                                  ...
                                  <p className="text-sm">Your economy has reached the highest classification</p>
                                </div>
                              );
                            }
                          })() : 'N/A'}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <div className="text-center p-4 border rounded-lg">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {gdpStats.volatility < 5 ? 'Very Stable' :
                           gdpStats.volatility < 10 ? 'Stable' :
                           gdpStats.volatility < 20 ? 'Moderate' : 'High Volatility'}
                        </div>
                      </div>
                      <div className="text-center p-4 border rounded-lg">
                        ...
                        <div className="text-sm font-medium mb-1">Total Growth</div>
                        <div className="text-xs text-muted-foreground">
                      ...
                      <div className="text-center p-4 border rounded-lg">
                        ...
                        <div className="text-sm font-medium mb-1">Peak-to-Trough</div>
                        <div className="text-xs text-muted-foreground">
                          Maximum variation
                        </div>
                      </div>
              ...
              <Alert>
                ...
                <AlertDescription>
                  Economic tier classifications are based on GDP per capita and determine growth rate caps in the IxStats system. 
                  Higher tiers indicate more mature economies with typically lower but more stable growth rates.
                </AlertDescription>
              </Alert>
            </div>
          </TabsContent>
            ...
            <Card>
              ...
              <CardContent>
                <Alert>
                  ...
                  <AlertDescription>
                    Advanced GDP projection models are available through the Predictive Models feature in the premium analytics suite. 
                    These include multi-scenario forecasting with confidence intervals based on historical patterns and economic indicators.
                  </AlertDescription>
                </Alert>
              </CardContent>
            </Card>
