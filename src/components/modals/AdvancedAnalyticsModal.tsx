...
export function AdvancedAnalyticsModal({
  ...
  const insights = useMemo(() => {
    ...
    if (
      volatilityMetrics?.population?.level === "high" ||
      volatilityMetrics?.population?.level === "extreme"
    ) {
      insightsList.push({
        type: "warning",
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-h-[90vh] max-w-7xl overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Comprehensive data analysis, volatility metrics, trend detection, and correlation
            studies
          </DialogDescription>
        </DialogHeader>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
              <GlassCard>
                <div className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-muted-foreground text-sm">GDP Volatility</p>
                      <p className="text-2xl font-bold">
                        {volatilityMetrics
                          ? `${(volatilityMetrics.gdp.value * 100).toFixed(1)}%`
                          : "N/A"}
                      </p>
                    </div>
                    <div
                      className={`rounded-full p-2 ${
                        volatilityMetrics?.gdp.level === "stable"
                          ? "bg-green-500/20"
                          : volatilityMetrics?.gdp.level === "moderate"
                            ? "bg-yellow-500/20"
                            : volatilityMetrics?.gdp.level === "high"
                              ? "bg-orange-500/20"
                              : "bg-red-500/20"
                      }`}
                    >
              ...
              <GlassCard>
                <div className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-muted-foreground text-sm">Population Volatility</p>
                      <p className="text-2xl font-bold">
                        {volatilityMetrics
                          ? `${(volatilityMetrics.population.value * 100).toFixed(1)}%`
                          : "N/A"}
                      </p>
                    </div>
                    <div
                      className={`rounded-full p-2 ${
                        volatilityMetrics?.population.level === "stable"
                          ? "bg-green-500/20"
                          : volatilityMetrics?.population.level === "moderate"
                            ? "bg-yellow-500/20"
                            : volatilityMetrics?.population.level === "high"
                              ? "bg-orange-500/20"
                              : "bg-red-500/20"
              ...
              <GlassCard>
                <div className="p-4">
                  ...
                  <p className="text-muted-foreground mt-1 text-xs">Historical records</p>
                </div>
              </GlassCard>
              ...
              <GlassCard>
                <div className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-muted-foreground text-sm">Growth Trend</p>
                      <p className="text-2xl font-bold">
          ...
          <TabsContent value="volatility" className="space-y-6">
            <GlassCard>
              <div className="p-6">
                <h3 className="mb-4 text-lg font-semibold">Volatility Analysis</h3>

                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                  <div>
                    <h4 className="mb-3 font-medium">GDP Volatility</h4>
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">
                          {volatilityMetrics
                            ? `${(volatilityMetrics.gdp.value * 100).toFixed(2)}%`
                            : "N/A"}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm">Trend:</span>
                        <span className="font-medium">{volatilityMetrics?.gdp.trend || "N/A"}</span>
                      </div>
                    </div>
                  </div>

                  <div>
                    <h4 className="mb-3 font-medium">Population Volatility</h4>
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        <span className="text-sm">Current Level:</span>
                        <Badge
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">
                          {volatilityMetrics
                            ? `${(volatilityMetrics.population.value * 100).toFixed(2)}%`
                            : "N/A"}
                        </span>
                      </div>
                      <div className="flex justify-between">
          ...
          <TabsContent value="correlations" className="space-y-6">
            <GlassCard>
              <div className="p-6">
                <h3 className="mb-4 text-lg font-semibold">Correlation Matrix</h3>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr>
                          ...
                          <th className="p-2 text-center">GDP/Capita</th>
                          <th className="p-2 text-center">GDP Growth</th>
                          <th className="p-2 text-center">Pop Growth</th>
                        </tr>
                      </thead>
                      <tbody>
          ...
          <TabsContent value="trends" className="space-y-6">
            <GlassCard>
              <div className="p-6">
                <h3 className="mb-4 text-lg font-semibold">Growth Rate Trends</h3>
                  ...
                  <GlassCard key={index} className={getInsightBgColor(insight.type)}>
                    <div className="p-4">
                      <div className="flex items-start gap-3">
                        <IconComponent
                          className={`mt-0.5 h-5 w-5 ${getInsightColor(insight.type)}`}
                        />
              ...
              {insights.length === 0 && (
                <GlassCard>
                  <div className="p-6 text-center">
                    <Lightbulb className="text-muted-foreground mx-auto mb-2 h-8 w-8" />
                    <p className="text-muted-foreground">
                      No insights available yet. Continue collecting data for automated insights.
                    </p>
                  </div>
                </GlassCard>
              )}
