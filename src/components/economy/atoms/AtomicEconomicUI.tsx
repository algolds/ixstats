      ...
      >
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <div className={`p-2 rounded-lg ${getEffectivenessBg(component.effectiveness)}`}>
              <component.icon className={`h-5 w-5 ${getEffectivenessColor(component.effectiveness)}`} />
            </div>
            ...
            <Progress 
              ...
              style={{
                background: `linear-gradient(to right, ${
                  component.effectiveness >= 85 ? '#22c55e' :
                  component.effectiveness >= 70 ? '#3b82f6' :
                  component.effectiveness >= 55 ? '#eab308' : '#ef4444'
                }, transparent)`
              }}
            />
          </div>
            ...
            <div className="space-y-2 pt-2 border-t">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div>
                  ...
                  <span className="ml-1 font-medium">{component.taxImpact.optimalCorporateRate}%</span>
                </div>
                <div>
              ...
              <div className="grid grid-cols-3 gap-2 text-xs">
                <div>
                  ...
                  <span className={`ml-1 font-medium ${
                    component.employmentImpact.unemploymentModifier < 0 ? 'text-green-600' : 'text-red-600'
                  }`}>
                    {component.employmentImpact.unemploymentModifier > 0 ? '+' : ''}{component.employmentImpact.unemploymentModifier.toFixed(1)}%
                  </span>
                </div>
                <div>
                  <span className="text-muted-foreground">Participation:</span>
                  <span className={`ml-1 font-medium ${
                    component.employmentImpact.participationModifier > 1 ? 'text-green-600' : 'text-red-600'
                  }`}>
                    {component.employmentImpact.participationModifier > 1 ? '+' : ''}{((component.employmentImpact.participationModifier - 1) * 100).toFixed(1)}%
                  </span>
                </div>
                <div>
                  <span className="text-muted-foreground">Wage Growth:</span>
                  <span className={`ml-1 font-medium ${
                    component.employmentImpact.wageGrowthModifier > 1 ? 'text-green-600' : 'text-red-600'
                  }`}>
                    {component.employmentImpact.wageGrowthModifier > 1 ? '+' : ''}{((component.employmentImpact.wageGrowthModifier - 1) * 100).toFixed(1)}%
                  </span>
                </div>
              </div>
            </div>
...
export const EconomicSynergyIndicator: React.FC<EconomicSynergyIndicatorProps> = ({
  ...
  return (
    <motion.div
      ...
      className={`flex items-center space-x-2 p-2 rounded-lg ${bgColor} ${className}`}
    >
      <Icon className={`h-4 w-4 ${color}`} />
      <div className="flex items-center space-x-1">
        <comp1.icon className="h-3 w-3 text-muted-foreground" />
        <span className="text-xs text-muted-foreground">+</span>
        <comp2.icon className="h-3 w-3 text-muted-foreground" />
      </div>
...
export const EconomicImpactPreview: React.FC<EconomicImpactPreviewProps> = ({
  ...
  const calculateSectorImpact = () => {
    ...
    sectors.forEach(sector => {
      impacts[sector] = selectedComponents.reduce((sum, compType) => {
  ...
  return (
    <Card className={`p-4 ${className}`}>
      <div className="space-y-4">
        ...
        {/* Sector Impacts */}
        <div>
          <h4 className="text-sm font-medium mb-2">Sector Performance</h4>
            ...
            {Object.entries(sectorImpacts).map(([sector, impact]) => (
              <div key={sector} className="flex items-center justify-between text-xs">
                ...
                <span className={`font-medium ${
                  impact > 1.1 ? 'text-green-600 dark:text-green-400' :
                  impact > 0.9 ? 'text-blue-600 dark:text-blue-400' :
        ...
        {/* Employment Impacts */}
        <div>
          <h4 className="text-sm font-medium mb-2">Employment Impact</h4>
          <div className="grid grid-cols-3 gap-2 text-xs">
            <div className="text-center">
              <div className={`font-medium ${
                employmentImpacts.unemployment < 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'
              }`}>
                {employmentImpacts.unemployment > 0 ? '+' : ''}{employmentImpacts.unemployment.toFixed(1)}%
              </div>
              <div className="text-muted-foreground">Unemployment</div>
            </div>
            <div className="text-center">
              <div className={`font-medium ${
                employmentImpacts.participation > 1 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'
              }`}>
            ...
            <div className="text-center">
              <div className={`font-medium ${
                employmentImpacts.wageGrowth > 1 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'
              }`}>
        ...
        {/* Tax Recommendations */}
        <div>
          <h4 className="text-sm font-medium mb-2">Optimal Tax Rates</h4>
          <div className="grid grid-cols-2 gap-2 text-xs">
