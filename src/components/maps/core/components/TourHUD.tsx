...
export function TourHUD({
  ...
  const { data: countryData, isLoading: isCountryLoading } = api.countries.getByNameWithAtomic.useQuery(
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className="fixed bottom-6 left-6 z-[40] w-full max-w-[380px] overflow-hidden rounded-2xl border border-slate-200 bg-white/90 text-slate-800 shadow-2xl backdrop-blur-xl dark:border-white/10 dark:bg-slate-950/80 dark:text-slate-100"
      >
        ...
        {/* HUD Header */}
        <div className="relative border-b border-slate-100 px-5 py-4 dark:border-white/5">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-3">
              {isCountryLoading || isStatsLoading ? (
                ...
                <img
                  ...
                  className="h-5 w-8 rounded object-cover shadow-md border border-slate-200 dark:border-white/10"
                />
              ) : (
                <div className="h-5 w-8 rounded bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-white/10" />
              )}
              <div>
            ...
            <button
              onClick={exitTour}
              className="rounded-full bg-slate-100 p-1 text-slate-500 hover:bg-slate-200 hover:text-slate-800 dark:bg-white/5 dark:text-slate-400 dark:hover:bg-white/10 dark:hover:text-white transition-colors"
            >
        ...
        <div className="relative px-5 py-4 space-y-4">
          ...
          {/* Quick Stats Grid */}
          <div className="grid grid-cols-3 gap-2 border-t border-slate-100 pt-3 dark:border-white/5">
            <div className="space-y-0.5 rounded-lg bg-slate-50/50 p-2 text-center border border-slate-100 dark:bg-white/5 dark:border-white/5">
              <span className="flex items-center justify-center gap-1 text-[9px] uppercase tracking-wider text-slate-500 dark:text-slate-400">
                <MapPin className="h-2.5 w-2.5 text-blue-500 dark:text-blue-400" />
                Capital
              </span>
              {isCountryLoading || isStatsLoading ? (
                <div className="h-3 w-16 mx-auto bg-slate-200 dark:bg-white/10 rounded animate-pulse mt-1" />
              ) : (
        ...
        <div className="relative flex items-center justify-between border-t border-slate-100 px-5 py-3.5 bg-slate-50/50 dark:border-white/5 dark:bg-slate-900/40">
          <div className="flex items-center gap-2">
            <button
              onClick={prevStep}
              disabled={currentStepIndex === 0}
              className="rounded-lg bg-slate-100 p-2 text-slate-600 hover:bg-slate-200 hover:text-slate-900 disabled:opacity-40 disabled:hover:bg-slate-100 disabled:hover:text-slate-600 dark:bg-white/5 dark:text-slate-300 dark:hover:bg-white/10 dark:hover:text-white dark:disabled:hover:bg-white/5 dark:disabled:hover:text-slate-300 transition-colors"
            >
              <ChevronLeft className="h-4 w-4" />
            </button>
            <button
              onClick={togglePause}
              className="rounded-lg bg-blue-600 p-2 text-white hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 shadow-md shadow-blue-500/10 transition-colors"
            >
              {isPaused ? <Play className="h-4 w-4" /> : <Pause className="h-4 w-4" />}
            </button>
            <button
              onClick={nextStep}
              className="rounded-lg bg-slate-100 p-2 text-slate-600 hover:bg-slate-200 hover:text-slate-900 dark:bg-white/5 dark:text-slate-300 dark:hover:bg-white/10 dark:hover:text-white transition-colors"
            >
          ...
          <button
            onClick={exitTour}
            className="text-xs font-medium text-slate-600 hover:text-slate-900 bg-slate-100 hover:bg-slate-200 px-3 py-1.5 rounded-lg transition-colors border border-slate-200 dark:text-slate-400 dark:hover:text-white dark:bg-white/5 dark:hover:bg-white/10 dark:border-white/5"
          >
            Exit Tour
          </button>
        </div>
