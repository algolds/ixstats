...
const SUBSYSTEMS = [
  ...
  { icon: Database, color: "text-emerald-400", bg: "from-emerald-400 to-emerald-600", label: "Countries" },
  { icon: BookOpen, color: "text-amber-400", bg: "from-amber-400 to-amber-600", label: "Wiki Data" },
  { icon: ImageIcon, color: "text-purple-400", bg: "from-purple-400 to-purple-600", label: "Media" },
] as const;
    ...
    >
      ...
      <div className="relative z-10 flex max-w-md flex-col items-center gap-8 px-6 text-center">
        ...
        <div className="relative h-36 w-36">
          {/* Glow backdrop */}
          <div className="absolute inset-[-20%] rounded-full bg-blue-500/10 blur-2xl animate-pulse" style={{ animationDuration: "3s" }} />
          {/* Outer ring - slow spin with gradient */}
          <div
            className="absolute inset-0 animate-[spin_8s_linear_infinite] rounded-full"
            style={{ border: "2px solid transparent", backgroundImage: "conic-gradient(from 0deg, rgba(59,130,246,0.4), transparent 40%, rgba(147,51,234,0.3) 60%, transparent)", backgroundOrigin: "border-box", backgroundClip: "border-box" }}
          />
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <img
              ...
              alt="Ixnay"
              className="h-16 w-16 brightness-0 invert drop-shadow-[0_0_12px_rgba(147,197,253,0.5)]"
              style={{ animation: "logoPulse 2.5s ease-in-out infinite" }}
            />
          </div>
          {/* Orbiting dot 1 */}
          <div className="absolute inset-0 animate-[spin_3s_linear_infinite]">
            <div className="absolute left-1/2 top-0 h-2 w-2 -translate-x-1/2 -translate-y-0.5 rounded-full bg-blue-400 shadow-[0_0_10px_rgba(59,130,246,0.7)]" />
          </div>
          {/* Orbiting dot 2 (opposite, slower) */}
          <div className="absolute inset-0 animate-[spin_5s_linear_infinite_reverse]">
            <div className="absolute left-1/2 bottom-0 h-1.5 w-1.5 -translate-x-1/2 translate-y-0.5 rounded-full bg-purple-400 shadow-[0_0_8px_rgba(147,51,234,0.6)]" />
          </div>
        </div>
        ...
        <div className="grid w-full grid-cols-2 gap-3">
          {SUBSYSTEMS.map((sys, i) => (
            <div key={sys.label} className="flex items-center gap-2">
              <sys.icon className={`h-4 w-4 flex-shrink-0 ${sys.color}`} />
              <div className="flex-1">
                <div className="text-[10px] font-medium text-white/50 text-left">{sys.label}</div>
      ...
      <style jsx>{`
        ...
        @keyframes logoPulse {
          0%, 100% { opacity: 0.7; transform: scale(1); filter: brightness(0) invert(1) drop-shadow(0 0 12px rgba(147,197,253,0.5)); }
          50% { opacity: 1; transform: scale(1.08); filter: brightness(0) invert(1) drop-shadow(0 0 20px rgba(147,197,253,0.8)); }
        }
      `}</style>
    </div>
  );
