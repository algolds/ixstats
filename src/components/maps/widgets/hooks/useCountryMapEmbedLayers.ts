...
function setWorldLayersVisibility(map: any, visibility: "visible" | "none") {
  if (!map) return;
...
function cleanupEmbedLayers(map: any) {
  if (!map) return;
  for (const layerId of EMBED_LAYER_IDS) {
...
export function useCountryMapEmbedLayers({
  ...
  const initMap = useCallback(async () => {
    ...
    releaseFn = acquireMap(`embed-${countryId}`, {
      ...
      onReady: (map) => {
        ...
        // Hide world map layers
        setWorldLayersVisibility(map, "none");

        // Clean up custom embed layers
        cleanupEmbedLayers(map);
              ...
              if (onNeighborClick) {
                map.on("click", "highlighted-labels", (e) => {
                  const cid =
                    ...
                    e.features?.[0]?.properties?._countryId;
        ...
        if (showCities && state.cities && state.cities.length > 0) {
          ...
          if (cityFeatures.length > 0) {
            ...
            updateCityFilter = () => {
              ...
              const anyBranch: any[] = [
                [">=", ["coalesce", ["get", "population"], 0], popThreshold],
              ];
    ...
    return () => {
      ...
      if (state.mapRef.current) {
        setWorldLayersVisibility(state.mapRef.current, "visible");
        cleanupEmbedLayers(state.mapRef.current);
