...
export function SimulationPreview({ countryId, visible }: SimulationPreviewProps) {
  ...
  return (
    <div className="border-border bg-card/50 border-t p-3">
      ...
      {/* Transport stats */}
      {transportStats &&
        (transportStats.totalRoutes > 0 || (transportStats.totalMaintenanceCost ?? 0) > 0) && (
          <div className="border-border mt-2 space-y-1 border-t pt-2">
            ...
            {transportStats.totalMaintenanceCost !== undefined &&
              transportStats.totalMaintenanceCost > 0 && (
                <div className="flex items-center justify-between text-[11px]">
            ...
            {transportStats.resources.map((res: any) => (
              <div key={res.id} className="flex items-center justify-between text-[10px]">
                <div className="flex items-center gap-1.5 truncate">
                  ...
                  />
                  <span className="text-foreground truncate font-medium">{res.name}</span>
