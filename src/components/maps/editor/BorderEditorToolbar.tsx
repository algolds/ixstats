...
const MODES: Array<{ id: BorderEditMode; label: string; icon: React.ReactNode; tip: string }> = [
  { id: "select", label: "Select", icon: <MousePointer2 className="h-4 w-4" />, tip: "Click to select a feature" },
  { id: "vertex_edit", label: "Edit Vertices", icon: <Pencil className="h-4 w-4" />, tip: "Click vertices to select, drag to move" },
  { id: "split", label: "Split", icon: <Scissors className="h-4 w-4" />, tip: "Draw a line to split a country" },
  { id: "merge", label: "Merge", icon: <Merge className="h-4 w-4" />, tip: "Select countries to merge together" },
];
...
export const BorderEditorToolbar = React.memo(function BorderEditorToolbar({
  ...
  return (
    <div className="flex flex-col gap-2 rounded-lg border border-border bg-card/90 p-2 backdrop-blur">
      {/* Mode buttons */}
      ...
      {mode === "split" && splitPointCount > 0 && (
        <div className="border-t border-border pt-2 text-xs text-amber-500">
      ...
      <div className="flex gap-1 border-t border-border pt-2">
        <button
          ...
          className="flex items-center gap-1 rounded bg-muted/50 px-2 py-1.5 text-xs text-foreground hover:bg-muted/80 disabled:opacity-30"
          title="Save draft"
        >
