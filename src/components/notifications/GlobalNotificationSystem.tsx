...
export function GlobalNotificationSystem({
  ...
  useEffect(() => {
    ...
    console.log(
      "[GlobalNotificationSystem] Setting up real-time connections for country:",
  ...
  return (
    ...
    >
      ...
      {process.env.NODE_ENV === "development" && <NotificationDebugPanel />}
    </UnifiedNotificationProvider>
  );
}
...
function NotificationDebugPanel() {
  ...
  return (
    <div className="fixed right-4 bottom-4 z-[10003] max-h-96 w-96 overflow-y-auto rounded-lg bg-black/90 p-4 text-xs text-white shadow-2xl">
      ...
      <div className="space-y-2">
        ...
        <div className="border-t border-gray-700 pt-2">
          ...
          {notifications.slice(0, 5).map((notification) => (
            <div key={notification.id} className="ml-2 border-b border-gray-800 py-1">
        ...
        <div className="text-[10px] text-gray-500">Press Ctrl+Shift+N to toggle</div>
      </div>
    </div>
  );
}
