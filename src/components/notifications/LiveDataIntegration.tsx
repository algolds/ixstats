...
export function LiveDataIntegration({
  ...
  const diplomaticEvents = [
    ...
    ...(diplomaticRelations ? diplomaticRelations.map((relation: any) => ({
      id: `rel-${relation.id}`,
      ...
      significance: relation.strength > 80 ? 'major' : relation.strength > 50 ? 'moderate' : 'minor'
    })) : []),
    
    // Recent changes (actual events)
    ...(recentDiplomaticChanges ? recentDiplomaticChanges.map((change: any) => ({
  ...
  useEffect(() => {
    ...
    const newItems = intelligence.filter(item => 
  ...
  useEffect(() => {
    ...
    if (previous && economicData) {
      ...
      if (economicData.gdp && previous.gdp && economicData.gdp !== previous.gdp) {
        ...
        if (Math.abs(gdpChange) > 1) { // 1% change threshold
          changes.push({
            metric: 'GDP',
      ...
      if (economicData.gdpPerCapita && previous.gdpPerCapita && economicData.gdpPerCapita !== previous.gdpPerCapita) {
        const gdpPerCapitaChange = ((economicData.gdpPerCapita - previous.gdpPerCapita) / previous.gdpPerCapita) * 100;
        if (Math.abs(gdpPerCapitaChange) > 2) { // 2% change threshold
          changes.push({
      ...
      // Growth rate changes
      if (economicData.growthRate !== undefined && previous.growthRate !== undefined && economicData.growthRate !== previous.growthRate) {
        const growthRateChange = economicData.growthRate - previous.growthRate;
        if (Math.abs(growthRateChange) > 0.5) { // 0.5% change threshold
          changes.push({
            metric: 'Growth Rate',
  ...
  useEffect(() => {
    ...
    const newEvents = diplomaticEvents.filter((event: any) => 
      !previous.some((prev: any) => prev.id === event.id)
    );

    if (newEvents.length > 0) {
      console.log(`[LiveDataIntegration] Processing ${newEvents.length} new diplomatic events via specialized service`);
      
      // Use the specialized diplomatic notification service
      newEvents.forEach(async (event: any) => {
        const diplomaticEvent = {
          id: event.id || `event-${Date.now()}`,
          type: (event.type || 'meeting') as 'agreement' | 'treaty' | 'conflict' | 'trade' | 'alliance' | 'sanction' | 'embassy' | 'meeting',
          ...
          significance: (event.significance || 'moderate') as 'minor' | 'moderate' | 'major' | 'historic',
    ...
    const checkAchievements = () => {
      ...
      if (intelligence && Array.isArray(intelligence)) {
        ...
        if (securityEvents > 10) {
          achievementNotificationService.processAchievementUnlock({
            id: 'security-expert',
