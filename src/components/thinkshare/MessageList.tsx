...
export function MessageList({
  ...
  const getTypingIndicators = () => {
    ...
    return Array.from(clientState.typingIndicators.values()).filter(
      (indicator: any) =>
      ...
      <div className="space-y-3">
        {isLoadingMessages ? (
          ...
          conversationMessages?.messages.map((message, index: number) => (
            <div
              ...
              className={`group flex ${message.accountId === currentAccount?.id ? "justify-end" : "justify-start"}`}
            >
              <MessageBubble
                message={message}
        ...
        {getTypingIndicators().map((indicator: any) => {
          ...
          const participant = selectedConversation.otherParticipants.find(
            (p: any) => p.accountId === indicator.accountId
          );
