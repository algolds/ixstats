  ...
  for (const child of children) {
    ...
    switch (el.type) {
      case "link": {
        ...
        if (el.internal) {
          const target = el.url.replace(/^\/wiki\//, "").replace(/_/g, " ");
          out += decodeURIComponent(target) === label ? `[[${target}]]` : `[[${target}|${label}]]`;
        } else {
          out += `[${el.url} ${label}]`;
        }
        break;
      ...
      case "chip-coord":
      case "chip-mapembed":
        // Atomic nodes without stored wikitext make the document incomplete.
        return out; // caller marks complete=false via flag below
      default:
        out += leavesToWikitext([child]);
    }
  }
