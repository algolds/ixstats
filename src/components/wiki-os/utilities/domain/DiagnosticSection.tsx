...
export function DiagnosticSection({ searchFilter }: DiagnosticSectionProps) {
  const [activeTab, setActiveTab] = useState<"orphans" | "deadEnds" | "brokenRedirects" | "short" | "long" | null>(null);
  ...
  const { data: brokenRedirects, isLoading: loadingRedirects } = api.wikios.getBrokenRedirects.useQuery({
    limit: 50,
  });
  const { data: shortestArticles, isLoading: loadingShort } = api.wikios.getShortestArticles.useQuery({
    limit: 25,
  });
              ...
              {loadingOrphans ? (
                ...
                <div className="space-y-1">
                  {orphans.map((item: any, idx: number) => (
                    ...
                    >
                      <Link
                        href={withBasePath(`/wiki/${encodeURIComponent(item.slug || item.title)}`)}
                        data-cuelume-press="page"
                        data-cuelume-hover="tick"
                        className="font-medium text-foreground hover:text-wiki hover:underline"
                      >
                        {item.title}
                      </Link>
