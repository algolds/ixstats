...
import { Save, RotateCcw, Info, Blocks, Zap, AlertTriangle, Package, HelpCircle, Target, DollarSign, TrendingUp } from "lucide-react";
...
export function AtomicGovernmentComponents({
  ...
  return (
    <TooltipProvider delayDuration={150}>
      ...
      {/* Header Section */}
      {!standalone && (
        <GlassCard variant="glass" glow={false} className="border-white/10 shadow-lg backdrop-blur-md">
          <CardHeader>
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <CardTitle className="text-2xl font-extrabold text-zinc-100 flex items-center gap-2">
              ...
              {!isReadOnly && (
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <Button
                    ...
                    className="bg-cyan-600 hover:bg-cyan-500 text-white font-bold shadow-md shadow-cyan-500/10"
                  >
      ...
      <Dialog open={selectedListOpen} onOpenChange={setSelectedListOpen}>
        <DialogContent className="max-h-[80vh] max-w-lg overflow-y-auto border-zinc-200 dark:border-white/10 bg-white/95 dark:bg-zinc-900/90 backdrop-blur-2xl text-zinc-900 dark:text-zinc-100 sm:max-w-lg shadow-2xl">
          <DialogHeader>
      ...
      <Dialog open={interactionsOpen} onOpenChange={setInteractionsOpen}>
        <DialogContent className="max-h-[85vh] max-w-lg overflow-y-auto border-zinc-200 dark:border-white/10 bg-white/95 dark:bg-zinc-900/90 backdrop-blur-2xl text-zinc-900 dark:text-zinc-100 sm:max-w-lg shadow-2xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-base font-bold text-foreground">
          ...
          <div className="space-y-6 pt-4">
            {/* Active Synergies */}
            <div>
              <h4 className="mb-3 flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-green-600 dark:text-green-400">
                <Zap className="h-4 w-4" />
                Active Synergies ({builder.synergies.length})
              </h4>
              {builder.synergies.length === 0 ? (
                <p className="text-xs text-zinc-500 italic pl-6">No active synergies.</p>
              ) : (
                  ...
                  {builder.synergies.map(({ comp1, comp2, score }, index) => {
                    ...
                    return (
                      <div
                        ...
                        className="rounded-xl border border-green-500/20 bg-green-500/5 p-3 hover:bg-green-500/10 dark:hover:bg-green-500/10 transition-colors"
                      >
                        <div className="flex items-center justify-between gap-3">
                          <div className="min-w-0 flex-1">
                            <p className="text-xs font-semibold text-zinc-900 dark:text-foreground">
                          ...
                          <Badge variant="outline" className="shrink-0 bg-green-600/10 border border-green-500/20 text-green-600 dark:text-green-400 font-bold">
                            +{score}%
                          </Badge>
                        </div>
                      </div>
            ...
            {/* Active Conflicts */}
            <div>
              <h4 className="mb-3 flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-red-600 dark:text-red-400">
                <AlertTriangle className="h-4 w-4" />
                Active Conflicts ({builder.conflicts.length})
              </h4>
              {builder.conflicts.length === 0 ? (
                <p className="text-xs text-zinc-500 italic pl-6">No active conflicts.</p>
              ) : (
                <div className="space-y-2 max-h-[30vh] overflow-y-auto pr-1">
                  {builder.conflicts.map(({ comp1, comp2 }, index) => {
                    ...
                    return (
                      <div
                        key={`${comp1}-${comp2}-${index}`}
                        className="rounded-xl border border-red-500/20 bg-red-500/5 p-3 hover:bg-red-500/10 dark:hover:bg-red-500/10 transition-colors"
                      >
                        <div className="flex items-center justify-between gap-3">
                          <div className="min-w-0 flex-1">
                            <p className="text-xs font-semibold text-zinc-900 dark:text-foreground">
                          ...
                          <Badge variant="outline" className="shrink-0 bg-red-600/10 border border-red-500/20 text-red-600 dark:text-red-400 font-bold">
                            -15%
                          </Badge>
                        </div>
                      </div>
      ...
      <Dialog open={effectivenessOpen} onOpenChange={setEffectivenessOpen}>
        <DialogContent className="max-h-[85vh] max-w-lg overflow-y-auto border-zinc-200 dark:border-white/10 bg-white/95 dark:bg-zinc-900/90 backdrop-blur-2xl text-zinc-900 dark:text-zinc-100 sm:max-w-lg shadow-2xl">
          ...
          <div className="space-y-6 pt-4">
            <div className="grid grid-cols-2 gap-4 rounded-xl border border-zinc-200 dark:border-white/5 bg-zinc-50 dark:bg-white/[0.02] p-4 text-center">
              <div className="space-y-1">
                <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Base Score</p>
                <p className="text-xl font-extrabold text-zinc-800 dark:text-zinc-300">
                  {builder.effectiveness.baseEffectiveness.toFixed(1)}%
                </p>
              </div>
              <div className="space-y-1">
                <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Synergy Bonus</p>
                <p className="text-xl font-extrabold text-green-600 dark:text-green-400">
                  +{builder.effectiveness.synergyBonus.toFixed(1)}%
                </p>
              </div>
              <div className="space-y-1 mt-2">
                <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Conflict Penalty</p>
                <p className="text-xl font-extrabold text-red-600 dark:text-red-400">
                  -{builder.effectiveness.conflictPenalty.toFixed(1)}%
                </p>
              </div>
              <div className="space-y-1 mt-2">
                <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Total Score</p>
            ...
            <div className="space-y-3">
              <h4 className="text-xs font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">Component Contributions</h4>
              {selectedComponentObjects.length === 0 ? (
                ...
                <div className="space-y-2 max-h-[30vh] overflow-y-auto pr-1">
                  {selectedComponentObjects.map((comp) => (
                    <div key={comp.type} className="flex items-center justify-between border-b border-zinc-200 dark:border-white/5 pb-2 text-xs">
                      <span className="text-zinc-700 dark:text-zinc-300 font-medium">{comp.name}</span>
                      <span className="font-mono text-zinc-500 dark:text-zinc-400 font-bold">{comp.effectiveness}% base</span>
                    </div>
                  ))}
                </div>
              )}
        ...
        <DialogContent className="max-h-[85vh] max-w-lg overflow-y-auto border-zinc-200 dark:border-white/10 bg-white/95 dark:bg-zinc-900/90 backdrop-blur-2xl text-zinc-900 dark:text-zinc-100 sm:max-w-lg shadow-2xl">
          ...
          <div className="space-y-6 pt-4">
            <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4 text-center">
              <p className="text-[10px] font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400">Total Implementation Cost</p>
            ...
            <div className="space-y-3">
              <h4 className="text-xs font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">Cost per Component</h4>
              {selectedComponentObjects.length === 0 ? (
                <p className="text-xs text-zinc-500 italic">No components selected.</p>
              ) : (
                <div className="space-y-2 max-h-[30vh] overflow-y-auto pr-1">
                  {selectedComponentObjects.map((comp) => (
                    <div key={comp.type} className="flex items-center justify-between border-b border-zinc-200 dark:border-white/5 pb-2 text-xs">
                      <div className="flex flex-col">
                        <span className="text-zinc-700 dark:text-zinc-300 font-medium">{comp.name}</span>
                        <span className="text-[9px] text-zinc-400 capitalize">{comp.category}</span>
                      </div>
                      <span className="font-mono text-emerald-600 dark:text-emerald-400 font-bold">${comp.implementationCost.toLocaleString()}</span>
                    </div>
                  ))}
                </div>
              )}
        ...
        <DialogContent className="max-h-[85vh] max-w-lg overflow-y-auto border-zinc-200 dark:border-white/10 bg-white/95 dark:bg-zinc-900/90 backdrop-blur-2xl text-zinc-900 dark:text-zinc-100 sm:max-w-lg shadow-2xl">
          ...
          <div className="space-y-6 pt-4">
            <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4 text-center">
              <p className="text-[10px] font-bold uppercase tracking-wider text-amber-600 dark:text-amber-400">Total Annual Maintenance Cost</p>
              <p className="text-2xl font-black text-zinc-900 dark:text-zinc-100 mt-1">
            ...
            <div className="space-y-3">
              <h4 className="text-xs font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">Maintenance per Component</h4>
              {selectedComponentObjects.length === 0 ? (
                <p className="text-xs text-zinc-500 italic">No components selected.</p>
              ) : (
                <div className="space-y-2 max-h-[30vh] overflow-y-auto pr-1">
                  {selectedComponentObjects.map((comp) => (
                    <div key={comp.type} className="flex items-center justify-between border-b border-zinc-200 dark:border-white/5 pb-2 text-xs">
                      ...
                      <span className="font-mono text-amber-600 dark:text-amber-400 font-bold">${comp.maintenanceCost.toLocaleString()}/yr</span>
                    </div>
                  ))}
                </div>
              )}
      ...
      <GlassCard variant="glass" glow={false} className="border-white/10 bg-white/[0.01] dark:bg-black/20 shadow-2xl backdrop-blur-xl">
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            {!hideSelectedList && (
              <div className="lg:col-span-1 border-t border-white/5 pt-6 lg:border-t-0 lg:border-l lg:border-white/5 lg:pt-0 lg:pl-6">
                <SelectedComponentsList
                  selectedComponents={selectedComponentObjects}
                  onDeselect={builder.deselectComponent}
                  isReadOnly={isReadOnly}
