...
import {
  ComponentType,
  ATOMIC_COMPONENTS,
  type AtomicGovernmentComponent,
} from "./atoms/AtomicGovernmentComponents";
...
export const AtomicGovernmentDashboard: React.FC<AtomicGovernmentDashboardProps> = ({
  ...
  const calculateEffectiveness = useMemo(() => {
    ...
    const components = Array.from(selectedComponents)
      ...
      .filter(Boolean) as AtomicGovernmentComponent[];
        ...
        <CardContent>
          ...
          <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-4">
            <AtomicMetric
              ...
              unit="%"
              trend={
                calculateEffectiveness.total > 70
                  ? "up"
                  : calculateEffectiveness.total < 50
                    ? "down"
                    : "stable"
              }
              status={
                calculateEffectiveness.total > 75
                  ? "success"
                  : calculateEffectiveness.total > 50
                    ? "warning"
                    : "error"
            ...
            <AtomicMetric
              ...
              status={
                totalCosts.capacity > 90
                  ? "error"
                  : totalCosts.capacity > 70
                    ? "warning"
                    : "success"
              }
              icon={Activity}
          ...
          <AtomicEffectiveness
            ...
            factors={[
              ...
              {
                name: "Conflict Penalties",
                impact: -calculateEffectiveness.conflict,
                positive: false,
              },
            ]}
            className="mb-6"
                      ...
                      (component) =>
                        component && (
                          <AtomicComponentCard
                            ...
                            synergies={component.synergies?.map(
                              (s) => ATOMIC_COMPONENTS[s as ComponentType]?.name || s
                            )}
                            conflicts={component.conflicts?.map(
                              (c) => ATOMIC_COMPONENTS[c as ComponentType]?.name || c
                            )}
                          />
                        )
                    )}
                ...
                <Card className="glass-hierarchy-child">
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      <AtomicStatus
                        status="active"
                        label={`${selectedComponents.size} Active Components`}
                        pulse
                      />
                      <AtomicStatus
                        status="pending"
                        label={`${synergyAnalysis.synergies.length} Synergies`}
                      />
                      <AtomicStatus
                        status="error"
                        label={`${synergyAnalysis.conflicts.length} Conflicts`}
                      />
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      <div className="flex justify-between">
                        <span className="text-muted-foreground text-sm">Implementation</span>
                        <span className="font-medium">
                          ₡{totalCosts.implementation.toLocaleString()}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground text-sm">Monthly Maintenance</span>
                        <span className="font-medium">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">
                          ₡{(totalCosts.maintenance * 12).toLocaleString()}
                        </span>
                      </div>
                      <div className="flex justify-between border-t pt-3">
                        ...
                        <span className="font-bold">
                          ...
                          {calculateEffectiveness.total > 0
                            ? Math.round(
                                totalCosts.maintenance / calculateEffectiveness.total
                              ).toLocaleString()
                            : "0"}
                        </span>
                      </div>
