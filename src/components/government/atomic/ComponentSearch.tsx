...
export const ComponentSearch = React.memo<ComponentSearchProps>(
  ({
    ...
    return (
      <div
        className={cn(
          "flex w-full items-center gap-3 overflow-hidden rounded-xl border px-3 h-12 shadow-md transition-all duration-300 backdrop-blur-md",
          isFocused
      ...
      >
        ...
        <input
          ...
          className="flex-1 bg-transparent text-sm outline-none border-0 ring-0 focus:ring-0 focus:outline-none text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-zinc-500"
        />

        {value && (
          <button
            onClick={() => onChange("")}
            className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full transition-colors hover:bg-slate-100 dark:hover:bg-white/10 cursor-pointer"
            title="Clear search"
          >
            <X className="text-slate-400 hover:text-cyan-500 dark:text-zinc-500 dark:hover:text-cyan-400 h-3.5 w-3.5 transition-colors" />
          </button>
        )}
      </div>
    );
