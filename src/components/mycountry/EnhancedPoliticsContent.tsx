...
export function EnhancedPoliticsContent({
  ...
  const politicsRings = useMemo((): RingConfig[] => {
    ...
    return [
      { key: "parties", label: "Parties", subtitle: "registered", color: partyColor, icon: Users, value: partyCount, target: Math.max(partyCount + 2, 5), displayValue: `${partyCount} active` },
      { key: "seats", label: "Parliament", subtitle: totalSeats > 0 ? `${totalSeats} total seats` : "not configured", color: seatColor, icon: Landmark, value: filledSeats, target: totalSeats || 1, displayValue: totalSeats > 0 ? `${filledSeats}/${totalSeats}` : "—" },
      { key: "elections", label: "Elections", subtitle: "held", color: completedElections > 0 ? "#8b5cf6" : "#6b7280", icon: BarChart3, value: completedElections, target: Math.max(completedElections + 2, 3), displayValue: `${completedElections} completed` },
    ];
  ...
  const statusBadges: StatusBadgeConfig[] = pendingElections > 0
    ? [{ icon: BarChart3, count: pendingElections, colorClass: "border-indigo-500/40 text-indigo-600 dark:text-indigo-400" }]
    : [];
