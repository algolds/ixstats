...
export function EnhancedDefenseContent({
  ...
  const defenseRings = useMemo((): RingConfig[] => {
    ...
    const scoreColor = securityScore >= 75 ? "#22c55e" : securityScore >= 50 ? "#3b82f6" : "#f97316";
    const readinessColor = avgReadiness >= 70 ? "#22c55e" : "#eab308";

    return [
      { key: "security", label: "Security", subtitle: securityLevel, color: scoreColor, icon: Shield, value: securityScore, target: 100, displayValue: `${securityScore}/100`, onClick: () => setActiveTab("command") },
      { key: "branches", label: "Branches", subtitle: "military forces", color: "#ef4444", icon: Sword, value: branchCount, target: branchCount + 2, displayValue: `${branchCount} active`, onClick: () => setActiveTab("forces") },
      { key: "readiness", label: "Readiness", subtitle: "avg. combat ready", color: readinessColor, icon: Target, value: avgReadiness, target: 100, displayValue: `${avgReadiness}%`, onClick: () => setActiveTab("operations") },
    ];
  ...
  const tabs = [
    { value: "command" as const, icon: Activity, label: "Command", shortLabel: "Cmd", badge: activeThreatCount },
    { value: "forces" as const, icon: Sword, label: "Forces", shortLabel: "Forces", badge: 0 },
    { value: "operations" as const, icon: Target, label: "Operations", shortLabel: "Ops", badge: 0 },
  ];

  const header = (
    <SectionHeaderBackground context="defense">
      <div id="overview" className="glass-hierarchy-parent rounded-xl border border-red-500/30 p-3 dark:border-red-500/20 sm:p-4">
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        <StatCell icon={Shield} label="Security" value={`${securityScore}/100`} color={securityScore >= 75 ? "text-green-600" : securityScore >= 50 ? "text-blue-600" : "text-orange-600"} />
        <StatCell icon={Sword} label="Branches" value={`${branchCount} active`} color="text-red-600" />
        <StatCell icon={Target} label="Readiness" value={`${avgReadiness}%`} color={avgReadiness >= 70 ? "text-green-600" : "text-yellow-600"} />
        <StatCell icon={Activity} label="Threats" value={`${securityData?.activeThreatCount ?? 0}`} color={(securityData?.activeThreatCount ?? 0) > 0 ? "text-red-600" : "text-green-600"} />
      </div>
      ...
      >
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as DefenseTab)} className="space-y-3">
              ...
              >
                ...
                {tab.badge > 0 && (
                  <span className={`inline-flex items-center justify-center rounded-full text-[9px] font-bold leading-none min-w-[14px] h-3.5 px-1 ${
                    tab.value === "command" && activeThreatCount > 0 ? "bg-red-500 text-white" : "bg-amber-500 text-white"
                  }`}>
                    {tab.badge}
                  </span>
                )}
          ...
          <TabsContent value="forces">
            <ThemedTabContent theme="defense" className="tab-content-enter">
              <TabHeroBanner context="defense_forces" title="Military Forces" subtitle="Branch customization and force structure" icon={Sword} accentColor="red" />
          ...
          <TabsContent value="operations">
            <ThemedTabContent theme="defense" className="tab-content-enter">
              <TabHeroBanner context="defense_operations" title="Active Operations" subtitle="Deployments and tactical missions" icon={Target} accentColor="red" />
