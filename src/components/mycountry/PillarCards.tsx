...
import { Bell, FileText, Building2, Handshake, Users, Landmark, BarChart3, Layers, ChevronRight } from "lucide-react";
...
export function PillarCards({ countryId, onNavigate }: PillarCardsProps) {
  ...
  const pillars: {
  ...
  }[] = [
    {
      ...
      badge: urgentIssueCount > 0 ? { label: `${urgentIssueCount} urgent`, colorClass: "bg-red-500 text-white" } : issueCount > 0 ? { label: `${issueCount} pending`, colorClass: "bg-amber-500 text-white" } : undefined,
      stats: [
        { icon: Bell, label: "Issues", value: `${issueCount} pending`, colorClass: urgentIssueCount > 0 ? "text-red-600 dark:text-red-400" : "text-amber-600 dark:text-amber-400" },
        { icon: FileText, label: "Policies", value: `${activePolicies} active`, colorClass: "text-amber-600 dark:text-amber-400" },
        { icon: Layers, label: "Actions", value: pendingActions > 0 ? `${pendingActions} pending` : "all clear", colorClass: pendingActions > 0 ? "text-orange-600 dark:text-orange-400" : "text-green-600 dark:text-green-400" },
      ],
    },
    {
      section: "diplomacy",
      ...
      stats: [
        { icon: Building2, label: "Embassies", value: `${activeEmbassies} active`, colorClass: "text-cyan-600 dark:text-cyan-400" },
        { icon: Handshake, label: "Relations", value: `${totalRelations} nations`, colorClass: "text-cyan-600 dark:text-cyan-400" },
        { icon: Handshake, label: "Avg Strength", value: `${avgStrength}%`, colorClass: avgStrength >= 50 ? "text-green-600 dark:text-green-400" : "text-cyan-600 dark:text-cyan-400" },
      ],
    },
    {
      section: "politics",
      ...
      badge: pendingElections > 0 ? { label: `${pendingElections} election${pendingElections > 1 ? "s" : ""} pending`, colorClass: "bg-indigo-500 text-white" } : undefined,
      stats: [
        { icon: Users, label: "Parties", value: `${partyCount} active`, colorClass: partyCount >= 3 ? "text-indigo-600 dark:text-indigo-400" : "text-orange-600 dark:text-orange-400" },
        { icon: Landmark, label: "Parliament", value: totalSeats > 0 ? `${totalSeats} seats` : "not set up", colorClass: "text-indigo-600 dark:text-indigo-400" },
        { icon: BarChart3, label: "Elections", value: pendingElections > 0 ? `${pendingElections} pending` : "none pending", colorClass: pendingElections > 0 ? "text-violet-600 dark:text-violet-400" : "text-indigo-600 dark:text-indigo-400" },
      ],
    },
  ];
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {pillars.map((pillar) => (
          <button
            ...
            className={`glass-hierarchy-child rounded-xl border ${pillar.border} p-4 text-left transition-all duration-200 hover:shadow-lg group`}
          >
            {/* Header */}
