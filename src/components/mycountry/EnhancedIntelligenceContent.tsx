...
export function EnhancedIntelligenceContent({
  ...
  const intelligenceRings = useMemo((): RingConfig[] => {
    ...
    return [
      { key: "security", label: "Security", subtitle: securityLevel, color: "#3B82F6", icon: Shield, value: securityScore, target: 100, displayValue: `${securityScore}/100`, onClick: () => setActiveTab("dashboard") },
      { key: "alerts", label: "Alerts", subtitle: `${totalAlerts} total`, color: alertColor, icon: AlertTriangle, value: criticalAlerts, target: totalAlerts || 1, displayValue: `${criticalAlerts} critical`, onClick: () => setActiveTab("reports") },
      { key: "network", label: "Network", subtitle: "embassies", color: "#3B82F6", icon: Globe, value: activeEmbassies, target: totalEmbassies || 1, displayValue: `${activeEmbassies} active`, onClick: () => setActiveTab("analysis") },
    ];
  ...
  const header = (
    ...
    >
      <SectionHeaderBackground context="intelligence">
        <div id="overview" className="glass-hierarchy-parent rounded-xl border border-blue-500/30 p-3 dark:border-blue-500/20 sm:p-4">
    ...
    <>
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
          <StatCell icon={Shield} label="Security" value={`${defenseOverview?.overallScore ?? 0}/100`} color="text-blue-600" />
          <StatCell icon={AlertTriangle} label="Alerts" value={`${intelligenceOverview?.alerts?.critical ?? 0} critical`} color={(intelligenceOverview?.alerts?.critical ?? 0) > 0 ? "text-red-600" : "text-green-600"} />
          <StatCell icon={Globe} label="Embassies" value={`${embassies?.filter((e: any) => e.status === "ACTIVE" || e.status === "active").length ?? 0} active`} color="text-blue-600" />
                ...
                >
                  ...
                  {tab.badge > 0 && (
                    <span className={`inline-flex items-center justify-center rounded-full text-[9px] font-bold leading-none min-w-[14px] h-3.5 px-1 ${
                      tab.value === "reports" && criticalAlerts > 0 ? "bg-red-500 text-white" : "bg-amber-500 text-white"
                    }`}>
                      {tab.badge}
                    </span>
                  )}
