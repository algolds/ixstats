...
export function EnhancedMyCountryContent({ variant = 'unified', title }: EnhancedMyCountryContentProps) {
  ...
  const [activeGovernmentTab, setActiveGovernmentTab] = useState<'components' | 'budget' | 'taxes'>('components');
  const [selectedComponents, setSelectedComponents] = useState<ComponentType[]>([]);
  const { flagUrl } = useFlag(country?.name || '');

  // Fetch existing government components
  const { data: existingComponents, refetch: refetchComponents } = api.government.getComponents.useQuery(
        ...
        <Alert className="border-indigo-200 bg-gradient-to-r from-indigo-50 to-purple-50">
          ...
          <AlertDescription>
            <div className="flex items-center justify-between">
              <div>
                <strong>Atomic Government System Active:</strong> {existingComponents.length} components deployed with {
                  (existingComponents.reduce((sum, c) => sum + c.effectivenessScore, 0) / existingComponents.length).toFixed(0)
                  ...
                  {!vitalityCollapsed && (
                    <>
                      <Badge variant="outline" className="text-xs w-fit">
                        {existingComponents && existingComponents.length > 0 ? 'ATOMIC ENHANCED' : 'LIVE DATA'}
                      </Badge>
                    </>
                  )}
                </CardHeader>
        ...
        <div className={variant === 'unified' ? "xl:col-span-3" : "col-span-full"}>
          ...
          {(variant === 'premium' || variant === 'unified') && (
            <Card className="mb-6 glass-hierarchy-parent border-amber-200 dark:border-amber-700/40">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <CardTitle className="text-xl">Government Management</CardTitle>
                      <p className="text-sm text-muted-foreground">
                        Atomic component-based governance system
                      </p>
                    </div>
                  </div>
              ...
              <CardContent>
                <Tabs value={activeGovernmentTab} onValueChange={setActiveGovernmentTab as any} className="space-y-4">
                  ...
                  <TabsContent value="taxes" className="space-y-4">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      <Card>
                        <CardHeader>
                          <CardTitle className="text-lg">Tax System Builder</CardTitle>
                          <p className="text-sm text-muted-foreground">
                            Design and implement your nation's tax system
                          </p>
                        </CardHeader>
                        <CardContent>
                          <TaxBuilder
                      ...
                      <Card>
                        <CardHeader>
                          <CardTitle className="text-lg">Government Builder</CardTitle>
                          <p className="text-sm text-muted-foreground">
                            Structure your government departments and roles
                          </p>
                        </CardHeader>
                        <CardContent>
                          <GovernmentBuilder
