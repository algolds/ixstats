...
export function DiplomacyOverview({ countryId, onTabChange }: DiplomacyOverviewProps) {
  ...
  const avgStrength = totalRelationships > 0
    ? Math.round(relationships.reduce((sum, r) => sum + (r.strength ?? 0), 0) / totalRelationships)
    : 0;

  const diplomacyMetrics = [
    { label: "Embassies", value: activeEmbassies, icon: Building2, color: "text-cyan-600", tab: "embassies-relations" },
    { label: "Relations", value: totalRelationships, icon: Handshake, color: "text-blue-600", tab: "embassies-relations" },
    { label: "Alliances", value: allianceCount, icon: Users, color: "text-purple-600", tab: "embassies-relations" },
    { label: "Strong Ties", value: strongRelationships, icon: Sparkles, color: "text-emerald-600", tab: "embassies-relations" },
    { label: "Active Policies", value: activePolicies, icon: Scale, color: "text-amber-600", tab: "foreign-policy" },
    { label: "Avg Strength", value: `${avgStrength}%`, icon: Globe, color: "text-pink-600", tab: "embassies-relations" },
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
        {diplomacyMetrics.map((metric) => {
          ...
          return (
            <button
              key={metric.label}
              className="glass-hierarchy-child cursor-pointer rounded-lg p-2.5 text-left transition-colors hover:bg-muted/50"
      ...
      <div className="flex flex-wrap gap-2">
        <Button variant="outline" size="sm" className="gap-1.5" onClick={() => onTabChange?.("embassies-relations")}>
          <Building2 className="h-3.5 w-3.5" />
          Embassies & Relations
        </Button>
        <Button variant="outline" size="sm" className="gap-1.5" onClick={() => onTabChange?.("foreign-policy")}>
        ...
        <Card className="glass-hierarchy-child">
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" onClick={() => onTabChange?.("embassies-relations")}>
                View All
              </Button>
            </div>
          </CardHeader>
            ...
            {recentEmbassies.length > 0 ? (
              <div className="space-y-2">
                {recentEmbassies.map((embassy: any) => (
                  <div
                    key={embassy.id}
                    className="rounded-lg border border-border/40 bg-muted/40 p-2.5 transition-all hover:shadow-sm"
                  >
                    <div className="flex items-center justify-between">
