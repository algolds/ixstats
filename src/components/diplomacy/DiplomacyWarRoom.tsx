...
const ACTION_TYPE_BADGES: Record<string, { label: string; colorClass: string }> = {
  embargo: { label: "EMBARGO", colorClass: "bg-red-100 text-red-700 dark:bg-red-950/30 dark:text-red-400" },
  sanction: { label: "SANCTION", colorClass: "bg-orange-100 text-orange-700 dark:bg-orange-950/30 dark:text-orange-400" },
  blockade: { label: "BLOCKADE", colorClass: "bg-red-100 text-red-700 dark:bg-red-950/30 dark:text-red-400" },
  free_trade: { label: "TRADE", colorClass: "bg-green-100 text-green-700 dark:bg-green-950/30 dark:text-green-400" },
  military_alliance: { label: "ALLIANCE", colorClass: "bg-blue-100 text-blue-700 dark:bg-blue-950/30 dark:text-blue-400" },
};
...
export function DiplomacyWarRoom({ countryId }: DiplomacyWarRoomProps) {
  ...
  const embassyData = useMemo(() => {
    const active = embassies?.filter((e: any) => e.status === "ACTIVE" || e.status === "active") ?? [];
    const pending = embassies?.filter((e: any) => e.status === "PENDING" || e.status === "pending") ?? [];
    return { active, pending, all: embassies ?? [] };
  }, [embassies]);

  const relationData = useMemo(() => {
    const sorted = [...(relations ?? [])].sort((a: any, b: any) => (b.strength ?? 0) - (a.strength ?? 0));
    const avgStrength = sorted.length > 0
      ? Math.round(sorted.reduce((sum: number, r: any) => sum + (r.strength ?? 0), 0) / sorted.length)
      : 0;
  ...
  return (
    <>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4">
        ...
        <CommandPanel
          ...
          stats={[
            ...
            ...(embassyData.pending.length > 0 ? [{ label: "pending", value: embassyData.pending.length }] : []),
          ]}
          ctaLabel="Establish"
        ...
        >
          ...
          {embassyData.pending.slice(0, Math.max(0, 4 - embassyData.active.length)).map((embassy: any) => (
            <CommandPanelItem
              key={embassy.id}
              accentColor="amber"
              title={embassy.country ?? embassy.name ?? "Embassy"}
        ...
        <CommandPanel
          ...
          stats={[
            ...
            ...(relationData.avgStrength > 0 ? [{ label: `avg ${relationData.avgStrength}%`, value: "" }] : []),
          ]}
          footerLabel="View All"
        ...
        >
          {relationData.sorted.slice(0, 4).map((rel: any) => {
            ...
            const color = strength >= 70 ? "green" : strength >= 40 ? "blue" : strength >= 20 ? "amber" : "red";
            return (
              <CommandPanelItem
                key={rel.id}
                accentColor={color}
                ...
                trailingColor={strength >= 70 ? "text-green-600" : strength >= 40 ? "text-blue-600" : "text-amber-600"}
              />
            );
          })}
        </CommandPanel>
        ...
        >
          {activeFP.slice(0, 4).map((fp: any) => {
            const badge = ACTION_TYPE_BADGES[fp.actionType] ?? { label: fp.actionType?.toUpperCase() ?? "POLICY", colorClass: "bg-slate-100 text-slate-700" };
            const targetName = fp.target?.name ?? "Unknown";
            return (
              <CommandPanelItem
                key={fp.id}
                accentColor={fp.actionType === "free_trade" || fp.actionType === "military_alliance" ? "green" : "red"}
                title={targetName}
      ...
      {/* Drill-down Sheets */}
      <Sheet open={activeSheet === "embassies"} onOpenChange={(open) => !open && setActiveSheet(null)}>
        <SheetContent side="right" className="sm:max-w-lg w-full overflow-y-auto">
          <SheetHeader>
            <SheetTitle>Embassies & Relations</SheetTitle>
            <SheetDescription>Full embassy network, alliances, and cultural exchanges</SheetDescription>
          </SheetHeader>
          <div className="mt-4">
      ...
      <Sheet open={activeSheet === "relations"} onOpenChange={(open) => !open && setActiveSheet(null)}>
        <SheetContent side="right" className="sm:max-w-lg w-full overflow-y-auto">
          <SheetHeader>
      ...
      <Sheet open={activeSheet === "foreign-policy"} onOpenChange={(open) => !open && setActiveSheet(null)}>
        <SheetContent side="right" className="sm:max-w-lg w-full overflow-y-auto">
          <SheetHeader>
