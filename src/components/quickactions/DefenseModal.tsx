...
export function DefenseModal({ countryId, open, onOpenChange }: DefenseModalProps) {
  ...
  const getSecurityLevelColor = (level: string) => {
    switch (level) {
      ...
      case "moderate":
        return "border-yellow-300 dark:border-yellow-700 bg-yellow-50 dark:bg-yellow-950/30";
      case "high_risk":
        return "border-orange-300 dark:border-orange-700 bg-orange-50 dark:bg-orange-950/30";
      case "critical":
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-5xl overflow-y-auto">
        ...
        <Tabs defaultValue="overview" className="w-full">
          ...
          <TabsContent value="overview" className="mt-4 space-y-4">
            ...
            <div
              className={cn(
                ...
                securityData ? getSecurityLevelColor(securityData.securityLevel) : "border-border"
              )}
            >
              ...
              <div className="mb-4 grid grid-cols-2 gap-3 md:grid-cols-5">
                <div className="bg-muted/30 dark:bg-muted/10 rounded-lg p-3 text-center">
                  <Target className="mx-auto mb-1 h-4 w-4 text-blue-600 dark:text-blue-400" />
                  ...
                  <div className="text-muted-foreground text-xs">Military</div>
                </div>
                <div className="bg-muted/30 dark:bg-muted/10 rounded-lg p-3 text-center">
                  <Users className="mx-auto mb-1 h-4 w-4 text-green-600 dark:text-green-400" />
                  <div className="text-sm font-bold">
                    <NumberFlowDisplay
                      value={securityData?.internalStability?.stabilityScore ?? 0}
                      decimalPlaces={0}
                    />
                    %
                  </div>
                ...
                <div className="bg-muted/30 dark:bg-muted/10 rounded-lg p-3 text-center">
                  <MapPin className="mx-auto mb-1 h-4 w-4 text-purple-600 dark:text-purple-400" />
                  <div className="text-sm font-bold">
                    <NumberFlowDisplay
                      value={securityData?.borderSecurity?.overallSecurityLevel ?? 0}
                      decimalPlaces={0}
                    />
                    %
                  </div>
                  <div className="text-muted-foreground text-xs">Borders</div>
                </div>
                <div className="bg-muted/30 dark:bg-muted/10 rounded-lg p-3 text-center">
                  <Radio className="mx-auto mb-1 h-4 w-4 text-indigo-600 dark:text-indigo-400" />
                  <div className="text-sm font-bold">
                    <NumberFlowDisplay value={securityData?.cybersecurity ?? 0} decimalPlaces={0} />
                    %
                  </div>
                  <div className="text-muted-foreground text-xs">Cyber</div>
                </div>
                <div className="bg-muted/30 dark:bg-muted/10 rounded-lg p-3 text-center">
                  <AlertTriangle className="mx-auto mb-1 h-4 w-4 text-orange-600 dark:text-orange-400" />
                  <div className="text-sm font-bold">
                    <NumberFlowDisplay
                  ...
                  <div className="text-muted-foreground text-xs">Counter-Terror</div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-4 gap-3">
                ...
                <div className="text-center">
                  <div className="text-lg font-bold text-red-600 dark:text-red-400">
                    <NumberFlowDisplay
                      value={securityData?.highSeverityThreats ?? 0}
                      decimalPlaces={0}
                    />
                  </div>
                ...
                <div className="text-center">
                  ...
                  <div className="text-muted-foreground text-xs">Readiness</div>
                </div>
              </div>
            </div>
          </TabsContent>
              ...
              {militaryBranches && militaryBranches.length > 0 ? (
                <div className="space-y-3">
                  {militaryBranches.map((branch) => (
                    <div
                      key={branch.id}
                      className="bg-card hover:bg-muted/30 dark:hover:bg-muted/10 rounded-lg border p-3 transition-colors"
                    >
                      <div className="mb-2 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          {!["navy", "air_force", "army"].includes(branch.branchType) && (
          ...
          {/* Stability Tab */}
          <TabsContent value="stability" className="mt-4 space-y-4">
            <div className="space-y-3">
              <div className="mb-2 flex items-center gap-2">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-muted/30 dark:bg-muted/10 rounded-lg p-3">
                      <div className="text-muted-foreground mb-1 text-xs">Stability Score</div>
                      <div className="text-2xl font-bold">
                        <NumberFlowDisplay
                          value={stabilityData.metrics.stabilityScore}
                          decimalPlaces={0}
                        />
                        %
                      </div>
                    ...
                    <div className="bg-muted/30 dark:bg-muted/10 rounded-lg p-3">
                      <div className="text-muted-foreground mb-1 text-xs">Crime Rate</div>
                      <div className="text-2xl font-bold text-red-600 dark:text-red-400">
                        <NumberFlowDisplay
                          value={stabilityData.metrics.crimeRate}
                          decimalPlaces={1}
                        />
                      </div>
                    </div>
                    <div className="bg-muted/30 dark:bg-muted/10 rounded-lg p-3">
                      <div className="text-muted-foreground mb-1 text-xs">Social Cohesion</div>
                      <div className="text-2xl font-bold text-green-600 dark:text-green-400">
                        <NumberFlowDisplay
                          value={stabilityData.metrics.socialCohesion}
                          decimalPlaces={0}
                        />
                        %
                      </div>
                    ...
                    <div className="bg-muted/30 dark:bg-muted/10 rounded-lg p-3">
                      <div className="text-muted-foreground mb-1 text-xs">Public Order</div>
                      <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">
                        <NumberFlowDisplay
                          value={stabilityData.metrics.policingEffectiveness}
                          decimalPlaces={0}
                        />
                        %
                      </div>
                  ...
                  <div className="space-y-2">
                    <h4 className="text-sm font-medium">Recent Stability Events</h4>
                    {stabilityData.activeEvents && stabilityData.activeEvents.length > 0 ? (
                      <div className="space-y-2">
                        {stabilityData.activeEvents.slice(0, 3).map((event) => (
                          <div key={event.id} className="bg-card rounded-lg border p-2 text-xs">
                            <div className="mb-1 flex items-center justify-between">
                              <Badge variant="outline" className="text-[10px]">
                                {event.eventType}
                              </Badge>
                              <span className="text-muted-foreground">
                                Impact: {event.stabilityImpact}
                              </span>
                            </div>
                      ...
                      <p className="text-muted-foreground bg-muted/30 dark:bg-muted/10 rounded-lg py-3 text-center text-xs">
                        No recent stability events
                      </p>
                    )}
                  </div>
                </div>
          ...
          <TabsContent value="threats" className="mt-4 space-y-4">
            <div className="space-y-3">
              ...
              {threats && threats.length > 0 ? (
                      ...
                      <div className="mb-2 flex items-start justify-between">
                        <div className="flex-1">
                          <div className="mb-1 flex items-center gap-2">
                            <Badge
                              className={cn("text-white", getThreatSeverityColor(threat.severity))}
                            >
                ...
                <div className="text-muted-foreground py-8 text-center">
                  <Shield className="mx-auto mb-3 h-12 w-12 text-green-600 opacity-50 dark:text-green-400" />
                  <p className="text-sm font-medium text-green-600 dark:text-green-400">
                    No active threats detected
                  </p>
                  <p className="text-muted-foreground text-xs">Security status: All clear</p>
                </div>
              )}
            </div>
          </TabsContent>
