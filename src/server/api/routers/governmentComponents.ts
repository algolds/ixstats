  ...
  getComponentUsageStats: adminProcedure
    .query(async ({ ctx }) => {
      try {
        ...
        // Least used (0 usage)
        const leastUsed = components
          .filter(comp => !usageMap.has(comp.type))
          .slice(0, 10);
        ...
        // Synergy stats
        const allSynergies = components.reduce((acc, comp) =>
          acc + comp.synergies.length, 0
        );
        const allConflicts = components.reduce((acc, comp) =>
          acc + comp.conflicts.length, 0
        );
        ...
        return {
          ...
          summary: {
            ...
            avgUsage: Array.from(usageMap.values()).reduce((sum, count) => sum + count, 0) / totalComponents,
          },
          topComponents,
          leastUsed,
          categoryStats,
