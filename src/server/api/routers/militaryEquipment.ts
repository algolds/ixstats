...
export const militaryEquipmentRouter = createTRPCRouter({
  ...
  getCatalogEquipment: publicProcedure
    .input(z.object({
      category: z.enum(['infantry', 'vehicle', 'aircraft', 'naval', 'missile', 'support']).optional(),
  ...
  createCatalogEquipment: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        console.log(`[MILITARY_EQUIPMENT] Admin ${ctx.auth.userId} created equipment: ${equipment.name} (${equipment.id})`);
  ...
  updateCatalogEquipment: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        if (input.specifications !== undefined) updateData.specifications = JSON.stringify(input.specifications);
        ...
        console.log(`[MILITARY_EQUIPMENT] Admin ${ctx.auth.userId} updated equipment: ${equipment.name} (${equipment.id})`);
  ...
  deleteCatalogEquipment: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        console.log(`[MILITARY_EQUIPMENT] Admin ${ctx.auth.userId} deleted equipment: ${equipment.name} (${equipment.id})`);
  ...
  bulkToggleEquipment: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        console.log(`[MILITARY_EQUIPMENT] Admin ${ctx.auth.userId} bulk toggled ${result.count} equipment items to ${input.isActive ? 'active' : 'inactive'}`);
  ...
  createManufacturer: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        console.log(`[MILITARY_EQUIPMENT] Admin ${ctx.auth.userId} created manufacturer: ${manufacturer.name} (${manufacturer.id})`);
  ...
  updateManufacturer: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        console.log(`[MILITARY_EQUIPMENT] Admin ${ctx.auth.userId} updated manufacturer: ${manufacturer.name} (${manufacturer.id})`);
   ...
   */
  getEquipmentUsageStats: publicProcedure
    .query(async ({ ctx }) => {
      try {
        ...
        const manufacturerUsage = manufacturerStats.map(mfr => ({
   ...
   */
  getManufacturerStats: publicProcedure
    .query(async ({ ctx }) => {
      try {
        const manufacturers = await ctx.db.militaryManufacturer.findMany({
          where: { isActive: true },
        ...
        const stats = manufacturers.map(mfr => {
          ...
          const avgTechLevel = mfr.equipment.length > 0
            ? mfr.equipment.reduce((sum, eq) => sum + eq.technologyTier, 0) / mfr.equipment.length
            : 0;

          // Category distribution
          const categoryDistribution = mfr.equipment.reduce((acc, eq) => {
        ...
        stats.forEach(s => {
          s.marketShare = totalMarketUsage > 0
            ? Math.round((s.totalUsage / totalMarketUsage) * 1000) / 10
            : 0;
        });
