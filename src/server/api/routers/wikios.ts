...
export const wikiosRouter = createTRPCRouter({
  ...
  // ---------------------------------------------------------------------------
  // Backward compatibility (old watchlist endpoints → Lore Stash)
  // ---------------------------------------------------------------------------

  /** @deprecated Use isStashed instead. */
  isWatching: protectedProcedure
    .input(z.object({ title: z.string().min(1).max(500) }))
    .query(async ({ input, ctx }) => {
      const items = await db.loreStashItem.findMany({
        where: { pageTitle: input.title, stash: { userId: ctx.auth.userId } },
        take: 1,
      });
      return { watching: items.length > 0 };
    }),

  /** @deprecated Use stashPage instead. */
  watchPage: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const pageSlug = encodeURIComponent(input.title.replace(/ /g, "_"));
      await db.loreStashItem.upsert({
        where: { stashId_pageTitle: { stashId: defaultStash.id, pageTitle: input.title } },
        create: { stashId: defaultStash.id, pageTitle: input.title, pageSlug },
        update: {},
      });
      return { success: true };
    }),

  /** @deprecated Use unstashPage instead. */
  unwatchPage: protectedProcedure
    .input(z.object({ title: z.string().min(1).max(500) }))
    .mutation(async ({ input, ctx }) => {
      const stashIds = (await db.loreStash.findMany({ where: { userId: ctx.auth.userId }, select: { id: true } })).map((s) => s.id);
      if (stashIds.length > 0) {
        await db.loreStashItem.deleteMany({ where: { stashId: { in: stashIds }, pageTitle: input.title } });
      }
      return { success: true };
    }),

  /** @deprecated Retained for watchlist page compatibility. */
  getWatchlist: protectedProcedure
    .input(z.object({ limit: z.number().min(1).max(200).default(50) }))
    .query(async ({ input, ctx }) => {
      const allItems = await db.loreStashItem.findMany({
        where: { stash: { userId: ctx.auth.userId } },
        orderBy: { savedAt: "desc" },
      });
      if (allItems.length === 0) return { items: [], totalWatched: 0 };

      const apiBase = process.env.WIKIOS_MEDIAWIKI_API ?? "https://ixwiki.com/api.php";
      const params = new URLSearchParams({
        action: "query", list: "recentchanges",
        rclimit: String(input.limit), rcprop: "title|timestamp|user|comment|sizes|flags|ids",
        rctype: "edit|new", formatversion: "2", format: "json",
      });
      const res = await fetch(`${apiBase}?${params}`, { signal: AbortSignal.timeout(10000) });
      const data = (await res.json()) as {
        query?: { recentchanges?: Array<{ title: string; timestamp: string; user: string; comment?: string; oldlen: number; newlen: number; minor?: boolean; revid: number; old_revid: number; type: string }> };
      };
      const stashedTitles = new Set(allItems.map((i) => i.pageTitle));
      const changes = (data.query?.recentchanges ?? [])
        .filter((rc) => stashedTitles.has(rc.title))
        .map((rc) => ({
          title: rc.title, timestamp: rc.timestamp, user: rc.user, comment: rc.comment ?? "",
          byteDiff: rc.newlen - rc.oldlen, minor: rc.minor ?? false, revid: rc.revid,
          oldRevid: rc.old_revid, type: rc.type, isNew: true,
        }));
      return { items: changes, totalWatched: allItems.length };
    }),

  /** @deprecated Retained for watchlist compatibility. */
  markWatchlistSeen: protectedProcedure
    .mutation(async () => {
      return { success: true };
    }),
