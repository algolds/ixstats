import { z } from "zod";
import { createTRPCRouter, publicProcedure, protectedProcedure, rateLimitedPublicProcedure, adminProcedure } from "~/server/api/trpc";
import { TRPCError } from "@trpc/server";
import { IxTime } from "~/lib/ixtime";
import { notificationAPI } from "~/lib/notification-api";
import { notificationHooks } from "~/lib/notification-hooks";
import { DiplomaticChoiceTracker } from "~/lib/diplomatic-choice-tracker";
import { calculateCulturalCompatibility, type CountryBasicInfo, type DiplomaticRelationship, type EmbassyConnection } from "~/lib/cultural-compatibility";
import { NPCCulturalParticipation, type NPCParticipationContext } from "~/lib/npc-cultural-participation";
import { NPCPersonalitySystem, type ObservableData } from "~/lib/diplomatic-npc-personality";
import { STRATEGIC_PRIORITIES, PARTNERSHIP_GOALS, KEY_ACHIEVEMENTS } from "~/lib/diplomatic-profile-options";
...
function calculateMissionCulturalBonus(completedMissionCount: number) {
  ...
  return {
    ...
    reasoning: completedMissionCount > 0
...
export const diplomaticRouter = createTRPCRouter({
    ...
    .query(async ({ ctx, input }) => {
      try {
        ...
        return exchanges.map(exchange => {
          ...
          const completedMissions = exchange.embassyMissions.filter(m => m.status === 'completed');
          ...
          const boostedCulturalImpact = baseCulturalImpact + (baseCulturalImpact * missionBonus / 100);
          ...
          const boostedDiplomaticValue = baseDiplomaticValue + (baseDiplomaticValue * diplomaticBonus / 100);

          const bonusReasoning = [];
          if (completedMissions.length > 0) {
            bonusReasoning.push(`+${missionBonus}% cultural impact from ${completedMissions.length} completed embassy mission${completedMissions.length > 1 ? 's' : ''}`);
            bonusReasoning.push(`+${diplomaticBonus}% diplomatic value from embassy mission support`);
          }
          if (activeMissions.length > 0) {
            bonusReasoning.push(`${activeMissions.length} active embassy mission${activeMissions.length > 1 ? 's' : ''} providing coordination support`);
          }
  ...
  createCulturalExchange: protectedProcedure
    .input(z.object({
      ...
      type: z.enum([
        ...
        'music', 'film', 'environmental', 'science', 'trade', 'humanitarian', 'agriculture', 'heritage', 'youth'
      ]),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.embassyMissionId) {
        ...
        if (mission.type !== 'cultural_outreach') {
          throw new TRPCError({ code: 'BAD_REQUEST', message: 'Can only link to cultural_outreach missions' });
        }

        if (mission.embassy.guestCountryId !== input.hostCountryId) {
          throw new TRPCError({ code: 'BAD_REQUEST', message: 'Mission must belong to your country\'s embassy' });
        }
      ...
      if (input.autoCreateMissions) {
        try {
          ...
          const missionCreationPromises = embassies.map(async (embassy) => {
            ...
            const duration = Math.ceil((new Date(input.endDate).getTime() - new Date(input.startDate).getTime()) / (1000 * 60 * 60 * 24)); // Duration in days
            const completesAt = new Date(new Date(input.startDate).getTime() + duration * 24 * 60 * 60 * 1000);
          ...
          console.error('[Diplomatic] Failed to auto-create embassy missions for cultural exchange:', error);
  ...
  linkExchangeToMission: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!ctx.user?.countryId || ctx.user.countryId !== exchange.hostCountryId) {
        throw new TRPCError({ code: 'FORBIDDEN', message: 'You can only link your own country\'s cultural exchanges' });
      }
  ...
  getInfluenceLeaderboard: rateLimitedPublicProcedure
    .query(async ({ ctx }) => {
      ...
      const leaderboard = countries.map(country => {
        const activeEmbassies = country.embassiesGuest.filter((e: { status: string }) => e.status === 'ACTIVE');
        const totalInfluence = activeEmbassies.reduce((sum: number, e: { influence: number | null }) => sum + (e.influence || 0), 0);
        const averageLevel = activeEmbassies.length > 0 ?
          activeEmbassies.reduce((sum: number, e: { level: number | null }) => sum + (e.level || 1), 0) / activeEmbassies.length : 0;
  ...
  getSharedData: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      try {
        ...
        const embassyAge = Math.floor((Date.now() - embassy.establishedAt.getTime()) / (1000 * 60 * 60 * 24)); // Days since established
        ...
        if (!input.dataType || input.dataType === 'economic') {
          ...
          const missionTradeBonus = completedTradeMissions.reduce((sum, mission) => sum + (mission.economicReward || 0), 0);
          ...
          try {
            ...
            if (historicalData.length >= 2) {
              ...
              const avgGdpGrowth = historicalData.reduce((sum, point) => sum + point.gdpGrowthRate, 0) / historicalData.length;
          ...
          const economicBenefit = Math.min(50,
            embassyLevel * 3.5 +
            embassyInfluence / 10 +
        ...
        if (!input.dataType || input.dataType === 'intelligence') {
          ...
          templates.forEach((template, index) => {
            ...
            const confidenceMultiplier = template.reportType === 'economic' ? 5 :
                                        template.reportType === 'political' ? 4 : 3;
            const confidence = Math.floor(template.confidenceBase + embassyLevel * confidenceMultiplier);
        ...
        if (!input.dataType || input.dataType === 'research') {
          ...
          const completedResearchCount = researchMissions.filter(m => m.status === 'completed').length;
          ...
          if (embassyLevel >= 2) {
            researchProjects.push({
              ...
              publications: Math.floor((totalPublications * 0.6) + (embassyLevel * 2 + embassyAge / 30)), // 60% from missions, 40% from time
          ...
          if (embassyLevel >= 3) {
            researchProjects.push({
              ...
              publications: Math.floor((totalPublications * 0.4) + (embassyLevel * 1.5 + embassyAge / 40)), // 40% from missions, 60% from time
        ...
        if (!input.dataType || input.dataType === 'cultural') {
          ...
          const completedExchanges = culturalExchanges.filter(e => e.status === 'completed').length;
          const activeExchanges = culturalExchanges.filter(e => e.status === 'active').length;
          const completedCulturalMissions = culturalMissions.filter(m => m.status === 'completed').length;
          ...
          const avgCulturalImpact = culturalExchanges.length > 0 ? totalCulturalImpact / culturalExchanges.length : 0;
          ...
          const avgDiplomaticValue = culturalExchanges.length > 0 ? totalDiplomaticValue / culturalExchanges.length : 0;

          sharedData.cultural = {
            exchangePrograms: completedExchanges + Math.floor(embassyLevel * 2 + embassyAge / 30), // Real + deterministic
            culturalEvents: activeExchanges + completedExchanges + completedCulturalMissions + Math.floor(embassyLevel * 3 + embassyAge / 15), // Real + deterministic
            artistsExchanged: Math.floor(completedExchanges * 3 + embassyLevel * 4 + embassyAge / 20), // Based on real exchanges + deterministic
            studentsExchanged: Math.floor(completedExchanges * 8 + embassyLevel * 10 + embassyAge / 10), // Based on real exchanges + deterministic
            culturalImpactScore: Math.min(100, Math.floor(avgCulturalImpact + embassyLevel * 12 + embassyInfluence + embassyAge / 5)), // Real impact + deterministic
            diplomaticGoodwill: Math.min(100, Math.floor(avgDiplomaticValue + embassyLevel * 15 + embassyInfluence * 0.8)) // Real value + deterministic
          };
        }
        ...
        if (!input.dataType || input.dataType === 'policy') {
          ...
          const activeTreaties = allTreaties.filter(t =>
            t.status === 'ratified' || t.status === 'active' || t.status === 'RATIFIED' || t.status === 'ACTIVE'
          );
          ...
          const policyFrameworks = activeTreaties.map(treaty => {
            ...
            } else if (treaty.type.toLowerCase().includes('memorandum') || treaty.type.toLowerCase().includes('mou')) {
            ...
            if (keyProvisions.length === 0) {
              if (treaty.type.toLowerCase().includes('trade')) {
                keyProvisions.push('Reduced tariffs on key exports', 'Streamlined customs procedures', 'Investment protection guarantees');
              } else if (treaty.type.toLowerCase().includes('cultural')) {
                keyProvisions.push('Annual cultural festivals', 'Student exchange programs', 'Artist residency initiatives');
              } else if (treaty.type.toLowerCase().includes('science') || treaty.type.toLowerCase().includes('research')) {
                keyProvisions.push('Joint research initiatives', 'Technology transfer agreements', 'Shared research facilities');
              } else {
                keyProvisions.push('Mutual cooperation', 'Regular consultations', 'Information sharing');
              }
            }
          ...
          if (policyFrameworks.length === 0) {
            if (embassyLevel >= 1) {
              policyFrameworks.push({
                ...
                status: 'ratified',
                effectiveDate: new Date(embassy.establishedAt.getTime() + 30 * 24 * 60 * 60 * 1000).toISOString(),
                keyProvisions: [
            ...
            if (embassyLevel >= 2) {
              policyFrameworks.push({
                ...
                agreementType: 'framework',
                status: 'ratified',
                effectiveDate: new Date(embassy.establishedAt.getTime() + 60 * 24 * 60 * 60 * 1000).toISOString(),
                keyProvisions: [
            ...
            if (embassyLevel >= 3) {
              policyFrameworks.push({
                ...
                effectiveDate: embassyAge > 90 ? new Date(embassy.establishedAt.getTime() + 90 * 24 * 60 * 60 * 1000).toISOString() : 'pending',
                keyProvisions: [
  ...
  updateEmbassyProfile: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.strategicPriorities !== undefined) updateData.strategicPriorities = input.strategicPriorities;
      if (input.partnershipGoals !== undefined) updateData.partnershipGoals = input.partnershipGoals;
        ...
        if (input[field] !== undefined) {
          try {
            ...
            if (removedOptions.length > 0) {
              ...
              const usageRecords = embassy.optionUsage.filter(
                usage => removedOptions.includes(usage.optionId)
              );
  ...
  generateCulturalScenario: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const scenarioContext = {
        ...
        relationshipState: relationship.status === 'alliance' ? 'allied' as const :
                          relationship.status === 'tension' ? 'tense' as const :
                          'neutral' as const,
      ...
      const { CulturalScenarioGenerator, CULTURAL_SCENARIO_TEMPLATES } = await import('~/lib/cultural-scenario-generator');

      const template = input.preferredScenarioType && CULTURAL_SCENARIO_TEMPLATES[input.preferredScenarioType as keyof typeof CULTURAL_SCENARIO_TEMPLATES]
        ? CULTURAL_SCENARIO_TEMPLATES[input.preferredScenarioType as keyof typeof CULTURAL_SCENARIO_TEMPLATES]
    ...
    .query(async ({ ctx, input }) => {
      ...
      const participationContext = {
        ...
        exchangeDetails: {
          ...
          duration: Math.ceil((exchange.endDate.getTime() - exchange.startDate.getTime()) / (1000 * 60 * 60 * 24))
        },
        existingExchanges: 0,
  ...
  calculateExchangeImpact: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const currentRelationship = {
        state: (relationship?.status === 'alliance' ? 'allied' :
               relationship?.status === 'tension' ? 'tense' :
               'neutral') as any,
        strength: relationship?.strength || 50,
    ...
    .query(async ({ ctx, input }) => {
      try {
        ...
        const compatibilityResults = allCountries.map(targetCountry => {
          const relation = diplomaticRelations.find(r =>
            (r.country1 === targetCountry.id || r.country2 === targetCountry.id)
          );

          const hasEmbassy = embassies.some(e =>
          ...
          if (relationship) {
            ...
            await ctx.db.diplomaticRelation.update({
              ...
              data: {
                ...
                culturalExchange: relationship.culturalExchange === 'High' ? 'Medium' : relationship.culturalExchange,
                updatedAt: new Date()
              }
            });
          }
          ...
          participants.map(async (participant) => {
            try {
              ...
              const relationships = await ctx.db.diplomaticRelation.findMany({
                where: {
                  OR: [
                    { country1: participant.countryId },
                    { country2: participant.countryId }
                  ]
                }
              });
              ...
              const observableData: ObservableData = {
                relationships: {
                  ...
                  allied: relationships.filter((r: { relationship: string }) => r.relationship === 'alliance').length,
                  friendly: relationships.filter((r: { relationship: string }) => r.relationship === 'friendly' || r.relationship === 'cooperative').length,
                  tense: relationships.filter((r: { relationship: string }) => r.relationship === 'cool' || r.relationship === 'strained').length,
                  hostile: relationships.filter((r: { relationship: string }) => r.relationship === 'hostile').length,
                  neutral: relationships.filter((r: { relationship: string }) => r.relationship === 'neutral').length,
                ...
                embassies: {
                  ...
                  culturalSpecialized: embassies.filter((e: { specialization: string | null }) => e.specialization === 'cultural').length,
                  economicSpecialized: embassies.filter((e: { specialization: string | null }) => e.specialization === 'economic').length,
                  securitySpecialized: embassies.filter((e: { specialization: string | null }) => e.specialization === 'security').length,
                  averageLevel: embassies.length > 0
                    ? embassies.reduce((sum: number, e: { level: number }) => sum + e.level, 0) / embassies.length
                    : 1,
                  averageInfluence: embassies.length > 0
                    ? embassies.reduce((sum: number, e: { influence: number }) => sum + e.influence, 0) / embassies.length
                    : 50
                },
                treaties: {
                ...
                cultural: {
                  highExchangeCount: relationships.filter((r: { culturalExchange: string | null }) => r.culturalExchange === 'High').length,
                  mediumExchangeCount: relationships.filter((r: { culturalExchange: string | null }) => r.culturalExchange === 'Medium').length,
                ...
                historical: {
                  ...
                  cooperativeActions: relationships.filter((r: { relationship: string }) =>
                    r.relationship === 'alliance' || r.relationship === 'friendly'
                  ).length,
                  aggressiveActions: relationships.filter((r: { relationship: string }) =>
              ...
              const relationshipWithHost = relationships.find((r: any) =>
              ...
              const participationContext: NPCParticipationContext = {
                ...
                existingExchanges: observableData.cultural.highExchangeCount +
   ...
   */
  getAllDiplomaticOptions: publicProcedure
    .query(async ({ ctx }) => {
      try {
   ...
   */
  getOptionUsageStats: adminProcedure
    .query(async ({ ctx }) => {
      try {
        ...
        const categoryStats = optionsWithStats.reduce((acc, option) => {
        ...
        // Get usage by type
        const typeStats = optionsWithStats.reduce((acc, option) => {
          if (!acc[option.type]) {
        ...
        // Group by day
        const usageByDay = recentUsage.reduce((acc, usage) => {
...
function determineCategoryFromValue(value: string): string {
  ...
  if (lowerValue.includes('economic') || lowerValue.includes('trade') ||
      lowerValue.includes('investment') || lowerValue.includes('market') ||
      lowerValue.includes('financial') || lowerValue.includes('agricultural')) {
    return 'Economic';
  }

  // Military & Security keywords
  if (lowerValue.includes('military') || lowerValue.includes('defense') ||
      lowerValue.includes('security') || lowerValue.includes('intelligence') ||
      lowerValue.includes('counter-terrorism') || lowerValue.includes('border') ||
      lowerValue.includes('maritime') || lowerValue.includes('cybersecurity')) {
    return 'Military';
  }

  // Technology & Innovation keywords
  if (lowerValue.includes('technology') || lowerValue.includes('research') ||
      lowerValue.includes('innovation') || lowerValue.includes('digital') ||
      lowerValue.includes('space') || lowerValue.includes('artificial intelligence') ||
      lowerValue.includes('telecommunications') || lowerValue.includes('ai ')) {
    return 'Technology';
  }

  // Cultural & Social keywords
  if (lowerValue.includes('cultural') || lowerValue.includes('educational') ||
      lowerValue.includes('scientific') || lowerValue.includes('healthcare') ||
      lowerValue.includes('sports') || lowerValue.includes('media') ||
      lowerValue.includes('student') || lowerValue.includes('scholar') ||
      lowerValue.includes('artist') || lowerValue.includes('festival') ||
      lowerValue.includes('language') || lowerValue.includes('heritage')) {
    return 'Cultural';
  }

  // Environmental & Energy keywords
  if (lowerValue.includes('climate') || lowerValue.includes('energy') ||
      lowerValue.includes('renewable') || lowerValue.includes('environmental') ||
      lowerValue.includes('sustainable') || lowerValue.includes('water') ||
      lowerValue.includes('conservation') || lowerValue.includes('emission') ||
      lowerValue.includes('circular economy')) {
    return 'Environmental';
  }

  // Diplomatic & Political keywords
  if (lowerValue.includes('diplomatic') || lowerValue.includes('regional') ||
      lowerValue.includes('humanitarian') || lowerValue.includes('conflict') ||
      lowerValue.includes('democratic') || lowerValue.includes('governance') ||
      lowerValue.includes('embassy') || lowerValue.includes('consulate') ||
      lowerValue.includes('ambassadorial') || lowerValue.includes('state visit') ||
      lowerValue.includes('summit') || lowerValue.includes('partnership') ||
      lowerValue.includes('crisis') || lowerValue.includes('mediation') ||
