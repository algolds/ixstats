...
export const wikiRouter = createTRPCRouter({
  ...
  resolveWikiPlaceholders: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      let userCountryId = input.activeCountryId;
      ...
      for (const p of input.placeholders) {
        if (p.startsWith("CountryData:")) {
          const parts = p.split(":");
          if (parts[1]) countryNames.add(parts[1]);
        } else if (p.startsWith("BusinessData:")) {
          const parts = p.split(":");
          if (parts[1]) companyNames.add(parts[1]);
        }
      }
      ...
      for (const p of input.placeholders) {
        if (p.startsWith("MyCountry:")) {
          ...
          const poi = pois.find((poi) => poi.name.toLowerCase() === companyName.toLowerCase());
          if (!poi) {
