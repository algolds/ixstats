...
import { 
  OptimizedCountryQueries, 
  queryMonitor
} from "~/lib/database-optimizations";
import { 
  CacheDecorators, 
  CacheUtils, 
  globalCache 
} from "~/lib/advanced-cache-system";
import { 
  ResponseOptimizer,
  MemoryOptimizer,
  PerformanceMonitor 
  ...
  getAll: publicProcedure
    ...
    .query(async ({ input = {} }) => {
      ...
      try {
        ...
        if (cached) {
          PerformanceMonitor.recordMetric('countries.getAll.cache_hit', performance.now() - startTime);
          return cached;
        }
        ...
        const optimizedCountries = countries.countries.map((country: any) => 
          MemoryOptimizer.optimizeObject(country, [
            'id', 'name', 'slug', 'continent', 'region', 
            'economicTier', 'populationTier', 'currentPopulation', 
            'currentTotalGdp', 'flag', 'governmentType', 'leader'
          ])
        );
    ...
    .query(async ({ input }) => {
      ...
      try {
        ...
        const countries = await CacheUtils.batchCache(
          ids,
          async (missingIds) => {
            const results = await OptimizedCountryQueries.getCountriesByIds(
              missingIds,
              { 
            ...
            return Object.fromEntries(
              results.map(country => [country.id, country])
            );
          },
        ...
        PerformanceMonitor.recordMetric('countries.getMultiple.error', performance.now() - startTime);
  ...
  getGlobalStats: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      try {
        ...
        PerformanceMonitor.recordMetric('countries.getGlobalStats.error', performance.now() - startTime);
