...
export const sdiRouter = createTRPCRouter({
  ...
  getIntelligenceFeed: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const data: IntelligenceItem[] = rawData.map(
        (item): IntelligenceItem => ({
          ...
          affectedCountries: item.affectedCountries
            ? JSON.parse(item.affectedCountries)
            : undefined,
  ...
  getEconomicIndicators: publicProcedure.query(async ({ ctx }): Promise<EconomicIndicator> => {
    ...
    console.log(
      "DEBUG (sdi.ts): Before globalGrowth calculation - totalGrowth:",
      totalGrowth,
      "count:",
      count
    );
  ...
  getSystemStatus: publicProcedure.query(async ({ ctx }) => {
    ...
    return {
      ...
      activeUsers: Math.min(
        25,
        Math.max(5, crisisCount * 2 + intelligenceCount + Math.floor(systemHealthScore / 10))
      ), // Users based on system activity
      ...
      const newItem = await ctx.db.intelligenceItem.create({
        data: {
          ...
          affectedCountries: input.affectedCountries
            ? JSON.stringify(input.affectedCountries)
            : undefined,
          timestamp: new Date(),
          isActive: true,
        },
  ...
  updateCrisisEvent: protectedProcedure
    .input(
      z.object({
        ...
        responseStatus: z
          .enum(["coordinating", "monitoring", "deployed", "standby", "resolved"])
          .optional(),
  ...
  updateDiplomaticRelation: protectedProcedure
    .input(
      z.object({
        id: z.string(),
        relationship: z
          .enum(["alliance", "neutral", "tension", "conflict", "partnership"])
          .optional(),
