  ...
  return {
      ...
      health: {
        ...
        invalidationStats: {
          ...
          averageInvalidationTime:
            Math.round(cacheHealth.invalidationStats.averageInvalidationTime * 100) / 100,
    ...
    realtime: {
      ...
      invalidation: {
        ...
        efficiency:
          Math.round(
            (cacheHealth.invalidationStats.totalInvalidations > 0
              ? 1000 / cacheHealth.invalidationStats.averageInvalidationTime
              : 100) * 100
          ) / 100,
      },
    },
...
function generateRecommendations(
  ...
  if (queryStats.queries.errorRate > 0.05) {
    highPriority.push(
      "High query error rate detected - review error handling and database connections"
    );
  }
  ...
  if (cacheStats.hitRate < 70) {
    mediumPriority.push(
      "Low cache hit rate - consider increasing cache TTL or implementing pre-loading"
    );
  }
