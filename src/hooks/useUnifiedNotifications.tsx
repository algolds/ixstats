...
export function UnifiedNotificationProvider({
  ...
  const [currentIslandNotification, setCurrentIslandNotification] =
    useState<UnifiedNotification | null>(null);
  const [preferences, setPreferences] =
    useState<UserNotificationPreferences>(DEFAULT_USER_PREFERENCES);
  ...
  const loadUserPreferences = async () => {
    ...
    if (stored) {
      try {
        ...
        orchestrator.setUserPreferences(effectiveUserId, {
...
export function useDataNotifications() {
  ...
  const createEconomicAlert = useCallback(
    async (data: { metric: string; value: number; change: number; threshold?: number }) => {
  ...
  const createAchievementNotification = useCallback(
    async (achievement: { title: string; description: string; category: string }) => {
      return createNotification({
        source: "system",
