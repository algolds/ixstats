...
export function useAchievementNotifications(config: AchievementNotificationConfig) {
  ...
  const { isConnected, recentEvents, actions } = useAchievementUpdates(
    countryId,
    enableRealTime
  );
    ...
    recentEvents.forEach((update: LiveIntelligenceUpdate) => {
      if (update.type === 'achievement_notification' && update.event.type === 'achievement_unlocked') {
        ...
        const achievement: AchievementNotification = {
          ...
          description: achievementData.achievementDescription || 'You have unlocked a new achievement!',
        ...
        if (enableNotificationCenter) {
          ...
          void createNotificationMutation.mutateAsync({
            title: `🏆 ${achievement.title}`,
            message: achievement.description,
            category: 'achievement',
            type: 'success',
  ...
  const unlockAchievement = useCallback((achievement: Omit<AchievementNotification, 'id' | 'unlockedAt'>) => {
    ...
    if (enableNotificationCenter) {
      void globalNotificationBridge.notify({
        ...
        priority: fullAchievement.tier === 'legendary' ? 'high' : fullAchievement.tier === 'platinum' ? 'medium' : 'low',
        category: 'achievement',
        deliveryMethod: enableDynamicIsland ? 'dynamic-island' : 'toast',
        metadata: {
