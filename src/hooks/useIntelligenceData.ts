...
export function useIntelligenceData({
        ...
        if (typeof mainPageWikitext === "string" && mainPageWikitext.length > 100) {
          ...
          if (infoboxMatch) {
            ...
            if (infoboxEnd > infoboxStart) {
              cleanContent =
                cleanContent.substring(0, infoboxStart) + cleanContent.substring(infoboxEnd);
            }
          }
  ...
  const vitalityMetrics = useMemo<VitalityMetric[]>(() => {
    ...
    const populationGrowth = Math.min(
      100,
      Math.max(0, (country.populationGrowthRate * 100 + 2) * 25)
    );
    ...
    return [
      {
        ...
        trend:
          country.adjustedGdpGrowth > 0.02
            ? "up"
  ...
  const countryInformation = useMemo<CountryInformation[]>(() => {
    ...
    if (country.continent || country.region) {
      info.push({
        ...
        items: [
          ...(country.continent
            ? [{ label: "Continent", value: country.continent, classification: "PUBLIC" as const }]
            : []),
          ...(country.region
            ? [{ label: "Region", value: country.region, classification: "PUBLIC" as const }]
            : []),
          ...(country.capital
            ? [{ label: "Capital", value: country.capital, classification: "PUBLIC" as const }]
            : []),
        ],
      });
    }
      ...
      info.push({
        ...
        items: [
          ...(country.governmentType
            ...
            : []),
          ...(country.leader
            ? [{ label: "Leader", value: country.leader, classification: "PUBLIC" as const }]
            : []),
        ],
      });
    }
    ...
    if (country.religion) {
      info.push({
        ...
        icon: RiGlobalLine,
        items: [{ label: "Primary Religion", value: country.religion, classification: "PUBLIC" }],
      });
    }
  ...
  // Save wiki overview
  const saveWikiOverview = useCallback(
    (content: string) => {
      setLocalWikiData({
        countryName: country.name,
        infobox: undefined,
