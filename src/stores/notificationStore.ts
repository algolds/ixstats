  ...
  subscribeWithSelector((set, get) => ({
    ...
    addNotification: async (notificationData) => {
      try {
        ...
        set({
          error: `Failed to add notification: ${error instanceof Error ? error.message : String(error)}`,
        });
      }
    },
    ...
    cleanup: () => {
      ...
      localStorage.setItem("notification-preferences", JSON.stringify(get().userPreferences));
