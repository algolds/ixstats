...
export function IncomeWealthDistribution({
  ...
  function getInequalityHealth() {
    ...
    if (gini <= 0.25) return { label: "Very Equal", color: "text-green-600", variant: "default" as const };
    if (gini <= 0.35) return { label: "Moderately Equal", color: "text-blue-600", variant: "secondary" as const };
    if (gini <= 0.45) return { label: "Average Inequality", color: "text-yellow-600", variant: "destructive" as const };
    if (gini <= 0.55) return { label: "High Inequality", color: "text-orange-600", variant: "destructive" as const };
  ...
  return (
    <TooltipProvider>
      <div className="space-y-6">
        ...
        {/* Health Status */}
        <Alert className={`border-l-4 ${inequalityHealth.color === 'text-green-600' ? 'border-l-green-500' : 
                                      inequalityHealth.color === 'text-blue-600' ? 'border-l-blue-500' :
                                      inequalityHealth.color === 'text-yellow-600' ? 'border-l-yellow-500' : 'border-l-red-500'}`}>
          <Scale className="h-4 w-4" />
          <AlertDescription className="flex items-center justify-between">
            <span>
              Income Inequality: <span className={`font-semibold ${inequalityHealth.color}`}>{inequalityHealth.label}</span>
              <span className="ml-4">
                Social Mobility: <span className={`font-semibold ${mobilityHealth.color}`}>{mobilityHealth.label}</span>
              </span>
            </span>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Population and wealth distribution across economic classes</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 gap-3">
              {classMetrics.map((cls, index) => (
                <div key={cls.name} className="p-3 rounded-lg border bg-muted/30">
                  ...
                  <div className="space-y-3">
                    {referenceCountry.incomeInequalityGini && (
                      <div className="flex justify-between items-center text-sm">
                        ...
                        <div className="space-x-2">
                          <Badge variant="outline">{referenceCountry.incomeInequalityGini.toFixed(2)}</Badge>
                          <span>vs</span>
                          <Badge variant={incomeData.incomeInequalityGini <= referenceCountry.incomeInequalityGini ? "default" : "secondary"}>
                    ...
                    {referenceCountry.povertyRate && (
                      <div className="flex justify-between items-center text-sm">
                        ...
                        <div className="space-x-2">
                          <Badge variant="outline">{formatPercentage(referenceCountry.povertyRate)}</Badge>
                          <span>vs</span>
                          <Badge variant={incomeData.povertyRate <= referenceCountry.povertyRate ? "default" : "secondary"}>
                  ...
                  <CardContent className="space-y-4">
                    <div>
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <Badge className={`${mobilityHealth.color.replace('text-', 'bg-').replace('600', '100')}`}>
              ...
              <TabsContent value="classes" className="space-y-4">
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Adjust population and wealth distribution by class</CardDescription>
                  </CardHeader>
                  <CardContent>
        ...
        <Alert>
          ...
          <AlertDescription>
            ...
            <p className="text-sm mt-1">
              Society has {inequalityHealth.label.toLowerCase()} (Gini: {incomeData.incomeInequalityGini.toFixed(2)}) 
              with {incomeData.povertyRate.toFixed(1)}% living in poverty. 
              Social mobility is {mobilityHealth.label.toLowerCase()}, 
