...
export function CountryInfobox({
  countryName,
  flagUrl: preCachedFlagUrl,
  onToggle,
    ...
    try {
      ...
      if (preCachedFlagUrl) {
        ...
        if (processedFlagUrl.includes("localhost") || processedFlagUrl.includes("127.0.0.1")) {
          processedFlagUrl = processedFlagUrl.replace(
        ...
        try {
          ...
          if (cachedFlag) {
            ...
            if (flag) {
              ...
              if (
                processedFlagUrl.includes("localhost") ||
                processedFlagUrl.includes("127.0.0.1")
              ) {
                processedFlagUrl = processedFlagUrl.replace(
                  /https?:\/\/(localhost|127\.0\.0\.1)(:\d+)?/,
                  "https://ixwiki.com"
      ...
      if (data?.renderedHtml) {
        setActiveTab("html");
        console.log(
          `[CountryInfobox] Received HTML content for ${countryName}, defaulting to HTML tab`
        );
      }
          ...
          if (coatName) {
            try {
              ...
              if (typeof coatUrl === "string") {
                ...
                if (
                  processedCoatUrl.includes("localhost") ||
                  processedCoatUrl.includes("127.0.0.1")
                ) {
                  processedCoatUrl = processedCoatUrl.replace(
                    /https?:\/\/(localhost|127\.0\.0\.1)(:\d+)?/,
                    "https://ixwiki.com"
                  );
                }
              ...
              console.warn(
                `[CountryInfobox] Failed to load coat of arms for ${countryName}:`,
                coatError
              );
              setCoatOfArmsUrl(null);
            }
        ...
        if (data.locator_map || data.image_map) {
          ...
          if (mapName) {
            try {
              ...
              console.log(
                `[CountryInfobox] Processing map image: "${mapName}" -> "${actualFileName}"`
              );
              const mapUrl = await ixnayWiki.getFileUrl(actualFileName);
              if (typeof mapUrl === "string") {
                let processedMapUrl = mapUrl;
                if (
                  processedMapUrl.includes("localhost") ||
                  processedMapUrl.includes("127.0.0.1")
                ) {
                  processedMapUrl = processedMapUrl.replace(
                    /https?:\/\/(localhost|127\.0\.0\.1)(:\d+)?/,
                    "https://ixwiki.com"
                  );
                }
              ...
              console.warn(
                `[CountryInfobox] Failed to load map image for ${countryName}:`,
                mapError
              );
              setMapImageUrl(null);
            }
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Failed to load country information";
  ...
  const handleRetry = () => {
    if (loadingState.retryCount < MAX_RETRIES) {
      console.log(
        `[CountryInfobox] Retrying load for ${countryName} (attempt ${loadingState.retryCount + 1}/${MAX_RETRIES})`
      );
      ...
      setTimeout(
        ...
        1000 * (loadingState.retryCount + 1)
  ...
  const handleClearCache = async () => {
    try {
      ...
      await fetch(`/api/mediawiki?country=${encodeURIComponent(countryName)}`, {
        method: "DELETE",
      });
      console.log(
        `[CountryInfobox] Cache cleared for ${countryName}, reloading data with enhanced template processing`
      );
  ...
  const formatInfoboxFields = (currentInfobox: CountryInfoboxType): InfoboxField[] => {
    ...
    const fieldMappings: Array<{
      ...
      {
        keys: ["conventional_long_name", "official_name"],
        label: "Official Name",
        icon: Globe,
        priority: 3,
      },
      ...
      {
        keys: ["area", "area_km2", "area_total"],
        label: "Area",
        icon: MapPin,
        priority: 8,
        formatter: (v) => (v.includes("km") ? v : `${parseFloat(v).toLocaleString()} km²`),
      },
      { keys: ["area_rank"], label: "Area Rank", icon: MapPin, priority: 9 },

      // Government
      {
        keys: ["government", "government_type"],
        label: "Government",
        icon: Building,
        priority: 10,
      },
      {
        keys: ["leader_title1", "head_of_state"],
        label: "Head of State",
        icon: Users,
        priority: 11,
      },
      ...
      {
        keys: ["leader_name2", "deputy_leader"],
        label: "Deputy Leader",
        icon: Users,
        priority: 14,
      },
      ...
      // Cultural
      {
        keys: ["official_languages", "official_language"],
      ...
      {
        keys: ["independence", "independence_date"],
        label: "Independence",
        icon: Clock,
        priority: 45,
      },
      ...
      {
        keys: ["driving_side", "drives_on", "drivingSide"],
        label: "Driving Side",
        icon: Navigation,
        priority: 49,
      },
        ...
        <CardFooter className="border-t p-4">
          <Button variant="outline" asChild className="w-full">
            <a href={getWikiUrl()} target="_blank" rel="noopener noreferrer">
              <ExternalLink className="mr-2 h-4 w-4" /> View on Wiki
            </a>
          </Button>
        </CardFooter>
      </Card>
  ...
  if (!infobox) {
    return (
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center">
            <Globe className="mr-2 h-5 w-5" /> {countryName}
          </CardTitle>
        ...
        <CardContent className="p-4 text-center">
          ...
          <p className="text-muted-foreground text-sm">
            No detailed infobox data found on Ixnay Wiki for this country.
          </p>
        ...
        <CardFooter className="border-t p-4">
          <Button variant="outline" asChild className="w-full">
            <a href={getWikiUrl()} target="_blank" rel="noopener noreferrer">
              <ExternalLink className="mr-2 h-4 w-4" /> View Wiki Page
            </a>
          </Button>
        </CardFooter>
      </Card>
  ...
  return (
    <Card className="transition-all duration-300">
      <CardHeader className="pb-4">
        <div className="flex items-center justify-between">
          ...
          <Button variant="outline" size="sm" asChild>
            <a href={getWikiUrl()} target="_blank" rel="noopener noreferrer">
              <ExternalLink className="mr-1.5 h-3.5 w-3.5" /> Wiki
            </a>
          </Button>
        </div>
      ...
      <CardContent className="country-infobox-content px-4 pt-0 pb-4">
            ...
            {flagUrl && (
              <div className="flex flex-col items-center">
                <img
                  ...
                  className="bg-muted/20 h-16 max-w-full rounded border object-contain shadow-sm"
                  onError={(e) => {
                    console.error(`Failed to load flag: ${flagUrl}`);
                    (e.target as HTMLImageElement).style.display = "none";
                  }}
                />
                <span className="text-muted-foreground mt-1 text-xs">Flag</span>
              </div>
            )}
            {coatOfArmsUrl && (
              <div className="flex flex-col items-center">
                ...
                />
                <span className="text-muted-foreground mt-1 text-xs">Coat of Arms</span>
              </div>
            )}
            {mapImageUrl && (
              <div className="flex flex-col items-center">
                ...
                />
                <span className="text-muted-foreground mt-1 text-xs">Map</span>
              </div>
            )}
          </div>
        )}
        ...
        {hasHtmlContent ? (
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value="structured">
              ...
              {fields.length > 12 && (
                <Button
                  ...
                  className="text-primary hover:text-primary/90 mt-4 w-full"
                >
                  {isExpanded ? (
                  ...
                  {isExpanded
                    ? "Show Less"
                    : `Show ${fields.length - 12} More Field${fields.length - 12 > 1 ? "s" : ""}`}
                </Button>
              )}
            </TabsContent>
            ...
            <TabsContent value="html">
              <div
                className="wiki-infobox prose prose-sm styled-infobox mt-4 max-w-none overflow-x-auto"
                // SECURITY: Sanitize wiki content to prevent XSS from external data
