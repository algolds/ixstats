// src/app/(wikios)/wiki-special/lorewards/page.tsx
            ...
            {leaders.length > 3 && (
              <div className="wikios-lw-table">
                ...
                {leaders.slice(3).map((user) => (
                  <Link
                    key={user.username}
                    href={withBasePath(`/wiki-special/user/${encodeURIComponent(user.username)}`)}
              ...
              {winners.map((w, i) => (
                <div key={`${w.date}-${w.type}-${i}`} className="wikios-lw-winner-card">
                  <div className="wikios-lw-winner-header">
                    ...
                    <span className="wikios-lw-winner-date">{formatDate(w.date)}</span>
                  </div>
                  {w.winnerUser && (
                    <Link href={withBasePath(`/wiki-special/user/${encodeURIComponent(w.winnerUser)}`)} className="wikios-lw-winner-user">
                  ...
                  {w.runnerUpUser && (
                    <div className="wikios-lw-winner-runner">
                      ...
                      <Link href={withBasePath(`/wiki-special/user/${encodeURIComponent(w.runnerUpUser)}`)} className="wikios-lw-runner-name">
                        {w.runnerUpUser}
                      </Link>
                    </div>
                  )}
...
function PodiumCard({ user, rank }: {
  ...
  return (
    <Link href={withBasePath(`/wiki-special/user/${encodeURIComponent(user.username)}`)} className={cn("wikios-lw-podium-card", rank === 1 && "wikios-lw-podium-first")}>
