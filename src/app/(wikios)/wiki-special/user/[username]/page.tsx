// src/app/(wikios)/wiki-special/user/[username]/page.tsx
...
import { StreakCalendar } from "~/components/wikios/profile/StreakCalendar";
import {
  User, Trophy, Calendar, FileText, Clock, Shield,
  TrendingUp, Award, Flame, BookOpen, Loader2, Hash,
          ...
          <>
            ...
            <div className="wikios-profile-header">
              ...
              <div className="wikios-profile-identity">
                ...
                <div className="wikios-profile-meta">
                  {info.registration && (
                    <span><Calendar size={12} /> Joined {new Date(info.registration).toLocaleDateString("en-US", { month: "long", year: "numeric" })}</span>
                  )}
                  <span><FileText size={12} /> {info.editCount.toLocaleString()} edits</span>
                  {rank && <span><Trophy size={12} /> Rank #{rank}</span>}
                </div>
            ...
            <div className="wikios-profile-grid">
              ...
              <div className="wikios-profile-main">
                ...
                {stats && (
                  <section className="wikios-profile-section">
                    <h2 className="wikios-profile-section-title"><Trophy size={16} /> Lorewards</h2>
                    <div className="wikios-profile-stats-grid">
                      <StatCard label="Daily Wins" value={stats.dailyWins} icon={<Award size={16} />} color="#fbbf24" />
                      <StatCard label="Runner-ups" value={stats.dailyRunnerUps} icon={<TrendingUp size={16} />} color="#94a3b8" />
                      <StatCard label="Weekly Wins" value={stats.weeklyWins} icon={<Trophy size={16} />} color="#60a5fa" />
                      <StatCard label="Monthly Wins" value={stats.monthlyWins} icon={<Trophy size={16} />} color="#a78bfa" />
                      <StatCard label="Current Streak" value={stats.currentStreak} icon={<Flame size={16} />} color={stats.currentStreak > 0 ? "#f97316" : "#64748b"} suffix="days" />
                      <StatCard label="Best Streak" value={stats.longestStreak} icon={<Flame size={16} />} color="#ef4444" suffix="days" />
                    </div>

                    {stats.totalScore > 0 && (
                      <div className="wikios-profile-total-row">
                        <span>Total Score: <strong>{stats.totalScore.toLocaleString()}</strong></span>
                        <span>Total Bytes: <strong>{stats.totalBytes.toLocaleString()}</strong></span>
                      </div>
                    )}
                  </section>
                )}
                ...
                {/* Streak Calendar */}
                <section className="wikios-profile-section">
                  <h2 className="wikios-profile-section-title"><Calendar size={16} /> Award Calendar</h2>
                ...
                {awards.length > 0 && (
                  <section className="wikios-profile-section">
                    <h2 className="wikios-profile-section-title"><Award size={16} /> Award History</h2>
                    <div className="wikios-profile-award-list">
                      {awards.map((a, i) => (
                        <div key={`${a.date}-${a.type}-${i}`} className="wikios-profile-award-row">
                          <span className={cn("wikios-profile-award-badge", `wikios-award-type-${a.type}`)}>
                            {a.type}
                          </span>
                          <span className="wikios-profile-award-date">{a.date}</span>
                          <span className={cn("wikios-profile-award-role", a.role === "winner" ? "wikios-award-winner" : "wikios-award-runner")}>
                            {a.role === "winner" ? "Winner" : "Runner-up"}
                          </span>
                          {a.page && (
                            <Link href={withBasePath(`/w/${encodeURIComponent(a.page.replace(/ /g, "_"))}`)} className="wikios-profile-award-page">
                              {a.page}
                            </Link>
                          )}
                          {a.score && <span className="wikios-profile-award-score">{a.score.toLocaleString()} pts</span>}
                        </div>
                      ))}
                    </div>
                  </section>
              ...
              <div className="wikios-profile-sidebar">
                {/* Recent Edits */}
                <section className="wikios-profile-section">
                  <h2 className="wikios-profile-section-title"><BookOpen size={16} /> Recent Edits</h2>
                  ...
                  <div className="wikios-profile-edits">
                    {edits.map((edit) => (
                      <div key={edit.revid} className="wikios-profile-edit-row">
                        <Link
                          href={withBasePath(`/w/${encodeURIComponent(edit.title.replace(/ /g, "_"))}`)}
                          className="wikios-profile-edit-title"
                        >
                          {edit.title}
                        </Link>
                        <div className="wikios-profile-edit-meta">
                          <span><Clock size={10} /> {formatMWTimeAgo(edit.timestamp)}</span>
                          <span className={cn(
                            "wikios-profile-edit-size",
                            edit.size > 0 ? "wikios-diff-positive" : edit.size < 0 ? "wikios-diff-negative" : ""
                          )}>
                            {edit.size > 0 ? <ArrowUpRight size={10} /> : edit.size < 0 ? <ArrowDownRight size={10} /> : null}
                            {edit.size > 0 ? "+" : ""}{edit.size.toLocaleString()}
                          </span>
                  ...
                  <Link href={withBasePath(`/wiki-special/contributions/${encodeURIComponent(username)}`)} className="wikios-profile-view-all">
                ...
                {recentEntries.length > 0 && (
                  <section className="wikios-profile-section">
                    <h2 className="wikios-profile-section-title"><Hash size={16} /> Recent Activity</h2>
                    <div className="wikios-profile-activity">
                      {recentEntries.slice(0, 7).map((e, i) => (
                        <div key={`${e.date}-${i}`} className="wikios-profile-activity-row">
                          <span className={cn("wikios-profile-activity-dot", e.role === "winner" ? "wikios-dot-gold" : "wikios-dot-silver")} />
  ...
  return (
    <div className="wikios-stat-card" style={{ "--stat-color": color } as React.CSSProperties}>
      ...
      <div className="wikios-stat-card-value">
        {value}{suffix ? <span className="wikios-stat-card-suffix"> {suffix}</span> : null}
      </div>
