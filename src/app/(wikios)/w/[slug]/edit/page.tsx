...
export default function WikiOSEditPage() {
  ...
  const { data: editorHtml, isLoading: editorLoading } =
  ...
  const { data: wikitextData, isLoading: wtLoading, refetch: refetchWikitext } =
  ...
  const conflictBanner = editConflict && (
    ...
    >
      <p style={{ fontWeight: 600, marginBottom: 6, color: "#f87171" }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        <button
          ...
          onClick={() => {
            ...
            if (mode === "source") {
              setSaving(true);
              saveWikitext.mutateAsync({ title, wikitext: wikitextData?.wikitext ?? "", summary: "Overwriting edit conflict", minor: false })
