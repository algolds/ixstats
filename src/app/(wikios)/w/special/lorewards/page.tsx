      ...
      <div className="wikios-special-page wikios-lw-page">
        ...
        <div className="wikios-lw-tabs">
          <button
            ...
            className={cn(
              "wikios-lw-main-tab",
              tab === "leaderboard" && "wikios-lw-main-tab-active"
            )}
          >
          ...
          <button
            ...
            className={cn("wikios-lw-main-tab", tab === "recent" && "wikios-lw-main-tab-active")}
          >
            <Clock size={14} /> Recent Winners
          </button>
          <button
            onClick={() => setTab("faq")}
            className={cn("wikios-lw-main-tab", tab === "faq" && "wikios-lw-main-tab-active")}
          >
            ...
            <div className="wikios-lw-period-bar">
              {(["alltime", "daily", "weekly", "monthly"] as Period[]).map((p) => (
                <button
                  ...
                  onClick={() => setPeriod(p)}
                  className={cn(
                    "wikios-lorewards-tab",
                    period === p && "wikios-lorewards-tab-active"
                  )}
                >
              ...
              <div className="wikios-lw-table">
                ...
                {leaders.slice(3).map((user) => (
                  ...
                  >
                    ...
                    <span className="wikios-lw-td-num wikios-lw-td-score">
                      {user.totalScore.toLocaleString()}
                    </span>
                    <span className="wikios-lw-td-num">
                      {user.currentStreak > 0 && (
                        <Flame size={11} className="mr-0.5 inline text-orange-400" />
                      )}
          ...
          <div className="wikios-lw-section">
            {winners.length === 0 && (
              <div className="wikios-stashes-empty-stash">
                <Trophy size={32} className="opacity-15" />
              ...
              {winners.map((w, i) => (
                <div key={`${w.date}-${w.type}-${i}`} className="wikios-lw-winner-card">
                  <div className="wikios-lw-winner-header">
                    <span
                      className={cn("wikios-profile-award-badge", `wikios-award-type-${w.type}`)}
                    >
                      {w.type}
                    </span>
                  ...
                  {w.winnerPage && (
                    <Link
                      href={withBasePath(
                        `/w/${encodeURIComponent(w.winnerPage.replace(/ /g, "_"))}`
                      )}
                      className="wikios-lw-winner-page"
                    >
                      <BookOpen size={12} />
                      {w.winnerPage}
                    </Link>
                  ...
                  {w.runnerUpUser && (
                    <div className="wikios-lw-winner-runner">
                      ...
                      <Link
                        href={withBasePath(`/w/special/user/${encodeURIComponent(w.runnerUpUser)}`)}
                        className="wikios-lw-runner-name"
                      >
                        {w.runnerUpUser}
                      </Link>
                    </div>
        ...
        {tab === "faq" && (
          <div className="wikios-lw-section wikios-lw-faq">
            <div className="wikios-lw-faq-intro">
              <p>
                The <strong>Order of the Lore</strong> (Lorewards) is IxWiki&apos;s award system
                recognizing outstanding contributions to the collaborative worldbuilding project.
                Established in 2017, it tracks daily, weekly, and monthly achievements through a
                combination of automated scoring and community panel review.
              </p>
            </div>

            <FaqSection icon={<Zap size={16} />} title="How are daily winners chosen?">
              <p>
                Every night at <strong>11:30 PM Eastern</strong>, the Lorewards bot analyzes all
                mainspace edits made that day. It scores each contributor&apos;s work based on
              ...
              <div className="wikios-lw-faq-signals">
                <SignalCard
                  icon={<FileText />}
                  name="Byte Count"
                  desc="The foundation — how much content was added (positive changes only)"
                />
                <SignalCard
                  icon={<Highlighter />}
                  name="Prose Quality"
                  desc="Real writing vs template/infobox filler. Prose-heavy edits score higher than parameter lists"
                />
                <SignalCard
                  icon={<GitBranch />}
                  name="Collaborative Lore"
                  desc="Editing articles outside your own country earns a bonus — building the shared world together"
                />
                <SignalCard
                  icon={<BookOpen />}
                  name="Edit Depth"
                  desc="Sustained work on a single article over time is rewarded. Deep worldbuilding > drive-by edits"
                />
                <SignalCard
                  icon={<Star />}
                  name="Article Importance"
                  desc="Edits to hub articles with many inbound links have slightly more impact"
                />
              </div>
              <p>
                The bot posts its top picks to the <strong>#lorewards</strong> Discord channel with
                Approve/Reject buttons. A rotating panel of community members reviews the pick. If
                no action is taken within one hour, the result is auto-approved.
              </p>
            </FaqSection>
            ...
            <FaqSection icon={<Scale size={16} />} title="How is the score calculated?">
              ...
              <div className="wikios-lw-faq-formula">
                <code>
                  finalScore = bytesAdded × proseRatio × collaborativeBonus × depthBonus ×
                  importanceBonus
                </code>
              </div>
              <div className="wikios-lw-faq-penalties">
                ...
                <ul>
                  <li>
                    <strong>Infobox/template filler</strong> — Edits that are &gt;80% template
                    parameters (like <code>| population = 50000</code>) are disqualified
                  </li>
                  <li>
                    <strong>List articles</strong> — Pages titled &ldquo;List of...&rdquo; receive a
                    70% penalty (0.3× multiplier)
                  </li>
                  <li>
                    <strong>Minor-only edits</strong> — If all edits are minor and the largest is
                    under 1,000 bytes, an 80% penalty applies
                  </li>
                </ul>
                <h4>Requirements</h4>
                <ul>
                  <li>
                    The daily winner must have at least one single edit of{" "}
                    <strong>1,000+ bytes</strong>
                  </li>
            ...
            <FaqSection
              ...
              title="How do weekly and monthly awards work?"
            >
              <p>
                <strong>Weekly winners</strong> are determined every Sunday based on the week&apos;s
                daily results:
              </p>
              <ul>
                <li>
                  Daily win = <strong>3 points</strong>
                </li>
                <li>
                  Daily runner-up = <strong>1 point</strong>
                </li>
                <li>
                  Active day (any placement) = <strong>1 point</strong>
                </li>
              </ul>
              <p>
                <strong>Monthly winners</strong> aggregate daily and weekly performance:
              </p>
              <ul>
                <li>
                  Weekly win = <strong>5 points</strong>
                </li>
                <li>
                  Daily win = <strong>2 points</strong>
                </li>
                <li>
                  Daily runner-up = <strong>1 point</strong>
                </li>
              ...
              <p>
                <strong>Annual winners</strong> are selected by community consensus considering
                overall contribution volume, quality, and consistency throughout the year.
              </p>
            </FaqSection>
            ...
            <FaqSection icon={<Medal size={16} />} title="What is the Medal Score?">
              <p>
                ...
                <Link href="/w/IxWiki:OOL" className="wikios-lw-faq-link">
                  IxWiki:OOL
                </Link>{" "}
              ...
              <div className="wikios-lw-faq-medals">
                ...
                <MedalRow
                  ...
                  name="Burg Cross"
                  points={250}
                  how="= 5 Gold medals combined (highest honor)"
                />
              </div>
              <p>
                Bonus: Any single edit over <strong>50,000 bytes</strong> earns an additional Bronze
                medal regardless of daily winner status.
              </p>
            </FaqSection>

            <FaqSection icon={<Users size={16} />} title="Who are the panelists?">
              <p>
                Loreward determinations are made by <strong>weekly rotating panels</strong> of three
                community members. The panel foreman nominates candidates, and the other two members
                vote. Panelists exercise subjective judgment — they may prefer a smaller,
                high-quality contribution over a larger but routine edit.
              </p>
              <p>
                The automated bot generates candidates to assist the panel, but panelists retain
                full authority to approve, reject, or re-run the bot&apos;s picks.
              </p>
            </FaqSection>
            ...
            <FaqSection icon={<HelpCircle size={16} />} title="What qualifies as an eligible edit?">
              ...
              <ul>
                <li>
                  Extraordinary contribution to <strong>collaborative global lore</strong>
                </li>
                <li>
                  Lore written <strong>for someone else</strong> or for shared world topics
                </li>
                <li>
                  Overall <strong>quality of written lore</strong>
                </li>
                <li>
                  Extremely well-written <strong>novel topics</strong>
                </li>
              ...
              <p>
                ...
                &ldquo;None awarded,&rdquo; it means the panel reviewed that day and found no
                eligible entries. Skipped dates indicate no panel was convened.
              </p>
            </FaqSection>
          </div>
        )}
...
function PodiumCard({
  ...
  return (
    <Link
      href={withBasePath(`/w/special/user/${encodeURIComponent(user.username)}`)}
      className={cn("wikios-lw-podium-card", rank === 1 && "wikios-lw-podium-first")}
    >
      ...
      <div className="wikios-lw-podium-content">
        ...
        <div className="wikios-lw-podium-score" style={{ color }}>
          {totalAwards} <span className="wikios-lw-podium-score-label">awards</span>
        </div>
        ...
        <div className="wikios-lw-podium-bottom">
          {user.currentStreak > 0 && (
            <span className="wikios-lw-podium-streak">
              <Flame size={10} />
...
function AwardSparkline({ username, color }: { username: string; color: string }) {
  const { data } = api.lorewards.getAwardFrequency.useQuery({ username }, { staleTime: 300000 });
  ...
  return (
    <svg className="wikios-lw-sparkline" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none">
      <defs>
