...
// Admin interface for batch lore card generation from wiki articles
...
export default function BatchLoreCardGeneratorPage() {
  ...
  const handleFetchArticles = async () => {
    if (articleCount < 10 || articleCount > 100) {
      toast({
        title: "Invalid Count",
        description: "Please enter a number between 10 and 100",
        type: "error",
      });
      return;
    }
    ...
    try {
      ...
      for (const source of sources) {
        ...
        if (!response.ok) {
          throw new Error(`Failed to fetch articles from ${source}`);
        }
          ...
          try {
            ...
            if (previewResponse.ok) {
              ...
              allArticles.push({
                ...
                wikiSource: source,
                approved: preview.qualityScore >= 40, // Auto-approve if quality >= 40
              });
            }
          } catch (error) {
      ...
      toast({
        ...
        description: `Found ${allArticles.length} eligible articles`,
        type: "success",
      });
    } catch (error) {
  ...
  const filteredArticles = useMemo(() => {
    return articles.filter((article) => {
      ...
      // Quality filter
      if (qualityFilter > 0 && article.qualityScore < qualityFilter) {
        return false;
      }
  ...
  const handleGenerateCards = async () => {
    ...
    if (approvedArticles.length === 0) {
      toast({
        ...
        description: "Please approve at least one article",
        type: "warning",
      });
      return;
    }
          ...
          setArticles((prev) =>
            prev.map((a) =>
              a.title === article.title
                ? { ...a, generating: false, generated: true }
                : a
            )
          );
            ...
            prev.map((a) =>
              a.title === article.title
                ? {
                    ...a,
                    generating: false,
                    error: errorData.message || "Failed to generate",
                  }
                : a
            )
          );
          setGenerationResults((prev) =>
            prev.failed === 0 || errorData.message?.includes("already exists")
    ...
    toast({
      ...
      description: `Success: ${generationResults.success}, Failed: ${generationResults.failed}, Skipped: ${generationResults.skipped}`,
      type: "success",
    });
  };
  ...
  return (
    <div className="bg-background text-foreground min-h-screen p-4 md:p-8">
      <div className="mx-auto max-w-7xl">
        ...
        {/* Stats */}
        {articles.length > 0 && (
          <div className="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-2 md:grid-cols-5">
        ...
        {/* Progress Bar */}
        {isGenerating && (
          <Card className="glass-card-parent mb-6 p-6">
        ...
        {/* Filters */}
        {articles.length > 0 && (
          <div className="glass-card-parent mb-6 rounded-xl border border-purple-500/20 p-4">
...
function ArticlePreviewCard({ article, onToggleApproval, disabled }: ArticlePreviewCardProps) {
  const rarityColor = RARITY_COLORS[article.estimatedRarity as keyof typeof RARITY_COLORS] || RARITY_COLORS.COMMON;
