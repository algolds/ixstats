...
export function StatusCards({ 
  ...
  const currentIxTime = statusLoading 
    ? "Loading..." 
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700">
        <div className="flex items-center">
          <Clock className={`h-8 w-8 ${getMultiplierColor(systemStatus?.ixTime?.multiplier ?? 4)}`} />
          <div className="ml-4">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700">
        <div className="flex items-center">
          <Zap className={`h-8 w-8 ${getMultiplierColor(systemStatus?.ixTime?.multiplier ?? 4)}`} />
          <div className="ml-4">
            <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Time Multiplier</p>
            <p className={`text-lg font-semibold ${getMultiplierColor(systemStatus?.ixTime?.multiplier ?? 4)}`}>
              {statusLoading ? "Loading..." : (systemStatus?.ixTime?.isPaused ? "PAUSED" : `${systemStatus?.ixTime?.multiplier ?? 4}x Speed`)}
            </p>
            <p className="text-xs text-gray-500 dark:text-gray-400">
              {botStatus?.botHealth?.available ? "Bot controlled" : "Local fallback"}
            </p>
          </div>
        </div>
      </div>
