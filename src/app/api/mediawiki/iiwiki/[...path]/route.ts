    ...
    if (filePathMatch && filePathMatch[1]) {
      ...
      try {
        ...
        if (cached?.data?.url) {
          ...
          console.log(
            `[IIWiki Proxy] Cache HIT for Special:FilePath resolution of ${filename} -> ${directUrl}`
          );
        }
      } catch (cacheErr) {
      ...
      if (directUrl) {
        try {
          ...
          if (imageResponse.ok) {
            ...
            console.warn(
              `[IIWiki Proxy] Image fetch via wsrv.nl failed with status ${imageResponse.status}. Falling back to direct fetch.`
            );
    ...
    if (subpath.startsWith("images/")) {
      ...
      try {
        ...
        if (imageResponse.ok) {
          ...
          console.warn(
            `[IIWiki Proxy] Direct image fetch via wsrv.nl failed with status ${imageResponse.status}.`
          );
          return new NextResponse(null, { status: 502, headers: corsHeaders });
        }
      } catch (err) {
