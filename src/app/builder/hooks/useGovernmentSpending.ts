...
export function useGovernmentSpending({
  ...
  const [autoGeneratedBuilder, setAutoGeneratedBuilder] = useState<GovernmentBuilderState | null>(
    null
  );
  const integrationState = useMemo(
    () => ({
      components: selectedAtomicComponents,
      governmentBuilder: governmentBuilderData,
  ...
  // Budget calculations
  const totalBudget =
    governmentBuilderData?.structure?.totalBudget || inputs.governmentSpending.totalSpending || 0;
  const totalAllocated = useMemo(
    () =>
      governmentBuilderData?.budgetAllocations?.reduce(
  ...
  const totalRevenue = useMemo(
    () =>
      governmentBuilderData?.revenueSources?.reduce(
        (sum, rev) => sum + (rev.revenueAmount || 0),
        0
      ) || 0,
  ...
  useEffect(() => {
    if (
      ...
      validation.hasGovernmentBuilder === false
    ) {
      ...
      const updatedInputs = {
        ...
        governmentSpending: {
          ...
          spendingCategories: generated.departments.map((dept, index) => {
            const allocation = generated.budgetAllocations.find(
              (a) => a.departmentId === index.toString()
            );
            return {
              category: dept.name,
