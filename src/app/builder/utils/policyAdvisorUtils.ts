...
export function generatePolicyAdvisorTips(inputs: EconomicInputs, activeSection?: string): PolicyAdvisorTip[] {
  ...
  if (!activeSection || activeSection === 'labor-employment') {
    ...
    const minWageToGdpRatio = (inputs.laborEmployment.minimumWage / inputs.coreIndicators.gdpPerCapita) * 100;
  ...
  if (!activeSection || activeSection === 'fiscal-system') {
    ...
    const budgetBalance = inputs.fiscalSystem.taxRevenueGDPPercent - inputs.fiscalSystem.governmentBudgetGDPPercent;
    if (budgetBalance < -5) {
      tips.push({
        id: 'budget-deficit',
        section: 'fiscal-system',
  ...
  if (!activeSection || activeSection === 'government-spending') {
    // Education spending
    const educationSpending = inputs.governmentSpending.spendingCategories.find(c => c.category === 'Education')?.percent || 0;
    ...
    // Healthcare spending
    const healthcareSpending = inputs.governmentSpending.spendingCategories.find(c => c.category === 'Healthcare')?.percent || 0;
    ...
    const infrastructureSpending = inputs.governmentSpending.spendingCategories.find(c => c.category === 'Infrastructure')?.percent || 0;
    ...
    // Social safety net
    const socialSpending = inputs.governmentSpending.spendingCategories.find(c => c.category === 'Social Safety Net')?.percent || 0;
