...
export function getRecommendedArchetype(
  ...
  return recommendations.sort((a: ArchetypeRecommendation, b: ArchetypeRecommendation) => b.matchScore - a.matchScore);
}
...
export function generateImplementationSteps(
  ...
  steps.push({
    ...
    description: 'Conduct detailed analysis of current economic state, identify gaps, and develop comprehensive transition strategy',
    duration: 6,
    priority: 'critical',
  ...
  steps.push({
    ...
    description: 'Establish necessary legal, regulatory, and institutional frameworks to support the new economic model',
    duration: 12,
    priority: 'critical',
  ...
  if (needsInfrastructureInvestment(targetArchetype, currentState)) {
    steps.push({
      ...
      description: 'Invest in critical infrastructure including transportation, telecommunications, energy, and digital infrastructure',
      duration: 24,
      priority: 'high',
  ...
  if (needsEducationReform(targetArchetype, currentState)) {
    steps.push({
      ...
      description: 'Reform education system and implement large-scale skills training programs aligned with target archetype requirements',
      duration: 18,
      priority: 'high',
  ...
  if (needsSectorRebalancing(targetArchetype, currentState)) {
    steps.push({
      ...
      description: 'Implement policies to shift economic activity toward target sector distribution, supporting emerging industries',
      duration: 36,
      priority: 'high',
  ...
  if (requiresInnovationEcosystem(targetArchetype, currentState)) {
    steps.push({
      ...
      description: 'Build innovation infrastructure including R&D centers, startup incubators, and technology transfer mechanisms',
      duration: 30,
      priority: 'medium',
  ...
  steps.push({
    ...
    description: 'Develop comprehensive social protection systems to support workers during transition and beyond',
    duration: 24,
    priority: 'high',
  ...
  steps.push({
    ...
    description: 'Implement public communication campaign and cultural programs to build support and align values with new model',
    duration: 36,
    priority: 'medium',
  ...
  steps.push({
    ...
    description: 'Integrate all components, optimize performance, and adjust policies based on early results',
    duration: 24,
    priority: 'medium',
  ...
  steps.push({
    ...
    description: 'Conduct comprehensive evaluation, institutionalize best practices, and establish long-term sustainability mechanisms',
    duration: 12,
    priority: 'medium',
...
export function calculateTransitionCost(
  ...
  const totalCost = infrastructureCost + educationCost + technologyCost + administrativeCost + socialCost;
...
export function calculateTransitionComplexity(
  ...
  return {
    ...
    riskLevel,
    mitigationStrategies: generateMitigationStrategies(overallComplexity, { institutional, economic, social, technical })
  };
}
...
export function generateWarningsAndRisks(
  ...
  if (complexity.complexityScore > 75) {
    criticalWarnings.push('Extremely high implementation complexity may lead to failure or abandonment');
    criticalWarnings.push('Risk of political resistance and social backlash');
  }

  if (currentState.politicalStability < 50) {
    criticalWarnings.push('Low political stability threatens sustained commitment required for transition');
  }

  if (currentState.infrastructureQuality < 40 && targetArchetype.implementationComplexity === 'high') {
    criticalWarnings.push('Severe infrastructure gaps may make transition infeasible without massive investment');
  }
  ...
  if (targetArchetype.economicComponents.includes(EconomicComponentType.INNOVATION_ECONOMY) && currentState.innovationCapacity < 50) {
...
function calculateEconomicAlignment(archetype: EconomicArchetype, characteristics: CountryCharacteristics): number {
  ...
  const growthDiff = Math.abs(characteristics.currentGrowthRate - archetype.growthMetrics.gdpGrowth);
...
function calculateCulturalFit(archetype: EconomicArchetype, characteristics: CountryCharacteristics): number {
  let score = 50;
...
function calculateInstitutionalReadiness(archetype: EconomicArchetype, characteristics: CountryCharacteristics): number {
  let score = 0;
...
function calculateGeographicSuitability(archetype: EconomicArchetype, characteristics: CountryCharacteristics): number {
...
function calculateCompatibility(archetype: EconomicArchetype, characteristics: CountryCharacteristics) {
  return {
...
function generateReasons(archetype: EconomicArchetype, characteristics: CountryCharacteristics): string[] {
...
function identifyBenefits(archetype: EconomicArchetype, characteristics: CountryCharacteristics): string[] {
  return archetype.strengths.slice(0, 5);
}

function identifyChallenges(archetype: EconomicArchetype, characteristics: CountryCharacteristics): string[] {
  return archetype.challenges.slice(0, 5);
}

function estimateTimeHorizon(archetype: EconomicArchetype, characteristics: CountryCharacteristics): 'short' | 'medium' | 'long' {
...
function determineRecommendationLevel(matchScore: number, compatibility: any): 'highly-recommended' | 'recommended' | 'consider' | 'not-recommended' {
...
function applyFocusAreaAdjustment(score: number, archetype: EconomicArchetype, focusArea: string): number {
...
function needsInfrastructureInvestment(archetype: EconomicArchetype, state: CountryCharacteristics): boolean {
  return state.infrastructureQuality < 60 && archetype.implementationComplexity !== 'low';
}

function needsEducationReform(archetype: EconomicArchetype, state: CountryCharacteristics): boolean {
  return state.humanCapitalIndex < 70 && archetype.economicComponents.includes(EconomicComponentType.HIGH_SKILLED_WORKERS);
}

function needsSectorRebalancing(archetype: EconomicArchetype, state: CountryCharacteristics): boolean {
...
function requiresInnovationEcosystem(archetype: EconomicArchetype, state: CountryCharacteristics): boolean {
  return archetype.economicComponents.includes(EconomicComponentType.INNOVATION_ECONOMY) && state.innovationCapacity < 60;
}

function calculateInfrastructureCost(archetype: EconomicArchetype, state: CountryCharacteristics, gdp: number): number {
  const gap = Math.max(0, 80 - state.infrastructureQuality);
  return (gap / 100) * gdp * 0.15;
}

function calculateEducationCost(archetype: EconomicArchetype, state: CountryCharacteristics, gdp: number): number {
  const gap = Math.max(0, 75 - state.humanCapitalIndex);
  return (gap / 100) * gdp * 0.08;
}

function calculateTechnologyCost(archetype: EconomicArchetype, state: CountryCharacteristics, gdp: number): number {
  const gap = Math.max(0, 70 - state.innovationCapacity);
  return (gap / 100) * gdp * 0.10;
}

function calculateAdministrativeCost(archetype: EconomicArchetype, state: CountryCharacteristics, gdp: number): number {
  return gdp * 0.03; // Fixed 3% for administrative costs
}

function calculateSocialCost(archetype: EconomicArchetype, state: CountryCharacteristics, gdp: number): number {
...
function estimateROITimeline(archetype: EconomicArchetype, state: CountryCharacteristics): number {
  const complexity = archetype.implementationComplexity === 'high' ? 3 : archetype.implementationComplexity === 'medium' ? 2 : 1;
  const readiness = state.politicalStability / 100;

  return Math.round(10 * complexity * (2 - readiness)); // Years to positive ROI
}

function calculateInstitutionalComplexity(archetype: EconomicArchetype, state: CountryCharacteristics): number {
  return 100 - state.politicalStability;
}

function calculateEconomicComplexity(archetype: EconomicArchetype, state: CountryCharacteristics): number {
...
function calculateSocialComplexity(archetype: EconomicArchetype, state: CountryCharacteristics): number {
  const culturalGap = archetype.culturalFactors.filter(f =>
    !state.culturalFactors.some(sf => sf.toLowerCase().includes(f.toLowerCase()))
  ).length;
  return Math.min(100, culturalGap * 10);
}

function calculateTechnicalComplexity(archetype: EconomicArchetype, state: CountryCharacteristics): number {
