...
export function EnhancedDial({
  ...
  const referenceAngle =
    referenceValue !== undefined
      ? startAngle + ((referenceValue - min) / (max - min)) * angleRange
      : null;

  const referenceX =
    referenceAngle !== null ? center + Math.cos(angleToRadians(referenceAngle - 90)) * radius : 0;
  const referenceY =
    referenceAngle !== null ? center + Math.sin(angleToRadians(referenceAngle - 90)) * radius : 0;
    ...
    <div className={cn("space-y-4", className)} style={cssVars as React.CSSProperties}>
      ...
      {(label || description) && (
        <div className="space-y-1 text-center">
          {label && (
            <label className="text-foreground flex items-center justify-center gap-2 text-sm font-medium">
      ...
      <div className="flex justify-center">
        <div
          ref={dialRef}
          className={cn("relative select-none", disabled && "cursor-not-allowed opacity-50")}
        ...
        >
          ...
          {/* SVG Dial */}
          <svg width={config.dial} height={config.dial} className="absolute inset-0">
            {/* Background Arc */}
            <path
              d={backgroundPath}
          ...
          {showValue && (
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center">
                <motion.div className="text-foreground text-2xl font-bold">
                  {!isNaN(value) ? Number(value).toFixed(precision) : "0"}
                </motion.div>
                {unit && <div className="text-muted-foreground text-sm">{unit}</div>}
              </div>
            </div>
          )}
          ...
          <motion.div
            ...
            whileDrag={{ scale: 1.2 }}
            className={cn(
              "absolute cursor-grab rounded-full transition-all duration-200",
