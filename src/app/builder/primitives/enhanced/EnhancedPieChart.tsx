  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 text-sm">
        <div className="h-3 w-3 rounded-sm" style={{ backgroundColor: data.payload.fill }} />
        <span className="font-medium text-gray-900 dark:text-gray-100">{data.name}</span>
      </div>
      <div className="mt-1 text-sm text-gray-900 dark:text-gray-100">
        Value: <span className="">{formatValue ? formatValue(data.value) : data.value}</span>
      </div>
      ...
      <div className={cn("space-y-4", className)} style={cssVars as React.CSSProperties}>
        ...
        >
          <motion.div
            ...
            className="h-16 w-16 rounded-full border-4 border-blue-500 border-t-transparent dark:border-blue-400"
          />
        </div>
      </div>
    );
      ...
      <div className="flex flex-col items-center gap-6 lg:flex-row">
        ...
        <div
          ...
          style={{
            ...
            width:
              width && !isNaN(width) ? `${width}px` : `${!isNaN(height) ? height + 32 : 300}px`,
          }}
        >
