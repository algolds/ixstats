...
export function IncomeDistribution({
  ...
  const getInequalityLevel = (gini: number) => {
    ...
    if (gini < 0.4)
      return { level: "Moderate", color: "blue", status: "Typical for Developed Nations" };
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <Alert
        className={`border-${inequalityAssessment.color}-200 bg-${inequalityAssessment.color}-50`}
      >
              ...
              <div key={incomeClass.name} className="space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className={`h-3 w-3 rounded-full bg-${incomeClass.color}-500`} />
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-muted-foreground text-xs">
                      {formatCurrency(incomeClass.averageIncome)}/yr
                    </div>
                  </div>
                </div>
                ...
                {showAdvanced && (
                  <div className="text-muted-foreground pl-5 text-xs">
                    Threshold: {formatCurrency(incomeClass.threshold)} • Population:{" "}
                    {Math.round(totalPopulation * (incomeClass.percent / 100)).toLocaleString()}
                  </div>
                )}
              </div>
            ))}
            ...
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
              <div className="rounded-lg bg-red-50 p-4 text-center">
                <div className="text-2xl font-bold text-red-600">
                  {data.incomeShare.bottom50.toFixed(1)}%
                </div>
                <div className="text-muted-foreground mt-1 text-xs">Bottom 50%</div>
              </div>
              <div className="rounded-lg bg-blue-50 p-4 text-center">
                <div className="text-2xl font-bold text-blue-600">
                  {data.incomeShare.middle40.toFixed(1)}%
                </div>
                <div className="text-muted-foreground mt-1 text-xs">Middle 40%</div>
              </div>
              <div className="rounded-lg bg-purple-50 p-4 text-center">
                <div className="text-2xl font-bold text-purple-600">
                  {data.incomeShare.top10.toFixed(1)}%
                </div>
                <div className="text-muted-foreground mt-1 text-xs">Top 10%</div>
              </div>
              <div className="rounded-lg bg-emerald-50 p-4 text-center">
                <div className="text-2xl font-bold text-emerald-600">
                  {data.incomeShare.top1.toFixed(1)}%
                </div>
                <div className="text-muted-foreground mt-1 text-xs">Top 1%</div>
              </div>
            </div>
          </div>
        </CardContent>
            ...
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
              <div className="rounded bg-gray-50 p-3 text-center">
                <div className="text-lg font-bold">
                  {formatCurrency(data.incomePercentiles.p10)}
                </div>
                <div className="text-muted-foreground text-xs">10th percentile</div>
              </div>
              <div className="rounded bg-gray-50 p-3 text-center">
                <div className="text-lg font-bold">
                  {formatCurrency(data.incomePercentiles.p25)}
                </div>
                <div className="text-muted-foreground text-xs">25th percentile</div>
              </div>
              <div className="rounded bg-blue-50 p-3 text-center">
                <div className="text-lg font-bold text-blue-600">
                  {formatCurrency(data.incomePercentiles.p50)}
                </div>
                <div className="text-muted-foreground text-xs">Median (50th)</div>
              </div>
              <div className="rounded bg-gray-50 p-3 text-center">
                <div className="text-lg font-bold">
                  {formatCurrency(data.incomePercentiles.p75)}
                </div>
                <div className="text-muted-foreground text-xs">75th percentile</div>
              </div>
              <div className="rounded bg-purple-50 p-3 text-center">
                <div className="text-lg font-bold text-purple-600">
                  {formatCurrency(data.incomePercentiles.p90)}
                </div>
                <div className="text-muted-foreground text-xs">90th percentile</div>
              </div>
              <div className="rounded bg-purple-50 p-3 text-center">
                <div className="text-lg font-bold text-purple-600">
                  {formatCurrency(data.incomePercentiles.p95)}
                </div>
              ...
              <div className="rounded bg-emerald-50 p-3 text-center">
                <div className="text-lg font-bold text-emerald-600">
                  {formatCurrency(data.incomePercentiles.p99)}
                </div>
                <div className="text-muted-foreground text-xs">99th percentile</div>
              </div>
              <div className="rounded bg-emerald-50 p-3 text-center">
                <div className="text-lg font-bold text-emerald-600">
                  {formatCurrency(data.incomePercentiles.p99_9)}
                </div>
                <div className="text-muted-foreground text-xs">99.9th percentile</div>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            {showAdvanced && (
              <>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="secondary">{data.urbanRuralIncomeGap.toFixed(1)}%</Badge>
                </div>
              </>
            )}
          </CardContent>
