...
import { Users, Briefcase, TrendingUp, TrendingDown, Clock, DollarSign, Shield, Activity } from 'lucide-react';
...
export function EmploymentMetrics({
  ...
  const employmentHealth = data.unemploymentRate < 5 ? 'healthy' : 
                          data.unemploymentRate < 8 ? 'moderate' : 'concerning';
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <Badge variant={employmentHealth === 'healthy' ? 'default' : employmentHealth === 'moderate' ? 'secondary' : 'destructive'}>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-yellow-600">
                    {Math.round(data.totalWorkforce * (data.underemploymentRate / 100)).toLocaleString()}
                  </span>
                </div>
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex justify-between items-center">
                ...
                <Badge variant="secondary">
                  {data.maleParticipationRate.toFixed(1)}%
                </Badge>
              </div>
            </CardContent>
          </Card>
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center p-3 bg-blue-50 rounded-lg">
                <div className="text-2xl font-bold text-blue-600">{data.employmentType.fullTime.toFixed(1)}%</div>
                <div className="text-xs text-muted-foreground mt-1">Full-Time</div>
              </div>
              <div className="text-center p-3 bg-purple-50 rounded-lg">
                <div className="text-2xl font-bold text-purple-600">{data.employmentType.partTime.toFixed(1)}%</div>
                <div className="text-xs text-muted-foreground mt-1">Part-Time</div>
              </div>
              <div className="text-center p-3 bg-orange-50 rounded-lg">
                <div className="text-2xl font-bold text-orange-600">{data.employmentType.selfEmployed.toFixed(1)}%</div>
                <div className="text-xs text-muted-foreground mt-1">Self-Employed</div>
              </div>
              <div className="text-center p-3 bg-gray-50 rounded-lg">
                <div className="text-2xl font-bold text-gray-600">{data.employmentType.informal.toFixed(1)}%</div>
                <div className="text-xs text-muted-foreground mt-1">Informal</div>
              </div>
            </div>
          </CardContent>
        </Card>
