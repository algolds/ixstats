...
export function ProductivityIndicators({
  ...
  const getCompetitivenessLevel = (score: number) => {
    if (score >= 80) return { level: 'World Leader', color: 'emerald', variant: 'default' as const };
    if (score >= 65) return { level: 'Highly Competitive', color: 'blue', variant: 'default' as const };
    if (score >= 50) return { level: 'Competitive', color: 'yellow', variant: 'secondary' as const };
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <MetricCard
          ...
          trend={data.laborProductivityGrowthRate > 2 ? 'up' : data.laborProductivityGrowthRate > 0 ? 'neutral' : 'down'}
        />
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <Badge variant={competitiveness.variant}>
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center">
                ...
                <Badge variant={data.laborProductivityGrowthRate > 2 ? 'default' : 'secondary'}>
                  {data.laborProductivityGrowthRate > 0 ? '+' : ''}{data.laborProductivityGrowthRate.toFixed(1)}%
                </Badge>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm">Multifactor Productivity</span>
                <Badge variant={data.multifactorProductivityGrowth > 1 ? 'default' : 'secondary'}>
                  {data.multifactorProductivityGrowth > 0 ? '+' : ''}{data.multifactorProductivityGrowth.toFixed(1)}%
                </Badge>
              </div>
            </div>
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <Badge variant="secondary">
                  {data.capitalProductivity.toFixed(2)}
                </Badge>
              </div>
              <div className="flex justify-between items-center">
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center p-4 bg-blue-50 rounded-lg">
                ...
                <div className="text-xs text-muted-foreground mt-1">Years of Education</div>
              </div>
              <div className="text-center p-4 bg-purple-50 rounded-lg">
                <div className="text-2xl font-bold text-purple-600">
                  {data.tertiaryEducationRate.toFixed(0)}%
                </div>
                <div className="text-xs text-muted-foreground mt-1">Tertiary Education</div>
              </div>
              <div className="text-center p-4 bg-emerald-50 rounded-lg">
                <div className="text-2xl font-bold text-emerald-600">
                  {data.skillsIndex}/100
                </div>
                <div className="text-xs text-muted-foreground mt-1">Skills Index</div>
              </div>
              <div className="text-center p-4 bg-orange-50 rounded-lg">
                <div className="text-2xl font-bold text-orange-600">
                  {data.brainDrainIndex.toFixed(0)}
                </div>
                <div className="text-xs text-muted-foreground mt-1">Brain Drain Index</div>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex justify-between items-center">
                ...
                <Badge variant="secondary">
                  {data.capitalIntensity.toFixed(2)}
                </Badge>
              </div>
              <div className="flex justify-between items-center">
