...
export function IncomeWealthDistribution({
  ...
  const [selectedView, setSelectedView] = useState<"classes" | "inequality" | "mobility">(
    "classes"
  );
  ...
  const handleClassPercentChange = (index: number, value: number) => {
    ...
    const totalOthers = newClasses.reduce(
      (sum, cls, idx) => (idx !== index ? sum + cls.populationPercent : sum),
      0
    );
  ...
  const handleWealthPercentChange = (index: number, value: number) => {
    ...
    // Calculate the total of all other percentages
    const totalOthers = newClasses.reduce(
      (sum, cls, idx) => (idx !== index ? sum + cls.wealthPercent : sum),
      0
    );
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="flex items-center text-lg font-semibold text-[var(--color-text-primary)]">
          <Scale className="mr-2 h-5 w-5 text-[var(--color-brand-primary)]" />
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        <div className="rounded-lg border border-[var(--color-border-primary)] bg-[var(--color-bg-secondary)] p-4">
          ...
          <div className={`text-xs ${inequalityRating.color}`}>{inequalityRating.label}</div>
        </div>
        ...
        <div className="rounded-lg border border-[var(--color-border-primary)] bg-[var(--color-bg-secondary)] p-4">
          ...
          <div className={`text-xs ${mobilityRating.color}`}>{mobilityRating.label}</div>
        </div>
      </div>
      ...
      {selectedView === "classes" && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            <div>
              <h4 className="text-md mb-4 font-semibold text-[var(--color-text-primary)]">
          ...
          <div className="space-y-4">
            ...
            {incomeData.economicClasses.map((cls, index) => (
              <div
                key={cls.name}
                className="rounded-lg border border-[var(--color-border-primary)] bg-[var(--color-bg-tertiary)] p-4"
              >
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center">
                      <input
                        ...
                        value={cls.populationPercent}
                        onChange={(e) =>
                          handleClassPercentChange(index, parseFloat(e.target.value))
                        }
                        className="slider mr-2 h-2 w-full cursor-pointer appearance-none rounded-lg bg-[var(--color-bg-tertiary)]"
                      />
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center">
                      <input
                        ...
                        value={cls.wealthPercent}
                        onChange={(e) =>
                          handleWealthPercentChange(index, parseFloat(e.target.value))
                        }
                        className="slider mr-2 h-2 w-full cursor-pointer appearance-none rounded-lg bg-[var(--color-bg-tertiary)]"
                      />
                      <span className="w-12 text-right text-sm font-medium text-[var(--color-text-primary)]">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              <div className="space-y-4">
                <div>
                  <label className="form-label flex items-center">
                    <Scale className="mr-2 h-4 w-4 text-[var(--color-brand-primary)]" />
                    Gini Coefficient
                  </label>
                  <div className="space-y-2">
                    <input
                      ...
                      onChange={(e) =>
                        handleInputChange("incomeInequalityGini", parseFloat(e.target.value))
                      }
              ...
              <div className="rounded-lg border border-[var(--color-border-primary)] bg-[var(--color-bg-tertiary)] p-4">
                <h5 className="mb-3 text-sm font-semibold text-[var(--color-text-primary)]">
                  Inequality Analysis
                </h5>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            <div className="space-y-4">
              <div>
                <label className="form-label flex items-center">
                  <ArrowUpRight className="mr-2 h-4 w-4 text-[var(--color-brand-primary)]" />
                ...
                <div className="space-y-2">
                  <input
                    ...
                    onChange={(e) =>
                      handleInputChange("socialMobilityIndex", parseFloat(e.target.value))
                    }
                    className="slider h-2 w-full cursor-pointer appearance-none rounded-lg bg-[var(--color-bg-tertiary)]"
                  />
                  <div className="flex justify-between text-xs text-[var(--color-text-muted)]">
              ...
              <div className="rounded-lg border border-[var(--color-border-primary)] bg-[var(--color-bg-tertiary)] p-4">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="h-2 w-32 rounded-full bg-[var(--color-bg-secondary)]">
                      <div
            ...
            <div className="rounded-lg border border-[var(--color-border-primary)] bg-[var(--color-bg-secondary)] p-4">
              ...
              <div className="space-y-4">
                <div className={`text-sm ${mobilityRating.color}`}>
                  <span className="font-medium">{mobilityRating.label}</span> - Social mobility
                  index of {incomeData.socialMobilityIndex}/100
                </div>
        ...
        <div className="flex items-start">
          ...
          <div>
            ...
            <p className="text-xs text-[var(--color-text-muted)]">
              Your society has a Gini coefficient of {incomeData.incomeInequalityGini.toFixed(2)} (
              {inequalityRating.label}) with {incomeData.povertyRate.toFixed(1)}% of the population
              living in poverty. Social mobility is {mobilityRating.label.toLowerCase()}, meaning
              {incomeData.socialMobilityIndex >= 60
