...
export function GovernmentSpending({
  ...
  const renderIcon = (iconName?: string): React.ReactElement => {
    ...
    const IconComponent: React.ElementType =
      iconName && iconMap[iconName] ? (iconMap[iconName] as React.ElementType) : MoreHorizontal;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="flex items-center text-lg font-semibold text-[var(--color-text-primary)]">
          <Building className="mr-2 h-5 w-5 text-[var(--color-brand-primary)]" />
          Government Spending
        </h3>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        <div className="rounded-lg border border-[var(--color-border-primary)] bg-[var(--color-bg-secondary)] p-4">
          ...
          <div className={`text-xs ${budgetHealth.color}`}>{budgetHealth.label}</div>
        </div>
      </div>
              ...
              <div className="space-y-3">
                {spendingData.spendingCategories
                  ...
                  .map((cat, index) => (
                    <div key={cat.category} className="flex items-center justify-between">
                      <div className="flex items-center" style={{ color: cat.color }}>
                        <div className="w-5 text-center text-xs font-medium text-[var(--color-text-muted)]">
                          {index + 1}
                        </div>
                        {renderIcon(cat.icon)}
                        <span className="text-sm text-[var(--color-text-primary)]">
        ...
        <div className="flex items-start">
          ...
          <div>
            ...
            <p className="text-xs text-[var(--color-text-muted)]">
              Your government spends {formatNumber(spendingData.totalSpending)} (
              {spendingData.spendingGDPPercent.toFixed(1)}% of GDP), with the highest allocation to{" "}
              {spendingData.spendingCategories.sort((a, b) => b.percent - a.percent)[0]?.category ??
                "Unknown"}{" "}
              (
              {(
                spendingData.spendingCategories.sort((a, b) => b.percent - a.percent)[0]?.percent ??
                0
              ).toFixed(1)}
