...
import {
  crossBuilderSynergyService,
  type CrossBuilderAnalysis,
} from "./CrossBuilderSynergyService";
...
export class EconomyIntegrationService {
  ...
  private generateEconomyBuilderFromAtomicComponents(
    ...
    const demographics = this.generateDemographicsFromComponents(
      components,
      demographicImpacts,
      population
    );
  ...
  private calculateSectorImpacts(components: EconomicComponentType[]): Record<string, number> {
    ...
    sectors.forEach((sector) => {
      impacts[sector] =
        components.reduce((sum, compType) => {
  ...
  private generateSectorsFromComponents(
    ...
    return sectors.map((sector) => ({
      ...
      technologyLevel:
        impacts[sector.id] > 1.2
          ? ("Advanced" as const)
          : impacts[sector.id] > 1.0
            ? ("Modern" as const)
  ...
  private determineEconomicStructure(components: EconomicComponentType[]) {
    ...
    let growthStrategy: "Export-Led" | "Import-Substitution" | "Balanced" | "Innovation-Driven" =
      "Balanced";
  ...
  private calculateGovernmentSpendingNeeds() {
    ...
    return {
      ...
      healthcare: totalGDP * 0.1 * spendingMultiplier,
  ...
  private detectPolicyConflicts(): string[] {
    ...
    if (
      components.includes(EconomicComponentType.FREE_MARKET_SYSTEM) &&
  ...
  private async handleComponentsChanged(components: unknown): Promise<void> {
    ...
    if (synergies.length > 0) {
      this.state.warnings.push(
        `Economic synergies detected: ${synergies.map((s) => s.description).join(", ")}`
      );
    }

    if (conflicts.length > 0) {
      this.state.warnings.push(
        `Economic conflicts detected: ${conflicts.map((c) => c.description).join(", ")}`
      );
    }
  }
   ...
   */
  private validateComponentCombination(components: EconomicComponentType[]): {
    isValid: boolean;
    message: string;
  } {
   ...
   * Validate economy builder
   */
  private validateEconomyBuilder(builder: EconomyBuilderState): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
   ...
   */
  private detectEconomicSynergies(
    components: EconomicComponentType[]
  ): Array<{ components: string[]; description: string }> {
   ...
   */
  private detectEconomicConflicts(
    components: EconomicComponentType[]
  ): Array<{ components: string[]; description: string }> {
   ...
   */
  async saveEconomyConfiguration(
    countryId: string,
    trpcClient: any
  ): Promise<{ success: boolean; countryId?: string; error?: string }> {
    try {
      ...
      const economyBuilderState = {
        ...
        selectedAtomicComponents:
          this.state.economyBuilder.selectedAtomicComponents || this.state.selectedAtomicComponents,
        lastUpdated: new Date(),
   ...
   */
  async syncWithGovernmentComponents(
    countryId: string,
    trpcClient: any,
