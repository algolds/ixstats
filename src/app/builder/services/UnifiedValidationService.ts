...
export class UnifiedValidationService {
    ...
    for (let i = 0; i < components.length; i++) {
      for (let j = i + 1; j < components.length; j++) {
        ...
        if (componentADef?.conflicts.includes(componentB)) {
          conflicts.push(
            `${componentADef.name} conflicts with ${componentBDef?.name || componentB}`
          );
        }

        if (componentADef?.synergies.includes(componentB)) {
          synergies.push(
            `${componentADef.name} synergizes with ${componentBDef?.name || componentB}`
          );
        }
      }
    }
    ...
    return {
      ...
      suggestions:
        conflicts.length > 0
          ...
          : ["Optimize component synergies", "Consider adding more synergistic components"],
      impact: {
  ...
  private validateEconomyGovernmentAlignment(context: ValidationContext): ValidationResult {
    ...
    const severity =
      alignmentIssues.length > 1 ? "high" : alignmentIssues.length > 0 ? "medium" : "low";
    ...
    return {
      ...
      suggestions:
        alignmentIssues.length > 0
          ...
          : ["Strengthen existing alignments", "Add more aligned components"],
      impact: {
  ...
  private validateEconomyTaxConsistency(context: ValidationContext): ValidationResult {
    ...
    for (const econComponent of economicComponents) {
      ...
      const corporateTax = taxSystem.taxCategories?.find((cat) =>
        cat.categoryName.includes("Corporate")
      );
      const incomeTax = taxSystem.taxCategories?.find((cat) => cat.categoryName.includes("Income"));

      if (
        corporateTax &&
        corporateTax.baseRate &&
        Math.abs(corporateTax.baseRate - optimalCorporateRate) > 10
      ) {
        ...
        consistencies.push(`Corporate tax rate aligns well with ${econDef.name} requirements`);
      }

      if (
        incomeTax &&
        incomeTax.baseRate &&
        Math.abs(incomeTax.baseRate - optimalIncomeRate) > 15
      ) {
        consistencyIssues.push(
          `Income tax rate (${incomeTax.baseRate}%) differs significantly from optimal rate for ${econDef.name} (${optimalIncomeRate}%)`
        );
      } else if (incomeTax) {
        consistencies.push(`Income tax rate aligns well with ${econDef.name} requirements`);
      }
    }

    const passed = consistencyIssues.length === 0;
    const severity =
      consistencyIssues.length > 2 ? "high" : consistencyIssues.length > 0 ? "medium" : "low";
    ...
    return {
      ...
      suggestions:
        consistencyIssues.length > 0
          ...
          : ["Maintain current tax-economic alignment", "Monitor tax efficiency metrics"],
      impact: {
  ...
  private validateGovernmentComponentFeasibility(context: ValidationContext): ValidationResult {
    ...
    const severity =
      feasibilityIssues.length > 2 ? "high" : feasibilityIssues.length > 0 ? "medium" : "low";
    ...
    return {
      ...
      suggestions:
        feasibilityIssues.length > 0
          ...
          : ["Proceed with implementation planning", "Develop detailed implementation roadmap"],
      impact: {
  ...
  private validateGovernmentCapacityRequirements(context: ValidationContext): ValidationResult {
    ...
    const availableCapacity = governmentBuilder?.structure?.totalBudget
      ? Math.min(governmentBuilder.structure.totalBudget / 1000000, 100)
      : 50; // Simplified capacity calculation
    ...
    return {
      ...
      suggestions: passed
        ? ["Capacity is adequate for implementation", "Consider adding more components"]
        : [
  ...
  private validateTaxRevenueSustainability(context: ValidationContext): ValidationResult {
    ...
    const requiredSpending = this.estimateRequiredGovernmentSpending(
      economyBuilder,
    ...
    return {
      ...
      suggestions: passed
        ? ["Revenue appears sustainable", "Monitor revenue collection efficiency"]
        : [
            "Increase tax rates",
  ...
  private validateTaxEconomicImpact(context: ValidationContext): ValidationResult {
    ...
    const totalTaxRate =
      taxSystem.taxCategories?.reduce((sum, cat) => sum + (cat.baseRate || 0), 0) || 0;
    ...
    if (avgTaxRate > 40) {
      ...
      positives.push(
        `Average tax rate (${avgTaxRate.toFixed(1)}%) is in optimal range for balanced growth`
      );
    }
    ...
    return {
      ...
      suggestions:
        issues.length > 0
          ...
          : ["Maintain current tax-economic alignment", "Monitor tax efficiency and compliance"],
      impact: {
  ...
  private validateCrossSystemSynergy(context: ValidationContext): ValidationResult {
    ...
    if (
      economyBuilder.selectedAtomicComponents.includes(EconomicComponentType.FREE_MARKET_SYSTEM) &&
    ...
    if (
      economyBuilder.selectedAtomicComponents.includes(
        EconomicComponentType.SOCIAL_MARKET_ECONOMY
      ) &&
    ...
    const severity: "low" | "medium" | "high" =
      missed.length > 2 ? "high" : missed.length > 0 ? "medium" : "low";
    ...
    return {
      ...
      suggestions:
        missed.length > 0
          ...
          : ["Excellent synergy optimization", "Maintain current system alignment"],
      impact: {
  ...
  private validatePolicyCoherence(context: ValidationContext): ValidationResult {
    ...
    const hasProtectedWorkers = economyBuilder.selectedAtomicComponents.includes(
    ...
    if (hasSocialMarket && hasProtectedWorkers && hasSocialDemocracy) {
      coherent.push(
        "Social market economy with worker protections and social democracy is highly coherent"
      );
    }
    ...
    const severity: "low" | "medium" | "high" =
      incoherent.length > 2 ? "high" : incoherent.length > 0 ? "medium" : "low";
    ...
    return {
      ...
      suggestions:
        incoherent.length > 0
          ...
          : ["Maintain excellent policy coherence", "Continue coherent policy development"],
      impact: {
  ...
  private validateImplementationComplexity(context: ValidationContext): ValidationResult {
    ...
    if (economicComponents > 8) {
      complexityFactors.push(
        `High number of economic components (${economicComponents}) increases complexity`
      );
    ...
    if (govComponents > 10) {
      complexityFactors.push(
        `High number of government components (${govComponents}) increases complexity`
      );
    ...
    if (taxCategories > 7) {
      complexityFactors.push(
        `Complex tax structure (${taxCategories} categories) may reduce efficiency`
      );
    ...
    const severity: "low" | "medium" | "high" =
      complexityScore > thresholds.high
        ? "high"
        : complexityScore > thresholds.medium
          ? "medium"
          : "low";
    ...
    return {
      ...
      suggestions: !passed
        ...
        : ["Complexity is well-managed", "Current structure is sustainable"],
      impact: {
  ...
  private validateEconomicGrowthPotential(context: ValidationContext): ValidationResult {
    ...
    const severity: "low" | "medium" | "high" =
      growthScore < 50 ? "high" : growthScore < 70 ? "medium" : "low";
    ...
    return {
      ...
      suggestions: !passed
        ...
        : ["Strong growth potential established", "Maintain growth-supporting policies"],
      impact: {
  ...
  private validateSocialEquityBalance(context: ValidationContext): ValidationResult {
    ...
    const hasGrowthComponents = economyBuilder.selectedAtomicComponents.some((c) =>
      [EconomicComponentType.INNOVATION_ECONOMY, EconomicComponentType.FREE_MARKET_SYSTEM].includes(
        c
      )
    );
    ...
    if (equityScore > 80 && !hasGrowthComponents) {
      equityFactors.push(
        "Warning: High equity focus without growth components may limit economic dynamism"
      );
    }
    ...
    const severity: "low" | "medium" | "high" =
      equityScore < 40 ? "high" : equityScore < 60 ? "medium" : "low";
    ...
    return {
      ...
      suggestions: !passed
        ...
        : ["Good equity-growth balance achieved", "Maintain current social policies"],
      impact: {
  ...
  private validateEnvironmentalSustainability(context: ValidationContext): ValidationResult {
    ...
    if (
      economyBuilder.selectedAtomicComponents.includes(EconomicComponentType.EXTRACTION_FOCUSED)
    ) {
      sustainabilityScore -= 15;
    ...
    if (
      economyBuilder.selectedAtomicComponents.includes(EconomicComponentType.MANUFACTURING_LED) &&
    ...
    const severity: "low" | "medium" | "high" =
      sustainabilityScore < 40 ? "high" : sustainabilityScore < 60 ? "medium" : "low";
    ...
    return {
      ...
      suggestions: !passed
        ...
        : ["Good environmental sustainability established", "Continue green economy development"],
      impact: {
  ...
  private getEconomicComponentDefinition(componentType: EconomicComponentType) {
    try {
      ...
      } = require("~/components/economy/atoms/AtomicEconomicComponents");
  ...
  private generateRecommendations(results: ValidationResult[]): string[] {
    ...
    recommendations.push(
      "Consider implementing suggested improvements for better system performance"
    );
  ...
  private calculateSystemHealth(results: ValidationResult[]): {
    economy: number;
    government: number;
    tax: number;
    overall: number;
  ...
  private calculateHealthScore(results: ValidationResult[]): number {
    ...
    const totalImpact = results.reduce((sum, result) => {
      const impact =
        result.impact.economic ||
        result.impact.government ||
        result.impact.tax ||
        result.impact.overall ||
        0;
  ...
  private calculateConsistencyScore(results: ValidationResult[]): number {
    const consistencyResults = results.filter(
      (r) => r.ruleId.includes("consistency") || r.ruleId.includes("alignment")
    );
    return this.calculateHealthScore(consistencyResults);
  }

  private calculateFeasibilityScore(results: ValidationResult[]): number {
    const feasibilityResults = results.filter(
      (r) => r.ruleId.includes("feasibility") || r.ruleId.includes("capacity")
    );
    return this.calculateHealthScore(feasibilityResults);
  }

  private calculateCompatibilityScore(results: ValidationResult[]): number {
    const compatibilityResults = results.filter(
      (r) => r.ruleId.includes("compatibility") || r.ruleId.includes("synergy")
    );
