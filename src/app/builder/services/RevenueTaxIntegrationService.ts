...
export class RevenueTaxIntegrationService {
  ...
  public readonly REVENUE_TAX_MAPPINGS: RevenueTaxMapping[] = [
    {
      ...
      defaultRate: 15,
      brackets: [
        { minIncome: 0, maxIncome: 15000, rate: 0, marginalRate: true, isActive: true, priority: 1 },
        { minIncome: 15000, maxIncome: 40000, rate: 10, marginalRate: true, isActive: true, priority: 2 },
        { minIncome: 40000, maxIncome: 85000, rate: 20, marginalRate: true, isActive: true, priority: 3 },
    ...
    },
    {
      ...
      defaultRate: 10,
      brackets: [
        { minIncome: 0, rate: 10, marginalRate: false, isActive: true, priority: 1 }
      ]
    },
    {
    ...
    revenueSources.forEach((revenue, index) => {
      if (this.isTaxRelated(revenue)) {
        ...
        const taxCategory: TaxCategoryInput = {
          ...
          calculationMethod: (collectionMethod?.calculationMethod || mapping.calculationMethod) as 'fixed' | 'percentage' | 'progressive' | 'tiered',
          minimumAmount: 0,
   ...
   */
  public taxCategoriesToRevenueSources(taxCategories: TaxCategoryInput[], totalRevenue: number): RevenueSourceInput[] {
  ...
  public syncRevenueWithTax(
    ...
    const newTaxCategories = revenueSources
      .filter(rs => !taxCategories.some(tc => tc.categoryName === rs.name) && this.isTaxRelated(rs))
      .map(rs => {
  ...
  private getMappingForRevenueCategory(category: RevenueCategory): RevenueTaxMapping {
    return this.REVENUE_TAX_MAPPINGS.find(m => m.revenueCategory === category) || 
  ...
  private getCollectionMethodByTaxCategory(taxCategory: TaxCategoryInput): string {
    const method = this.COLLECTION_METHODS.find(m => 
      m.taxCategoryType === taxCategory.categoryType && m.isTaxRelated
    );
    return method?.id || 'self_assessment';
  }

  private getDefaultCollectionMethod(taxCategoryType: string): CollectionMethod {
    return this.COLLECTION_METHODS.find(m => 
      m.taxCategoryType === taxCategoryType && m.isTaxRelated
