...
export class IntegrationTestingService {
  ...
  private async runSynergyTests(context: IntegrationTestContext): Promise<IntegrationTestSuite> {
    ...
    return {
      ...
      description:
        "Tests for synergy detection and analysis across economy, government, and tax systems",
      tests,
  ...
  private async runSyncTests(context: IntegrationTestContext): Promise<IntegrationTestSuite> {
    ...
    return {
      ...
      description:
        "Tests for bidirectional synchronization between economy, government, and tax systems",
      tests,
      overallPassRate: (passedTests / results.length) * 100,
      executionTime,
      results,
  ...
  private initializeTests(): void {
    ...
    this.addTest({
      ...
      testFunction: async (context) => {
        ...
        try {
          ...
          const impact = this.services.govSyncService.getGovernmentImpactOfEconomy(
            context.economyBuilder
          );
          const executionTime = Date.now() - startTime;
    ...
    this.addTest({
      ...
      testFunction: async (context) => {
        ...
        try {
          const analysis =
            await this.services.effectivenessCalculator.calculateUnifiedEffectiveness(
              context.economyBuilder,
              context.governmentBuilder,
              context.taxSystem,
      ...
      testFunction: async (context) => {
        ...
        try {
          ...
          const overallSuccess =
            synergyAnalysis.overallCrossBuilderScore > 60 &&
          ...
          return {
            ...
            warnings:
              synergyAnalysis.overallCrossBuilderScore < 80
                ? ["Synergy score could be improved"]
                : [],
            metrics: {
  ...
  private calculatePerformanceMetrics(testSuites: IntegrationTestSuite[]): {
    ...
    const averageTestTime =
      allResults.reduce((sum, result) => sum + result.executionTime, 0) / allResults.length;

    const slowestResult = allResults.reduce(
      (slowest, current) => (current.executionTime > slowest.executionTime ? current : slowest),
      allResults[0] || { executionTime: 0, testId: "none" }
    );

    const fastestResult = allResults.reduce(
      (fastest, current) => (current.executionTime < fastest.executionTime ? current : fastest),
      allResults[0] || { executionTime: 0, testId: "none" }
    );
