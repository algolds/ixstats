...
function findMinisterForDepartment(
  content: string,
  deptName: string
): { minister?: string; evidence?: string } {
...
export function parseDepartments(pages: { title: string; content: string }[]): ParsedDepartment[] {
  ...
  while ((match = ministryPattern.exec(combinedContent)) !== null) {
    ...
    const confidence =
      match[0].toLowerCase().includes("ministry of") ||
      match[0].toLowerCase().includes("department of")
        ? 90
        : 60;
    ...
    const { minister, evidence: ministerEvidence } = findMinisterForDepartment(
      combinedContent,
      normalizedName
    );
