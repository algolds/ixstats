...
export function extractDataFromWikiSections(sections: { title: string; content: string }[]): ExtractedBuilderData {
  ...
  for (const section of sections) {
    ...
    if (lowerTitle.includes('economy') || lowerTitle.includes('economic')) {
      let conf = 0;
      const economy: NonNullable<ExtractedBuilderData['economy']> = { confidence: 0, majorIndustries: [] };
      
      // GDP Nominal
      const gdpMatch = content.match(/GDP\s*(?:nominal)?\s*(?:is|of|stood at|reached|was)?\s*(?:around|approximately)?\s*(?:[\$ۣ])?\s*([\d,\.]+\s*(?:trillion|billion|million)?)/i);
      if (gdpMatch && gdpMatch[1]) {
        const val = parseNumericValue(gdpMatch[1]);
        if (val) { economy.gdpNominal = val; conf += 30; }
      }
      
      // GDP Per Capita
      const perCapitaMatch = content.match(/per capita\s*(?:is|of|stood at)?\s*(?:[\$ۣ])?\s*([\d,\.]+)/i);
      if (perCapitaMatch && perCapitaMatch[1]) {
        const val = parseNumericValue(perCapitaMatch[1]);
        if (val) { economy.gdpPerCapita = val; conf += 20; }
      }
      
      // Inflation
      const inflationMatch = content.match(/inflation\s*(?:rate)?\s*(?:is|of|stood at)?\s*([\d,\.]+)\s*%/i);
      if (inflationMatch && inflationMatch[1]) {
        const val = parseFloat(inflationMatch[1]);
        if (!isNaN(val)) { economy.inflationRate = val; conf += 15; }
      }
      
      // Unemployment
      const unemploymentMatch = content.match(/unemployment\s*(?:rate)?\s*(?:is|of|stood at)?\s*([\d,\.]+)\s*%/i);
      if (unemploymentMatch && unemploymentMatch[1]) {
        const val = parseFloat(unemploymentMatch[1]);
        if (!isNaN(val)) { economy.unemploymentRate = val; conf += 15; }
      }
      
      // Industries
      const industriesMatch = content.match(/(?:major|primary|key) industries (?:include|are) ([^\.]+)\./i);
      ...
      // Economic system
      const econSystemMatch = content.match(/(mixed economy|free market|market economy|planned economy|state capitalism|social market economy)/i);
      ...
      // Central bank
      const centralBankMatch = content.match(/(?:central bank|reserve bank|monetary authority) (?:is |called |known as )?(?:the )?([A-Z][a-zA-Z\s]+?Bank[^,\.]*|[A-Z][a-zA-Z\s]+?Reserve[^,\.]*)/i);
      ...
      const soeMatch = content.match(/state-owned enterprises|nationalized industries|public sector (?:dominates|controls|owns)/i);
      economy.hasStateOwnedEnterprises = !!soeMatch;
      if (economy.hasStateOwnedEnterprises) conf += 10;
      
      // Free trade zones
      const ftzMatch = content.match(/free trade zone|special economic zone|export processing zone/i);
      ...
      // Trade partners
      const tradeMatch = content.match(/(?:major|primary|key) trade partners (?:include|are) ([^\.]+)/i);
      ...
      // Welfare programs
      const welfareMatch = content.match(/universal healthcare|national health service|free education|welfare state|social safety net/i);
      economy.hasWelfarePrograms = !!welfareMatch;
      if (economy.hasWelfarePrograms) conf += 10;
      
      // Universal healthcare
      const healthcareMatch = content.match(/universal healthcare|national health service|publicly funded healthcare/i);
      economy.hasUniversalHealthcare = !!healthcareMatch;
      if (economy.hasUniversalHealthcare) conf += 5;
      
      // Public education
      const educationMatch = content.match(/free education|public education|universal education|state-funded education/i);
    ...
    if (lowerTitle.includes('government') || lowerTitle.includes('politics')) {
      let conf = 0;
      const gov: NonNullable<ExtractedBuilderData['government']> = { confidence: 0, departments: [] };
      
      // Government Type
      const typeMatch = content.match(/is a (federal republic|constitutional monarchy|unitary republic|dictatorship|theocracy|absolute monarchy)[,\.]/i);
      ...
      // Legislature
      const legMatch = content.match(/(?:legislature|parliament) is (?:the|called the)? ([A-Z][a-zA-Z\s]+?)(?:[,\.]| and)/);
      ...
      const deptRegex = /(?:ministry|department|secretariat|bureau)\s+(?:of\s+)?([A-Z][a-zA-Z\s]+?)(?:,|\.|and|;|$)/gi;
      let deptMatch;
      ...
      const ministerRegex = /(?:minister|secretary|head)\s+(?:of\s+)?(?:the\s+)?([A-Z][a-zA-Z\s]+?)\s*(?:is|was|:)\s*([A-Z][a-zA-Z\s.]+)/gi;
      let minMatch;
      ...
      // Judicial system
      const judicialMatch = content.match(/(?:has |features |maintains )?(an? )?(independent judiciary|supreme court|constitutional court|federal court system|judicial branch)[,\.]?/i);
      ...
      // Electoral system
      const electoralMatch = content.match(/(?:uses |employs |operates under )?(proportional representation|first-past-the-post|single transferable vote|mixed member proportional|ranked choice voting|two-round system)[,\.]?/i);
      ...
      // Constitution
      const constitutionMatch = content.match(/(?:has |governed by |operates under )(a |an )?(written constitution|codified constitution|uncodified constitution|constitutional framework)/i);
      gov.hasConstitution = !!constitutionMatch;
      if (gov.hasConstitution) conf += 10;
      
      // Political parties
      const partiesMatch = content.match(/(?:political parties|major parties|leading parties) (?:include|are) ([^\.]+)/i);
      ...
      const sepPowersMatch = content.match(/separation of powers|checks and balances|three branches of government/i);
      gov.hasSeparationOfPowers = !!sepPowersMatch;
      if (gov.hasSeparationOfPowers) conf += 10;
      
      // Federal structure
      const federalMatch = content.match(/federal structure|federal system|federation of|states and territories|provincial governments/i);
      gov.hasFederalStructure = !!federalMatch;
      if (gov.hasFederalStructure) conf += 10;
      
      // Local government
      const localGovMatch = content.match(/local government|municipal authorities|local councils|regional assemblies/i);
    ...
    if (lowerTitle.includes('demographic') || lowerTitle.includes('population')) {
      ...
      const popMatch = content.match(/population (?:of|is|was|estimated at) ([\d,\.]+\s*(?:million|billion|thousand)?)/i);
      if (popMatch && popMatch[1]) {
        const val = parseNumericValue(popMatch[1]);
        if (val) { demo.population = val; conf += 40; }
      }
      
      const lifeMatch = content.match(/life expectancy (?:is|of) ([\d,\.]+)/i);
      if (lifeMatch && lifeMatch[1]) {
        const val = parseFloat(lifeMatch[1]);
        if (!isNaN(val)) { demo.lifeExpectancy = val; conf += 20; }
      }
