...
export function generateDefaultTradeData(nominalGDP: number, gdpPerCapita: number): TradeData {
  const isAdvanced = gdpPerCapita > 35000;
  ...
  return {
    ...
    tradingPartners: [
      {
        country: "Partner A",
        exportsTo: totalExports * 0.22,
        importsFrom: totalImports * 0.18,
        tradeBalance: totalExports * 0.22 - totalImports * 0.18,
      },
      {
        country: "Partner B",
        exportsTo: totalExports * 0.18,
        importsFrom: totalImports * 0.25,
        tradeBalance: totalExports * 0.18 - totalImports * 0.25,
      },
      {
        country: "Partner C",
        exportsTo: totalExports * 0.15,
        importsFrom: totalImports * 0.15,
        tradeBalance: 0,
      },
    ],
...
export function calculateSustainabilityScore(data: Partial<ComprehensiveEconomyData>): number {
  ...
  const productivityGrowth = Math.min(
    100,
    (data.productivity.laborProductivityGrowthRate + 1) * 25
  );
