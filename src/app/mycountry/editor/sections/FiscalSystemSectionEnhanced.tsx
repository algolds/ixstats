...
export function FiscalSystemSectionEnhanced({
  ...
  const debtStatus =
    fiscal.totalDebtGDPRatio > 100
      ? "critical"
      : fiscal.totalDebtGDPRatio > 60
        ? "warning"
        : "healthy";
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Badge
          ...
          className="px-3 py-1"
        >
          {budgetStatus === "surplus" ? "Surplus" : "Deficit"}:{" "}
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-muted-foreground text-xs">Tax Revenue</p>
                <p className="text-muted-foreground text-xs">{formatCurrency(taxRevenue)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">
                  {fiscal.governmentBudgetGDPPercent.toFixed(1)}%
                </p>
                <p className="text-muted-foreground text-xs">Gov Budget</p>
                <p className="text-muted-foreground text-xs">{formatCurrency(govBudget)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="glass-surface">
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              <div
                className={`rounded-lg p-2 ${budgetStatus === "surplus" ? "bg-green-100 dark:bg-green-900/30" : "bg-red-100 dark:bg-red-900/30"}`}
              >
              ...
              <div>
                ...
                <p className="text-muted-foreground text-xs">{formatCurrency(budgetBalance)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="glass-surface">
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              <div
                className={`rounded-lg p-2 ${debtStatus === "critical" ? "bg-red-100 dark:bg-red-900/30" : debtStatus === "warning" ? "bg-amber-100 dark:bg-amber-900/30" : "bg-green-100 dark:bg-green-900/30"}`}
              >
                <AlertTriangle
                  className={`h-5 w-5 ${debtStatus === "critical" ? "text-red-600 dark:text-red-400" : debtStatus === "warning" ? "text-amber-600 dark:text-amber-400" : "text-green-600 dark:text-green-400"}`}
                />
              </div>
              <div>
                <p className="text-2xl font-bold">{fiscal.totalDebtGDPRatio.toFixed(1)}%</p>
                <p className="text-muted-foreground text-xs">Debt/GDP</p>
                <p className="text-muted-foreground text-xs">{formatCurrency(totalDebt)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="overview" className="mt-6 space-y-6">
          <Card className="glass-surface">
            <CardHeader>
              ...
              <CardDescription>
                Total government revenue as % of GDP. This is the high-level revenue mix; detailed
                tax rules are configured under the Tax System tab.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <EnhancedSlider
              ...
              <Alert className="mt-4">
                ...
                <AlertDescription>
                  {fiscal.taxRevenueGDPPercent < 20 &&
                    "Low tax revenue - limited government services"}
                  {fiscal.taxRevenueGDPPercent >= 20 &&
                    fiscal.taxRevenueGDPPercent < 35 &&
                    "Moderate tax revenue - balanced approach"}
                  {fiscal.taxRevenueGDPPercent >= 35 &&
                    "High tax revenue - extensive government services"}
                </AlertDescription>
              </Alert>
            </CardContent>
          </Card>
          ...
          <Card className="glass-surface">
            <CardHeader>
              ...
              <CardDescription>
                Total government spending as % of GDP. The allocations to departments are managed in
                the Government builder.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <EnhancedSlider
              ...
              {budgetBalance < 0 && (
                <Alert className="mt-4" variant="destructive">
                  ...
                  <AlertDescription>
                    Budget deficit of {formatCurrency(Math.abs(budgetBalance))} will increase
                    national debt
                  </AlertDescription>
                </Alert>
              )}
            ...
            <CardContent>
              ...
              >
                ...
                <AlertDescription>
                  ...
                  {debtStatus === "warning" &&
                    "Elevated debt levels - monitor fiscal sustainability"}
                  {debtStatus === "critical" &&
                    "Critical debt levels - urgent fiscal reforms needed"}
                </AlertDescription>
              </Alert>
            </CardContent>
          </Card>
          ...
          <Card className="glass-surface">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <p className="text-muted-foreground text-sm">Per Capita Debt</p>
                  <p className="text-xl font-bold">{formatCurrency(perCapitaDebt)}</p>
                </div>
                <div>
                  <p className="text-muted-foreground text-sm">Debt/Revenue Ratio</p>
                  <p className="text-xl font-bold">{(totalDebt / taxRevenue).toFixed(2)}x</p>
                </div>
                <div>
                  <p className="text-muted-foreground text-sm">Annual Deficit</p>
        ...
        {/* Taxation Tab */}
        <TabsContent value="taxation" className="mt-6 space-y-6">
          <Card className="glass-surface">
            <CardHeader>
              ...
              <CardDescription>
                Design your comprehensive tax structure with brackets, exemptions, and deductions.
                These mechanics feed into your overall revenue but are distinct from the simple
                revenue % slider above.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <TaxBuilder
