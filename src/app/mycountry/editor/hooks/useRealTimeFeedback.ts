...
export function useRealTimeFeedback(
  ...
  const calculateFeedback = useCallback(async (inputs: EconomicInputs, original: EconomicInputs | null) => {
    ...
    const gdpChangePercent = originalGdpPerCapita > 0 ? (gdpChange / originalGdpPerCapita) * 100 : 0;
    ...
    const originalUnemploymentRate = original?.laborEmployment.unemploymentRate || unemploymentRate;
    ...
    fiscalHealth.push({
      ...
      status: budgetBalance >= 0 ? 'good' : budgetBalance >= -inputs.coreIndicators.nominalGDP * 0.03 ? 'warning' : 'danger',
      unit: '$'
    });
    ...
    if (gdpPerCapita < 25000) {
      recommendations.push("Consider increasing investment in education and infrastructure to boost GDP per capita");
    }
    if (growthRate < 2) {
      recommendations.push("Economic growth is below healthy levels - review fiscal and monetary policies");
    }
    ...
    if (budgetBalance < -inputs.coreIndicators.nominalGDP * 0.05) {
      recommendations.push("Large budget deficit - consider reducing spending or increasing revenue");
    }
