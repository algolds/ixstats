...
export function useCountryEditorData() {
  ...
  const [econSaveState, setEconSaveState] = useState<{
    isSaving: boolean;
    lastSavedAt: Date | null;
    pendingChanges: boolean;
    error: Error | null;
  }>({ isSaving: false, lastSavedAt: null, pendingChanges: false, error: null });
  ...
  useEffect(() => {
    if (country && !economicInputs) {
      ...
      if (isNaN(inputs.fiscalSystem.taxRevenueGDPPercent))
        inputs.fiscalSystem.taxRevenueGDPPercent = 0;
      if (isNaN(inputs.fiscalSystem.governmentBudgetGDPPercent))
        inputs.fiscalSystem.governmentBudgetGDPPercent = 0;
      if (isNaN(inputs.fiscalSystem.totalDebtGDPRatio)) inputs.fiscalSystem.totalDebtGDPRatio = 0;
      if (isNaN(inputs.fiscalSystem.budgetDeficitSurplus))
        inputs.fiscalSystem.budgetDeficitSurplus = 0;
      if (isNaN(inputs.fiscalSystem.governmentRevenueTotal))
        inputs.fiscalSystem.governmentRevenueTotal = 0;
  ...
  useEffect(() => {
    if (country && "flag" in country && country.flag) {
      // Type-safe check for flag property
  ...
  useEffect(() => {
    if (existingGovernment && !governmentData && country) {
      ...
      const departmentIdToIndex = new Map<string, string>(
        (existingGovernment.departments as any[]).map((d: any, idx: number) => [
          d.id,
          idx.toString(),
        ])
      );
        ...
        departments: (existingGovernment.departments as any[]).map((dept: any) => ({
          ...
          functions: Array.isArray(dept.functions)
            ...
            : typeof dept.functions === "string"
              ? (() => {
                  try {
                    const parsed = JSON.parse(dept.functions as string);
  ...
  const handleInputsChange = (newInputs: EconomicInputs) => {
    if (
      !newInputs ||
      !newInputs.coreIndicators ||
      !newInputs.laborEmployment ||
      !newInputs.fiscalSystem ||
      !newInputs.governmentSpending ||
      !newInputs.demographics
    ) {
  ...
  useEffect(() => {
    ...
    const changed =
      JSON.stringify(economicInputs.laborEmployment) !==
        JSON.stringify(originalInputs.laborEmployment) ||
      ...
      JSON.stringify(economicInputs.governmentSpending) !==
        JSON.stringify(originalInputs.governmentSpending) ||
      economicInputs.coreIndicators.realGDPGrowthRate !==
        originalInputs.coreIndicators.realGDPGrowthRate ||
      economicInputs.coreIndicators.inflationRate !== originalInputs.coreIndicators.inflationRate ||
      economicInputs.coreIndicators.currencyExchangeRate !==
        originalInputs.coreIndicators.currencyExchangeRate;
    ...
    econDebounceRef.current = setTimeout(async () => {
      try {
        ...
        const economicData = {
          ...
          totalWorkforce: Math.round(
            economicInputs.coreIndicators.totalPopulation *
              (economicInputs.laborEmployment.laborForceParticipationRate / 100)
          ),
          ...
          averageAnnualIncome:
            (economicInputs.laborEmployment.minimumWage || 15) *
            (economicInputs.laborEmployment.averageWorkweekHours || 40) *
            52,
          ...
          taxRevenuePerCapita:
            economicInputs.coreIndicators.totalPopulation > 0
              ? economicInputs.fiscalSystem.governmentRevenueTotal /
                economicInputs.coreIndicators.totalPopulation
              : 0,
          ...
          debtPerCapita:
            economicInputs.coreIndicators.totalPopulation > 0
              ? ((economicInputs.fiscalSystem.totalDebtGDPRatio / 100) *
        ...
        setEconSaveState((prev) => ({
          ...prev,
          isSaving: false,
          error: err instanceof Error ? err : new Error("Autosave failed"),
        }));
      }
    }, 2000);
  ...
  const validateInputs = (inputs: EconomicInputs) => {
    ...
    if (realTimeValidation) {
      if (inputs.fiscalSystem.budgetDeficitSurplus < -inputs.coreIndicators.nominalGDP * 0.2) {
        newErrors.push({
          field: "budgetDeficit",
          message: "Large budget deficit may impact stability",
          severity: "warning",
        });
      }
    }
  ...
  return {
    ...
    existingGovernment,
    governmentLoading,
    createGovernmentMutation,
    updateGovernmentMutation,
    handleInputsChange,
    validateInputs,
    handleSave,
    handleReset,
    ...
    warningCount,
    econSaveState,
    populationTier: economicInputs?.coreIndicators?.totalPopulation
      ? calculatePopulationTier(economicInputs.coreIndicators.totalPopulation)
      : "Unknown", // Add populationTier
  };
}
