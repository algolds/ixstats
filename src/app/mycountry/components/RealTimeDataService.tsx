...
export function RealTimeDataService({
  ...
  const analyzeDataChanges = useCallback(
    (current: DataSnapshot, previous: DataSnapshot) => {
      const changes = [];
      const SIGNIFICANT_THRESHOLD = 0.05; // 5% change threshold
      const CRITICAL_THRESHOLD = 0.1; // 10% change threshold
      ...
      if (Math.abs(vitalityChange) > 5) {
        // 5 point change in vitality
        addNotification({
