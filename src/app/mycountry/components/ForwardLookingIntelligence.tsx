...
const generatePredictiveInsights = (vitalityData: VitalityIntelligence[]): PredictiveInsight[] => {
  ...
  for (const vitality of vitalityData) {
    const shortTermForecast = vitality.forecast?.shortTerm ?? {
      projected: 0,
      confidence: 0,
      factors: [],
    };
    const longTermForecast = vitality.forecast?.longTerm ?? {
      projected: 0,
      confidence: 0,
      factors: [],
    };
    ...
    if ((shortTermForecast.projected ?? 0) !== (vitality.score ?? 0)) {
      ...
      insights.push({
        ...
        relevanceScore: Math.min(
          100,
          50 + Math.abs(difference ?? 0) * 5 + (shortTermForecast.confidence ?? 0) / 2
        ),
      });
    }
    ...
    if (Math.abs((longTermForecast.projected ?? 0) - (vitality.score ?? 0)) > 8) {
      ...
      insights.push({
        ...
        relevanceScore: Math.min(
          100,
          40 + Math.abs(difference ?? 0) * 3 + (longTermForecast.confidence ?? 0) / 3
        ),
      });
    }
  }
  ...
  for (const vitality of vitalityData) {
    if ((vitality.score ?? 0) < 80) {
      milestones.push({
        ...
        status:
          vitality.trend === "up" ? "on_track" : vitality.trend === "down" ? "at_risk" : "on_track",
        requirements: [
...
function PredictiveInsightCard({
  ...
  return (
    <motion.div
      ...
      className={`border-border cursor-pointer rounded-lg border p-3 transition-all hover:shadow-sm ${impactConf.bg}`}
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-1 flex items-center gap-2">
            <h4 className="line-clamp-1 text-sm font-medium">{insight.title}</h4>
            <ImpactIcon className={`h-3 w-3 ${impactConf.color}`} />
          </div>

          <p className="text-muted-foreground mb-2 line-clamp-2 text-xs">{insight.description}</p>

          <div className="flex items-center gap-2 text-xs">
            <Badge variant="outline" className="px-1 py-0">
              {horizonConf.label}
            </Badge>
            <span className="text-muted-foreground">{insight.probability}% probability</span>
            <span className="text-muted-foreground">•</span>
            <span className={`capitalize ${impactConf.color}`}>{insight.magnitude} impact</span>
          </div>
        </div>
        ...
        {expanded && (
          ...
          >
            {insight.implications.length > 0 && (
              <div>
                <div className="mb-1 text-xs font-medium">Key Implications:</div>
  ...
  return (
    <div className="border-border bg-card rounded-lg border p-3">
      <div className="mb-3 flex items-start gap-3">
        <AreaIcon className="text-muted-foreground mt-0.5 h-4 w-4" />
      ...
      {milestone.blockers.length > 0 && (
        <div className="border-border mt-2 border-t pt-2">
          <div className="mb-1 text-xs font-medium text-red-600">Blockers:</div>
...
export function ForwardLookingIntelligence({
  ...
  const [activeTab, setActiveTab] = useState<"predictions" | "milestones" | "competitive">(
    "predictions"
  );
  ...
  const milestones = useMemo(() => generateMilestoneTracking(vitalityData), [vitalityData]);
            ...
            <TabsContent value="milestones" className="mt-0">
              <div className="space-y-3">
                {milestones.map((milestone, index) => (
                  <MilestoneCard
                    key={
                      milestone.id && milestone.id.trim()
                        ? `milestone-${milestone.id.trim()}`
                        : `milestone-fallback-${index}`
                    }
                    milestone={milestone}
              ...
              <div className="space-y-3">
                <div className="border-border bg-card rounded-lg border p-3">
                  <div className="mb-2 flex items-center gap-2">
                    ...
                    <h4 className="text-sm font-medium">Regional Power Dynamics</h4>
                  ...
                  <div className="text-muted-foreground text-xs">
                    • Trade bloc negotiations accelerating • Infrastructure cooperation increasing
                  </div>
                </div>
                ...
                <div className="border-border bg-card rounded-lg border p-3">
                  <div className="mb-2 flex items-center gap-2">
                    ...
                    <h4 className="text-sm font-medium">Policy Innovation Tracking</h4>
                    <Badge variant="outline" className="text-xs">
                      Respond
                    </Badge>
                  </div>
                  ...
                  <div className="text-muted-foreground text-xs">
                    • Digital transformation initiatives • Sustainable development programs
                  </div>
                </div>
              </div>
            </TabsContent>
