...
const getGradeColor = (grade: string): string => {
  return (
    gradeColors[grade as keyof typeof gradeColors] || "text-gray-600 bg-gray-100 border-gray-300"
  );
};
...
export function EnhancedEconomicIntelligence({
  ...
  const analysis = useMemo(() => {
    try {
      ...
      const comprehensive = integratedAnalyzer.analyzeCountry(
        countryStats,
        economyData,
        historicalData
      );
  ...
  if (!comprehensive || !healthSummary) {
    return (
      <Card className={className}>
        ...
        <CardContent>
          <p className="text-sm text-gray-600">Economic analysis data is incomplete</p>
        </CardContent>
      </Card>
    );
  }
      ...
      <Card className="border-l-4 border-l-blue-500">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
              <div className="mb-2 flex items-center gap-2">
                {React.createElement(
                  statusIcons[healthSummary.status.toLowerCase() as keyof typeof statusIcons] ||
                    Shield,
                  {
                    className: `h-4 w-4 ${
                      healthSummary.status === "Excellent" || healthSummary.status === "Strong"
                        ? "text-green-600"
                        : healthSummary.status === "Good" || healthSummary.status === "Fair"
                          ? "text-blue-600"
                          : "text-red-600"
                    }`,
                  }
              ...
              <div className="mt-1 text-xs text-gray-600">{healthSummary.score}/100</div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <Badge
                  variant={
                    healthSummary.healthIndicators.growth === "strong" ? "default" : "secondary"
                  }
                  className="text-xs"
                >
                ...
                <Badge
                  variant={
                    healthSummary.healthIndicators.stability === "high" ? "default" : "secondary"
                  }
                  className="text-xs"
                >
          ...
          <div className="rounded-lg bg-blue-50 p-3 dark:bg-blue-950/20">
            <p className="text-sm text-blue-800 dark:text-blue-200">{healthSummary.keyMessage}</p>
          </div>
        </CardContent>
      </Card>
            ...
            <div className="space-y-1 text-xs">
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {comprehensive.resilience.components.fiscalStability}%
                </span>
              </div>
              <div className="flex justify-between">
                <span>Monetary:</span>
                <span className="font-medium">
                  {comprehensive.resilience.components.monetaryStability}%
                </span>
              </div>
            </div>
            {comprehensive.resilience?.riskFactors?.length > 0 && (
              <div className="mt-2 rounded bg-yellow-50 p-2 text-xs dark:bg-yellow-950/20">
          ...
          <CardContent>
            ...
            <div className="space-y-1 text-xs">
              <div className="flex justify-between">
                <span>Labor:</span>
                <span className="font-medium">
                  {comprehensive.productivity.components.laborProductivity}%
                </span>
              </div>
              <div className="flex justify-between">
                <span>Capital:</span>
                <span className="font-medium">
                  {comprehensive.productivity.components.capitalEfficiency}%
                </span>
              </div>
            </div>
          ...
          <CardContent>
            ...
            <div className="space-y-1 text-xs">
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {comprehensive.wellbeing.components.livingStandards}%
                </span>
              </div>
              <div className="flex justify-between">
                <span>Healthcare:</span>
                <span className="font-medium">
                  {comprehensive.wellbeing.components.healthcareAccess}%
                </span>
              </div>
            </div>
            <div className="mt-2 rounded bg-purple-50 p-2 text-xs dark:bg-purple-950/20">
          ...
          <CardContent>
            ...
            <div className="space-y-1 text-xs">
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {comprehensive.complexity.components.exportDiversity}%
                </span>
              </div>
              <div className="flex justify-between">
                <span>Financial:</span>
                <span className="font-medium">
                  {comprehensive.complexity.components.financialSophistication}%
                </span>
              </div>
            </div>
            {comprehensive.complexity.futureOpportunities.length > 0 && (
              <div className="mt-2 rounded bg-orange-50 p-2 text-xs dark:bg-orange-950/20">
            ...
            <div className="space-y-3">
              {comprehensive.priorityRecommendations.slice(0, 3).map((rec, index) => (
                <motion.div
                  ...
                  className="flex items-start gap-3 rounded-lg border p-3 transition-colors hover:bg-gray-50 dark:hover:bg-gray-900/50"
                >
                  <Badge
                  ...
                  <div className="flex-1">
                    <div className="text-sm font-medium">{rec.area}</div>
                    <div className="mt-1 text-xs text-gray-600">{rec.action}</div>
                    <div className="mt-1 text-xs text-gray-500">
                      Timeline: {rec.timeframe.replace("_", " ")}
                    </div>
                  </div>
                  {onTakeAction && (
                    <Button size="sm" variant="outline" onClick={() => onTakeAction(rec.action)}>
                      Take Action
                    </Button>
                  )}
                </motion.div>
