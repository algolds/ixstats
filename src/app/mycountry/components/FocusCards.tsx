...
function FocusCardComponent({
  ...
  return (
    ...
    >
      <Card
        className={`glass-hierarchy-child cursor-pointer overflow-hidden border-0 transition-all duration-300`}
        style={{
          ...
          <div className="mb-4 grid grid-cols-2 gap-3">
            {card.metrics.slice(0, expanded ? undefined : 4).map((metric, i) => (
              <div key={`metric-${card.id || "fallback"}-${i}`} className="space-y-1">
                ...
                <div className="text-muted-foreground text-xs">{metric.change}</div>
              </div>
            ))}
          </div>
                    ...
                    <div className="flex flex-1 items-center gap-2">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-muted-foreground text-xs">
                          Impact:{" "}
                          {action.estimatedImpact.economic ||
                            action.estimatedImpact.social ||
                            action.estimatedImpact.diplomatic}
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`capitalize ${
                      card.trends.shortTerm === "improving"
                        ? "text-green-600"
                        : card.trends.shortTerm === "declining"
                          ? "text-red-600"
                          : "text-yellow-600"
                    }`}
                  >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`capitalize ${
                      card.trends.longTerm === "improving"
                        ? "text-green-600"
                        : card.trends.longTerm === "declining"
                          ? "text-red-600"
                          : "text-yellow-600"
                    }`}
                  >
  ...
  return [
    {
      ...
      status:
        safeScore(countryData.economic.healthScore) >= 80
          ? "excellent"
          : safeScore(countryData.economic.healthScore) >= 60
            ? "good"
            : safeScore(countryData.economic.healthScore) >= 40
              ? "concerning"
              : "critical",
      priority: "high",
      metrics: [
    ...
    },
    {
      ...
      status:
        safeScore(countryData.population.healthScore) >= 80
          ? "excellent"
          : safeScore(countryData.population.healthScore) >= 60
            ? "good"
            : safeScore(countryData.population.healthScore) >= 40
              ? "concerning"
              : "critical",
      priority: "high",
      metrics: [
    ...
    },
    {
      ...
      status:
        safeScore(countryData.diplomatic.healthScore) >= 80
          ? "excellent"
          : safeScore(countryData.diplomatic.healthScore) >= 60
            ? "good"
            : safeScore(countryData.diplomatic.healthScore) >= 40
              ? "concerning"
              : "critical",
      priority: "medium",
      metrics: [
    ...
    },
    {
      ...
      status:
        safeScore(countryData.government.healthScore) >= 80
          ? "excellent"
          : safeScore(countryData.government.healthScore) >= 60
            ? "good"
            : safeScore(countryData.government.healthScore) >= 40
              ? "concerning"
              : "critical",
      priority: "high",
      metrics: [
