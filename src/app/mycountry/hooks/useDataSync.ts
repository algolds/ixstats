...
} from "../components/GlobalNotificationSystem";
...
export function useDataSync(countryId: string, options: DataSyncOptions = {}) {
  ...
  const useDataNotifications = () => ({
    ...
    createAchievementNotification: async (data: {
      title: string;
      description: string;
      rarity: string;
    }) => {
        ...
        switch (changeType) {
          case "population":
            if (previousDataRef.current) {
              const growthRate =
                (data.currentPopulation - previousDataRef.current.currentPopulation) /
                previousDataRef.current.currentPopulation;
          ...
          case "gdpPerCapita":
            if (previousDataRef.current) {
              const changePercent =
                ((data.currentGdpPerCapita - previousDataRef.current.currentGdpPerCapita) /
                  previousDataRef.current.currentGdpPerCapita) *
                100;
          ...
          case "economicTier":
            if (previousDataRef.current) {
              const isImprovement =
                getTierRank(data.economicTier) > getTierRank(previousDataRef.current.economicTier);
              addNotification({
          ...
          case "economicVitality":
            if (
              ...
              data.economicVitality &&
              previousDataRef.current.economicVitality
            ) {
