  ...
  return [
    // Economic Intelligence (using real API vitality score)
    {
      ...
      keyMetrics: [
        ...
        },
        {
          ...
          unit: "%",
          trend:
            country.realGDPGrowthRate > 0.02
              ? "up"
          ...
          status:
            country.realGDPGrowthRate > 0.03
              ? "excellent"
              : country.realGDPGrowthRate > 0
                ? "good"
                : "concerning",
    ...
    // Population Intelligence (using real API vitality score)
    {
      ...
      forecast: {
        shortTerm: {
          projected: Math.min(
            100,
            country.populationWellbeing + country.populationGrowthRate * 100
          ),
          confidence: 80,
          factors: [
        ...
        longTerm: {
          projected: Math.min(
            100,
            country.populationWellbeing + country.populationGrowthRate * 400
          ),
          confidence: 70,
          factors: [
          ...
          projected: Math.min(
            ...
            Math.max(
              ...
              country.diplomaticStanding +
                country.economicVitality * 0.1 -
                (100 - country.governmentalEfficiency) * 0.05
            )
          ),
          confidence: 60,
        ...
        longTerm: {
          projected: Math.min(
            ...
            Math.max(
              0,
              country.diplomaticStanding +
                country.economicVitality * 0.2 +
                country.populationWellbeing * 0.1 -
                (100 - country.governmentalEfficiency) * 0.1
            )
          ),
          confidence: 50,
          factors: [
    ...
    // Governance Intelligence (using real API vitality score)
    {
      ...
      trend: calculateTrend(
        ...
        previousCountry?.governmentalEfficiency
      ),
      change: {
        value: previousCountry
 ...
 */
function mapAlertCategory(
  category: string
): "economic" | "population" | "diplomatic" | "governance" | "crisis" {
  switch (category) {
    case "security":
      return "crisis";
    case "social":
 ...
 */
export function transformApiIntelligenceToAlerts(
  intelligenceItems: ApiIntelligenceItem[]
): CriticalAlert[] {
  return intelligenceItems
    .filter(
      (item) => item.type === "alert" && (item.severity === "high" || item.severity === "critical")
    )
...
export function transformApiDataToExecutiveIntelligence(
  ...
  const averageVitality =
    vitalityIntelligence.reduce((sum, v) => sum + v.score, 0) / vitalityIntelligence.length;
  ...
  return {
    ...
    forwardIntelligence: {
      ...
      opportunities: transformOpportunities(
        intelligenceItems.filter((i) => i.type === "opportunity")
      ),
      risks: transformRisks(
        intelligenceItems.filter((i) => i.severity === "high" || i.severity === "critical")
      ),
    ...
    confidenceLevel: (() => {
      ...
      const avg =
        intelligenceItems.reduce((sum, item) => sum + (item.confidence ?? 0), 0) /
...
function mapSeverityToUrgency(
  severity: "low" | "medium" | "high" | "critical"
): "urgent" | "important" | "routine" | "future" {
  switch (severity) {
    case "critical":
      return "urgent";
    case "high":
...
function generateCompetitiveIntelligence(
  ...
  if (economicVitality) {
    competitiveData.push({
      ...
      strengths:
        economicVitality.score > 75 ? ["Strong GDP growth", "Stable economic policies"] : [],
...
function calculatePeerAverage(
  ...
  const tierAdjustment =
    country.economicTier === "developed" ? 10 : country.economicTier === "emerging" ? 5 : 0;
 ...
 */
function calculateRegionalAverage(
  country: ApiCountryData,
  metric: "economic" | "population" | "diplomatic" | "governance"
): number {
  ...
  const regionAdjustment =
    country.region === "North America" || country.region === "Europe"
      ? 5
      : country.region === "Asia"
        ? 3
        : 0;
...
function calculateCountryRank(
  ...
  if (score >= 70) return Math.floor(maxCountries * 0.3) + 1; // Top 30%
  if (score >= 60) return Math.floor(maxCountries * 0.5) + 1; // Top 50%
  if (score >= 50) return Math.floor(maxCountries * 0.7) + 1; // Top 70%
