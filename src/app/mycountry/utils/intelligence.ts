...
export const createExecutiveIntelligence = (
  ...
  const urgentActions = vitality
    .flatMap((v) => v.recommendations.filter((r) => r.urgency === "urgent"))
    .slice(0, 3);
  ...
  return {
    ...
    confidenceLevel: Math.round(
      vitality.reduce((sum, v) => sum + v.forecast.shortTerm.confidence, 0) / vitality.length
    ),
