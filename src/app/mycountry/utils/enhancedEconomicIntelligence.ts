...
export function transformToExecutiveIntelligence(
  ...
  try {
    ...
    const trendingInsights: TrendingInsight[] = executiveIntelligence.trendingInsights.map(
      (insight) => ({
        id: insight.id,
        title: insight.title,
    ...
    const urgentActions: ActionableRecommendation[] =
      executiveIntelligence.actionableRecommendations.map((rec) => ({
        ...
        estimatedBenefit:
          rec.impact === "high" ? "Significant improvement" : "Moderate improvement",
    ...
    const vitalityIntelligence: VitalityIntelligence[] = [
      {
        ...
        forecast: {
          shortTerm: {
            projected: healthCheck.score,
            confidence: 85,
            factors: ["Economic stability"],
          },
          longTerm: {
            projected: healthCheck.score + 5,
            confidence: 70,
            factors: ["Economic reforms"],
          },
        },
        comparisons: {
    ...
    switch (area) {
      ...
      case "complexity":
        baseMetrics.push({
          ...
          unit: "",
          trend: "stable",
          status: ["Strong", "Very Strong", "Extravagant"].includes(countryStats.economicTier)
            ? "excellent"
            : "good",
          changeValue: 0,
          changePercent: 0,
