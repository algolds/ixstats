...
export function GlobalNotificationProvider({ children }: { children: React.ReactNode }) {
  ...
  const getNotificationsByCategory = useCallback((category: GlobalNotification['category']) => {
    return notifications.filter(n => n.category === category);
  }, [notifications]);

  const getNotificationsByPriority = useCallback((priority: GlobalNotification['priority']) => {
...
function getNotificationIcon(type: GlobalNotification['type'], category: GlobalNotification['category']) {
...
function getNotificationTheme(type: GlobalNotification['type'], priority: GlobalNotification['priority']) {
...
export function NotificationCard({ 
  ...
  return (
    <motion.div
      ...
      className={`
        relative p-4 rounded-lg border backdrop-blur-md transition-all duration-200
        ${theme.bg} ${theme.glow}
        ${compact ? 'p-3' : 'p-4'}
      `}
    >
      <div className="flex items-start gap-3">
        <div className={`flex-shrink-0 p-2 rounded-lg ${theme.icon}`}>
