...
export function VaultSettingsCard() {
  ...
  return (
    <div className="glass-surface glass-refraction overflow-hidden rounded-3xl p-1 transition-all duration-500 hover:shadow-2xl">
      <div className="relative rounded-[calc(1.5rem-1px)] bg-white/40 p-6 dark:bg-slate-900/40 overflow-hidden">
        ...
        <div className="mb-6 flex items-center justify-between relative z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-900 dark:text-white">Vault Rewards & Upgrades</h2>
            </div>
          </div>
          <Link
        ...
        {/* Vault Stats Panel */}
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 mb-6 relative z-10">
          {/* Balance and Premium status */}
          <div className="rounded-2xl bg-slate-50/50 p-4 dark:bg-slate-800/30 flex flex-col justify-between">
            <div>
              <label className="block text-[10px] font-bold tracking-wider text-slate-400 uppercase dark:text-slate-500">
                Credits Balance
              </label>
              <p className="text-xl font-black text-slate-950 dark:text-white mt-1 flex items-center gap-1">
            ...
            {isPremium && (
              <span className="mt-2 inline-flex self-start items-center gap-1 rounded-md bg-indigo-100 px-2 py-0.5 text-[9px] font-bold text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400">
                Premium Active
              </span>
            )}
          </div>
          ...
          <div className="rounded-2xl bg-slate-50/50 p-4 dark:bg-slate-800/30">
            ...
            <div className="mt-2 h-2.5 w-full rounded-full bg-slate-200/50 overflow-hidden dark:bg-slate-800/80">
              <div
                className="h-full rounded-full bg-gradient-to-r from-amber-500 to-yellow-500 transition-all duration-500"
                style={{ width: `${progressPercent}%` }}
              />
            </div>
            <p className="text-[9px] text-slate-400 mt-2">Level up to unlock larger passive yields</p>
          </div>

          {/* Login Streak & Claim Rewards */}
          <div className="rounded-2xl bg-slate-50/50 p-4 dark:bg-slate-800/30 flex justify-between items-center">
            <div>
              <div className="flex items-center gap-1">
                <Flame className="h-4 w-4 text-orange-500" />
                <span className="text-sm font-bold text-slate-900 dark:text-white">{loginStreak} Days</span>
              </div>
              <label className="block text-[10px] font-bold tracking-wider text-slate-400 uppercase dark:text-slate-500 mt-1">
                Streak Bonus
              </label>
            </div>
            <button
        ...
        <div className="border-t border-slate-100 pt-6 dark:border-slate-800 relative z-10 mb-6">
          ...
          {ownedLoading || itemsLoading ? (
            ...
            <div className="py-4 text-center text-xs text-slate-400">No account upgrades available.</div>
          ) : (
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-2 mb-2">
                        <div className={`p-1.5 rounded-lg ${isOwned ? "bg-emerald-500/10 text-emerald-600" : "bg-slate-100 text-slate-400 dark:bg-slate-800"}`}>
        ...
        <div className="border-t border-slate-100 pt-6 dark:border-slate-800 relative z-10">
          <div className="mb-4 flex items-center gap-2">
          ...
          {ownedLoading || itemsLoading ? (
            ...
            <div className="py-4 text-center text-xs text-slate-400">No cosmetics configured in storefront.</div>
          ) : (
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              {cosmeticsList.map((item) => {
                ...
                return (
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-2 mb-2">
                        <div className={`p-1.5 rounded-lg ${isOwned ? "bg-purple-500/10 text-purple-600" : "bg-slate-100 text-slate-400 dark:bg-slate-800"}`}>
                          <ItemIcon className="h-4 w-4" />
                        </div>
                        <span className="text-xs font-bold text-slate-800 dark:text-slate-200">{item.name}</span>
                      </div>
