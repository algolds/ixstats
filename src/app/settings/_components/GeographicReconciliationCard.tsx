...
export function GeographicReconciliationCard({ countryId }: GeographicReconciliationCardProps) {
  ...
  const handleRebase = () => {
    if (rollups.subdivisionPopulationSum === 0 && rollups.cityPopulationSum === 0) {
      toast.error("Cannot rebase: no settlements or subdivisions have been defined on the map yet.");
      return;
    }

    if (confirm("Are you sure you want to overwrite your national baseline population and GDP totals with the sum of your geographic subdivisions/cities? This affects the national economic builder simulation.")) {
      startTransition(async () => {
      ...
      <CardContent className="space-y-5 p-5">
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
          ...
          <div className="space-y-3.5 lg:col-span-6">
            <div>
              ...
              <div className="mt-1.5 flex items-center gap-3">
                <Select
                  ...
                  disabled={isLoading}
                >
                  <SelectTrigger className="h-9 w-[180px] border-border/40 bg-white/20 text-xs shadow-xs focus:ring-purple-500/30 dark:bg-white/[0.04]">
            ...
            {/* Mode Descriptions */}
            <div className="rounded-xl bg-white/35 p-3.5 text-xs leading-relaxed dark:bg-white/[0.02] border border-border/20">
              {currentMode === "hybrid" && (
                <div className="space-y-1">
                  ...
                  <p className="text-muted-foreground/90 text-[11px]">
                    National totals stay fixed at the simulation baseline (e.g. from Nation Builder or World Bank). Subdivisions and cities display absolute geographic breakdown data. You can rebase manually anytime.
                  </p>
                </div>
              )}
              {currentMode === "top-down" && (
                <div className="space-y-1">
                  ...
                  <p className="text-muted-foreground/90 text-[11px]">
                    The national baseline dictates the absolute totals. Subdivision populations and GDP inputs are scaled dynamically on read to sum exactly to the national total. Respects specific city shares.
                  </p>
                </div>
              )}
              {currentMode === "bottom-up" && (
                <div className="space-y-1">
                  ...
                  <p className="text-muted-foreground/90 text-[11px]">
                    Authoritative stats roll up directly from the map. Any time you add a city, adjust subdivision boundaries, or update settlement populations, the national baseline immediately sums them up.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="space-y-4 lg:col-span-6 lg:border-l lg:border-border/30 lg:pl-5">
            <div>
              ...
              <div className="mt-2.5 space-y-3">
                ...
                <div className="space-y-1">
                  ...
                  />
                  <div className="flex justify-between text-[10px] text-muted-foreground/70">
                    <span>Σ Geographics: ${Math.round(rollups.subdivisionGdpContributionSum).toLocaleString()}</span>
            ...
            <div className="border-border/30 border-t pt-3 flex items-center justify-between">
              <div className="space-y-0.5">
                <p className="text-[11px] font-medium text-foreground">Manual Reconciliation</p>
              ...
              <Button
                ...
                className="h-8 border-purple-500/30 bg-purple-500/5 text-xs text-purple-600 hover:bg-purple-500/15 dark:text-purple-400 dark:border-purple-400/20"
              >
