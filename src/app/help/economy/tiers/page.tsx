    ...
    >
      <Section title="Tier Basics">
        <ul className="list-disc pl-6 space-y-2">
          <li>Tiers range from Impoverished to Extravagant and are calculated from GDP per capita and population size.</li>
          <li>Tier data lives in `api.economics.getCountryIndicators` and is stored in Prisma fields `economicTier` and `populationTier`.</li>
      ...
      <Section title="Why Tiers Matter">
        <InfoBox title="Gameplay Effects">
          <ul className="list-disc pl-6 space-y-1">
            ...
            <li>Builder defaults (economic templates) adjust growth caps and resilience modifiers by tier.</li>
            <li>Compliance tasks compare current tier against projections to highlight underperformance.</li>
          </ul>
        </InfoBox>
      </Section>
      ...
      <Section title="Where to Monitor">
        <ul className="list-disc pl-6 space-y-2">
          <li>MyCountry Economic tab – `EconomicSummaryWidget` and `CoreEconomicIndicators` highlight tier shifts.</li>
      ...
      <InfoBox title="Implementation Notes">
        <ul className="list-disc pl-6 space-y-1">
          <li><BarChart3 className="inline h-4 w-4" /> Update tiers after seeding data or running bulk imports.</li>
          <li><Gauge className="inline h-4 w-4" /> Keep tests (`npm run test:economics`) green when adjusting thresholds.</li>
        </ul>
      </InfoBox>
    </ArticleLayout>
  );
