  ...
  if (isLoading) {
    return (
      <div className="space-y-3">
        {[...Array(5)].map((_, i) => (
          <div key={i} className="glass-hierarchy-child animate-pulse rounded-lg p-3">
      ...
      {activities.map((activity, index) => {
        ...
        return (
          <motion.div
            ...
            className="glass-hierarchy-child group cursor-pointer rounded-lg p-3 transition-transform hover:scale-[1.02]"
          >
            <div className="flex items-center gap-3">
              <div
              ...
              <div className="min-w-0 flex-1">
                <div className="mb-1 flex items-center justify-between">
                  <h4 className="text-foreground truncate text-sm font-medium">{activity.title}</h4>
                  <div className="text-muted-foreground flex items-center gap-1 text-xs">
                    <Clock className="h-3 w-3" />
                    {formatTimeAgo(activity.timestamp)}
                  </div>
                </div>
                <p className="text-muted-foreground mb-2 text-xs">{activity.description}</p>
                <div className="flex items-center justify-between">
                  <Badge variant="outline" className="text-xs">
                    {activity.country}
                  </Badge>
                  <span className={`text-xs font-medium ${activity.color}`}>{activity.value}</span>
                </div>
              </div>
            </div>
          </motion.div>
