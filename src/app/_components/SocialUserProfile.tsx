        ...
        <CardContent className="p-6">
          ...
          <div className="mb-6 flex flex-col items-center text-center">
            <div className="relative mb-4">
              ...
              {/* Online Status */}
              <div className="border-background absolute top-0 right-0 h-4 w-4 rounded-full border-2 bg-green-500" />
            </div>

            {/* User Info */}
            <div className="mb-4">
              <div className="mb-2 flex items-center justify-center gap-2">
                <h2 className="text-foreground text-xl font-bold">{user?.firstName || "User"}</h2>
                {!roleLoading && roleUser?.role ? (
                  <Badge
                    className={`flex items-center gap-1 px-2 py-1 text-xs ${
                      roleUser.role.level <= 0
                        ? "border-purple-200 bg-purple-100 text-purple-800"
                        : roleUser.role.level <= 10
                          ? "border-red-200 bg-red-100 text-red-800"
                          : roleUser.role.level <= 20
                            ? "border-blue-200 bg-blue-100 text-blue-800"
                            : roleUser.role.level <= 30
                  ...
                  <Badge className="flex items-center gap-1 border-gray-200 bg-gray-100 px-2 py-1 text-xs text-gray-800">
            ...
            {/* Meta Info */}
            <div className="text-muted-foreground mb-4 flex items-center justify-center gap-6 text-sm">
          ...
          {/* Influence Progress */}
          <div className="glass-hierarchy-child mt-4 rounded-lg p-3">
            <div className="mb-2 flex items-center justify-between">
              <span className="text-sm font-medium">Influence Level</span>
              <span className="text-muted-foreground text-sm">{socialMetrics.influence}/100</span>
            </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  {friends.map((friend) => (
                    <div
                      key={friend.id}
                      className="glass-hierarchy-interactive flex items-center gap-4 rounded-lg p-3"
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-muted-foreground text-sm">
                          {formatLastOnline(friend.lastOnline)} • {friend.mutualConnections} mutual
                          connections
                        </p>
                      </div>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  {achievements.map((achievement) => (
                    <div
                      key={achievement.id}
                      className={cn(
                        "glass-hierarchy-interactive rounded-lg border-2 p-4",
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <div className="mb-1 flex items-center gap-2">
                            <h5 className="text-foreground font-semibold">{achievement.title}</h5>
                            <Badge
                              variant="outline"
                              className={cn(
                                "text-xs capitalize",
