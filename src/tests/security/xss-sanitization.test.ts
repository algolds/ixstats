...
describe("XSS Sanitization Test Suite", () => {
  describe("sanitizeUserContent() - Strict User Content", () => {
    ...
    it("preserves wiki card embed data attributes", () => {
      ...
      expect(output).toContain('data-title="Test Title"');
      expect(output).toContain('data-summary="Test Summary"');
      expect(output).toContain('data-imageurl="https://example.com/img.jpg"');
      expect(output).toContain('data-source="iiwiki"');
    });
  });
  ...
  describe("sanitizeWikiContent() - Moderate Wiki Content", () => {
    ...
    it("allows semantic HTML elements", () => {
      const input =
        '<section><article><h2>Title</h2><time datetime="2025-01-01">Date</time></article></section>';
      const output = sanitizeWikiContent(input);
  ...
  describe("Integration Tests - Real-World Scenarios", () => {
    ...
    it("sanitizes builder import data", () => {
      const importData = {
        capital:
          '<a href="https://ixwiki.com/wiki/Capital">Capital City</a><script>alert(1)</script>',
