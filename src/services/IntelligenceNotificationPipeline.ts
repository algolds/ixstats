...
export class IntelligenceNotificationPipeline {
   ...
   */
  private async analyzeSignificance(
    ...
    for (const deliveryChannel of notification.delivery.channels) {
      try {
        ...
        if (deliveryChannel.fallback) {
          try {
            const fallbackResult = await this.deliverImmediate(
              notification,
  ...
  private analyzeEconomicSignificance(item: IntelligenceItem): number {
    ...
    const keywordMatches = economicKeywords.filter(
      (keyword) =>
  ...
  private analyzeStrategicSignificance(item: IntelligenceItem): number {
    ...
    const keywordMatches = strategicKeywords.filter(
      (keyword) =>
        item.title.toLowerCase().includes(keyword) || item.summary.toLowerCase().includes(keyword)
    );
  ...
  private analyzeOperationalSignificance(item: IntelligenceItem): number {
    ...
    const keywordMatches = operationalKeywords.filter(
      (keyword) =>
        item.title.toLowerCase().includes(keyword) || item.summary.toLowerCase().includes(keyword)
    );
  ...
  private calculateImpact(item: IntelligenceItem): number {
    ...
    const matches = highImpactKeywords.filter(
      (keyword) =>
        item.title.toLowerCase().includes(keyword) || item.summary.toLowerCase().includes(keyword)
    ).length;
  ...
  private findApplicableTemplates(
    ...
    for (const template of this.templates.values()) {
      ...
      const categoryMatch =
        template.triggers.categories.length === 0 ||
        template.triggers.categories.includes(item.category);

      // Check priority match
      const priorityMatch =
        template.triggers.priorities.length === 0 ||
  ...
  private shouldNotifyUser(
    ...
    const minPriorityLevel =
      priorityLevels[categoryPref.minPriority as keyof typeof priorityLevels] || 1;
  ...
  private async createPersonalizedNotification(
    ...
    return {
      ...
      delivery: {
        ...
        scheduledTime:
          template.delivery.timing === "scheduled"
            ? Date.now() + (template.delivery.batchWindow || 0)
            : undefined,
        ...
        actions: [
          {
            ...
            label: "Emergency Briefing",
            type: "danger",
            action: "api_call",
            data: { endpoint: "/api/emergency" },
          },
          {
            id: "escalate",
            label: "Escalate",
  ...
  private async processBatch(
    ...
    console.log(
      `Processing batch of ${notifications.length} notifications for ${userId} via ${channel}`
    );
  }
  ...
  private determineDeliveryChannels(
    ...
    if (
      ...
      (item.priority !== "critical" || !userPrefs.timing.immediateForCritical)
    ) {
      channels.push({
        channel: "email",
    ...
    return channels.length > 0
      ? channels
      : template.delivery.defaultChannels.map((ch) => ({
          channel: ch as any,
          config: {},
          priority: 1,
        }));
  ...
  private updateProcessingStats(
    ...
    this.processingStats.averageProcessingTime =
      (this.processingStats.averageProcessingTime * (this.processingStats.processed - 1) +
        processingTime) /
      this.processingStats.processed;
  }
