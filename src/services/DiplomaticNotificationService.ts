...
interface DiplomaticEvent {
  id: string;
  type: 'agreement' | 'treaty' | 'conflict' | 'trade' | 'alliance' | 'sanction' | 'embassy' | 'meeting';
  title: string;
  description: string;
  countries: string[];
...
class DiplomaticNotificationService extends EventEmitter {
  ...
  private eventTypeConfigs: Record<string, {
  ...
  async processRelationshipChange(change: RelationshipChange) {
    ...
    try {
      ...
      console.log(`[DiplomaticNotificationService] Relationship change: ${change.fromCountry} -> ${change.toCountry} (${changeAmount.toFixed(1)} points)`);
      this.emit('relationshipChanged', { change, notification });
    } catch (error) {
      console.error('[DiplomaticNotificationService] Failed to process relationship change:', error);
    }
  }
  ...
  async batchProcessEvents(events: DiplomaticEvent[]) {
    console.log(`[DiplomaticNotificationService] Batch processing ${events.length} diplomatic events`);
    ...
    console.log(`[DiplomaticNotificationService] Batch complete: ${successful} successful, ${failed} failed`);
...
class AchievementNotificationService {
  ...
  async processAchievementUnlock(achievement: Achievement) {
    ...
    try {
      ...
      console.log(`[AchievementNotificationService] Achievement unlocked: ${achievement.name} (${achievement.rarity})`);
    } catch (error) {
  ...
  processProgressUpdate(achievement: Achievement) {
    ...
    if (achievement.progress && achievement.maxProgress) {
      ...
      if ([25, 50, 75].includes(Math.floor(progressPercent))) {
        console.log(`[AchievementNotificationService] Achievement progress: ${achievement.name} - ${progressPercent.toFixed(0)}%`);
      }
    }
  }
}
