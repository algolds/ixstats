...
export class EnhancedNotificationPriority {
  ...
  async calculateEnhancedPriority(
    ...
    const engagementHistory = this.calculateEngagementInfluence(
      notification, 
      userEngagement
    );
    ...
    const reasoning = this.generatePriorityReasoning(
      notification,
      { basePriority, contextualBoost, urgencyMultiplier, userRelevance, temporalFactor, engagementHistory },
      contextualFactors
    );
  ...
  private calculateContextualBoost(
    ...
    if (factors.isExecutiveSession) {
      const executiveCategories: NotificationCategory[] = [
        'economic', 'diplomatic', 'governance', 'crisis', 'achievement'
      ];
  ...
  private calculateUserRelevance(
    ...
    if (similarEngagements.length > 0) {
      const avgEngagement = similarEngagements.reduce((sum, e) => {
        const score = e.action === 'action-taken' ? 3 :
                     e.action === 'clicked' ? 2 :
                     e.action === 'viewed' ? 1 : 0;
        return sum + score;
  ...
  private calculateEngagementInfluence(
    ...
    const recentDismissals = engagement.filter(e => 
      e.action === 'dismissed' && 
    ...
    const recentActions = engagement.filter(e => 
  ...
  private calculateClusterScore(cluster: NotificationCluster): number {
    ...
    const priorityBonus = cluster.priority === 'critical' ? 20 : 
                         cluster.priority === 'high' ? 15 : 
                         cluster.priority === 'medium' ? 10 : 5;
  ...
  private checkPageRelevance(context: NotificationContext): boolean {
    return ['dashboard', 'mycountry', 'admin'].some(page => 
      context.currentRoute.includes(page)
    );
  }

  private determineDeviceAttention(context: NotificationContext): 'focused' | 'background' | 'away' {
  ...
  private generatePriorityReasoning(
    ...
    if (breakdown.basePriority > 80) {
      reasoning.push(`High base priority (${notification.priority}) for ${notification.category} notification`);
    }
...
export const calculateEnhancedPriority = async (
  ...
  return await enhancedPriorityInstance.calculateEnhancedPriority(notification, context, userPreferences);
};
