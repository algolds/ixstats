...
class GlobalNotificationBridge extends EventEmitter {
  ...
  private async handleDataStreamEvent(event: DataStreamEvent) {
    const matchingRules = this.rules.filter(
      (rule) => rule.type === event.type && rule.condition(event.data)
    );
  ...
  private async createNotificationFromRule(rule: NotificationRule, event: DataStreamEvent) {
        ...
        userPreferences: {
          ...
          categories: {
            economic: {
              ...
              deliveryMethods: ["dynamic-island"] as DeliveryMethod[],
            },
            diplomatic: {
              enabled: true,
              minPriority: "low" as const,
            ...
            crisis: {
              ...
              deliveryMethods: ["dynamic-island", "toast"] as DeliveryMethod[],
            },
            opportunity: {
              enabled: true,
              minPriority: "low" as const,
    ...
    try {
      ...
      console.log(
        `[NotificationBridge] Created ${rule.priority} notification:`,
        notification.title
      );
  ...
  private getNotificationType(
    priority: NotificationPriority
  ): "info" | "success" | "warning" | "error" | "alert" | "update" {
    switch (priority) {
      case "critical":
        return "error";
      case "high":
  ...
  private getSeverity(priority: NotificationPriority): "informational" | "important" | "urgent" {
    switch (priority) {
      case "critical":
        return "urgent"; // Critical maps to urgent since critical is not in NotificationSeverity
      case "high":
        return "urgent";
      case "medium":
        return "important";
  ...
  private getDeliveryMethod(
    priority: NotificationPriority
  ): "toast" | "dynamic-island" | "modal" | "command-palette" {
    switch (priority) {
      case "critical":
        return "modal";
      case "high":
  ...
  wireIntelligenceStream(intelligenceData: IntelligenceItem[]) {
    intelligenceData.forEach((item) => {
      ...
      const rawPriority =
        this.getDataProperty(itemData, "priority") ||
        this.getDataProperty(itemData, "severity") ||
        "medium";
   ...
   */
  wireEconomicStream(economicData: {
    metric: string;
    value: number;
   ...
   */
  wireDiplomaticStream(diplomaticEvent: {
    eventType: string;
    title?: string;
    description?: string;
    countries?: string[];
   ...
   */
  wireAchievementStream(achievement: {
    name: string;
    description: string;
    unlocked: boolean;
    category?: string;
...
export function useGlobalNotificationBridge() {
  return {
    ...
    wireIntelligence:
      globalNotificationBridge.wireIntelligenceStream.bind(globalNotificationBridge),
