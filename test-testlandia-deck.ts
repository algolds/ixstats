/**
 * Test: Fetch Testlandia's NationStates Card Deck
 */

import { fetchNSDeck, getNSCardData } from "./src/lib/ns-api-client";

async function testNationDeck(nationName: string) {
  console.log(`🎴 Fetching ${nationName}'s NationStates Card Deck\n`);
  console.log("=" + "=".repeat(60) + "\n");

  try {
    // Fetch nation's deck
    console.log(`📡 Fetching deck for nation: ${nationName}...\n`);
    const deck = await fetchNSDeck(nationName);

    console.log(`✅ Successfully fetched ${nationName}'s deck!\n`);
    console.log("=" + "=".repeat(60));
    ...
    if (deck.cards.length === 0) {
      ...
      console.log("\n💡 Trying alternative nation: Testlandia...");
      return testNationDeck("Testlandia");
    }
    ...
    for (let i = 0; i < sortedByValue.length && i < 10; i++) {
    ...
    if (deck.cards.length > 0 && sortedByValue.length > 0) {
    ...
    console.log("\n" + "=".repeat(60));
    console.log(`✅ ${nationName} Deck Test Complete!\n`);

  } catch (error) {
    console.error(`❌ Error fetching ${nationName}'s deck:`, error);
...
// Run test - try Urcea first, fall back to Testlandia if empty
testNationDeck("Urcea")
  .then(() => {
