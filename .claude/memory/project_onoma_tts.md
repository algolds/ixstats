...
**WikiOS narrator UI lives in the Halo, not a standalone component** (June 2026): the old `WikiOSNarratorPlayer.tsx` floating pill was DELETED and merged into the Dynamic Island. Controls are in `DynamicIsland/WikiView.tsx` under a **"Workspace | Now Playing"** segmented tab (Now Playing = reading-progress scrubber + morphing play/pause + skip + `⋯` popover for speed/voice/follow-scroll/clear-cache, accent `#3b82f6`). Compact pill play/pause = `DynamicIsland/PlayPauseMorph.tsx` (shared morph glyph, also used in `WikiDIPlugin.tsx` breadcrumb). `MiniPlayer.tsx` is suppressed for `activeTrack.isDynamicTts` so narration shows ONLY in the Halo. Logic still in `useWikiNarrator.ts` (state/actions via WikiContext).
