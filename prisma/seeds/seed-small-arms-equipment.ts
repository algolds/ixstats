...
async function seedSmallArmsEquipment() {
  ...
  try {
    ...
    const manufacturerData = Object.entries(SMALL_ARMS_MANUFACTURERS).map(([key, manufacturer]) => ({
      key,
    ...
    for (const [equipmentType, items] of Object.entries(equipmentCategories)) {
      ...
      const equipmentData = items.map(item => ({
        ...
        key: item.name.toUpperCase().replace(/[\s\-\.\/]/g, '_').replace(/[^A-Z0-9_]/g, ''),
        name: item.name,
      ...
      for (const itemData of equipmentData) {
        try {
          ...
          if (error.code === 'P2003') {
            console.log(`    ⚠️  Skipping ${itemData.name} - manufacturer "${itemData.manufacturerKey}" not found`);
