...
async function main() {
  ...
  for (const [, synergy] of synergyMap) {
    try {
      ...
      if (wasCreated) {
        console.log(
          `✅ Created ${synergy.synergyType}: ${comp1} ↔ ${comp2} (${synergy.bonusPercent > 0 ? "+" : ""}${synergy.bonusPercent}%)`
        );
        createdCount++;
      } else {
        console.log(
          `🔄 Updated ${synergy.synergyType}: ${comp1} ↔ ${comp2} (${synergy.bonusPercent > 0 ? "+" : ""}${synergy.bonusPercent}%)`
        );
        updatedCount++;
      }
    } catch (error) {
      console.error(
        `❌ Error processing synergy ${synergy.component1} ↔ ${synergy.component2}:`,
        error
      );
      errorCount++;
    }
