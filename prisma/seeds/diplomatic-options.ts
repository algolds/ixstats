...
async function main() {
  ...
  console.log(`📝 Total options:        ${STRATEGIC_PRIORITIES.length + PARTNERSHIP_GOALS.length + KEY_ACHIEVEMENTS.length}`);
  console.log('='.repeat(60));

  // Category breakdown
  const allOptions = await prisma.diplomaticOption.findMany();
  const categoryStats = allOptions.reduce((acc, option) => {
