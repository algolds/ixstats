...
if (!fs.existsSync(projTransPath)) {
  ...
  const setProjectionCalls = (projTransContent.match(/map\.current\.setProjection\s*\(/g) || []).length;
  if (setProjectionCalls !== 1) {
    ERRORS.push(`❌ useProjectionTransition.ts: Expected 1 setProjection call, found ${setProjectionCalls}`);
  }
  ...
  if (zoomListeners !== 1) {
    WARNINGS.push(`⚠️  useProjectionTransition.ts: Expected 1 'zoom' listener (for UI), found ${zoomListeners}`);
  }
  ...
  if (zoomendListeners !== 1) {
    ERRORS.push(`❌ useProjectionTransition.ts: Expected 1 'zoomend' listener, found ${zoomendListeners}`);
  }
  ...
  if (easeToCallsInApply > 0) {
    ERRORS.push(`❌ useProjectionTransition.ts: Found ${easeToCallsInApply} easeTo calls (should be 0)`);
  }
  ...
  if (triggerRepaints > 0) {
    ERRORS.push(`❌ useProjectionTransition.ts: Found ${triggerRepaints} triggerRepaint calls (should be 0)`);
  }

  // Check for setMaxBounds calls (should be 0)
  const maxBoundsCalls = (projTransContent.match(/setMaxBounds\s*\(/g) || []).length;
  if (maxBoundsCalls > 0) {
    WARNINGS.push(`⚠️  useProjectionTransition.ts: Found ${maxBoundsCalls} setMaxBounds calls (not recommended)`);
  }
}
