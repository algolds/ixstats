/**
 * One-time AST splitter for the economicComponents router (mirrors split-activities-ast.ts).
 *
 ...
 * in economicComponents/index.ts, so api.economicComponents.* paths are unchanged.
 *
 * Run from the project root:  bun run scripts/split-economiccomponents-ast.ts
 */
...
const routerFile = "src/server/api/routers/economicComponents.ts";
const outDir = "src/server/api/routers/economicComponents";
...
const origDecl = origSf.getVariableDeclarationOrThrow("economicComponentsRouter");
...
for (const [groupName, names] of Object.entries(groups)) {
  ...
  const decl = sf.getVariableDeclarationOrThrow("economicComponentsRouter");
  decl.rename(`economicComponents${cap(groupName)}Router`);
...
console.log(
  "Done. Now create economicComponents/index.ts (mergeRouters) and delete economicComponents.ts."
);
