...
async function main() {
  ...
  if (!mapLayer) {
    ...
    console.log(`MapLayer found: id=${mapLayer.id}, featureId=${mapLayer.featureId}, displayName=${mapLayer.displayName}, sourceUploadId=${mapLayer.sourceUploadId}, isActive=${mapLayer.isActive}`);
    const geomStr = JSON.stringify(mapLayer.geometry);
    console.log(`Geometry size: ${geomStr.length} characters`);
    if (geomStr.length > 500) {
      console.log(`Geometry preview: ${geomStr.substring(0, 250)}... [TRUNCATED] ...${geomStr.substring(geomStr.length - 250)}`);
    } else {
  ...
  for (const sub of subdivisions) {
    ...
    console.log(`- Sub: id=${sub.id}, name=${sub.name}, type=${sub.type}, status=${sub.status}, geometry size=${geomStr.length} characters`);
  }
  ...
  for (const req of editRequests) {
    console.log(`- Request: id=${req.id}, editType=${req.editType}, editSubtype=${req.editSubtype}, status=${req.status}, createdAt=${req.createdAt}`);
  }
  ...
  if (mapLayer) {
    ...
    for (const sess of sessions) {
      console.log(`- Session: id=${sess.id}, userId=${sess.userId}, isDraft=${sess.isDraft}, expiresAt=${sess.expiresAt}`);
    }
  }
}
