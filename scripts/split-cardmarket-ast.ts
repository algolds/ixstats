/**
 * One-time AST splitter for the card-market mega-router (mirrors split-activities-ast.ts).
 *
 ...
 * in card-market/index.ts, so api.cardMarket.* paths are unchanged.
 *
 * Run from the project root:  bun run scripts/split-cardmarket-ast.ts
 */
...
const routerFile = "src/server/api/routers/card-market.ts";
const outDir = "src/server/api/routers/card-market";
fs.mkdirSync(outDir, { recursive: true });

// 5 logical groups (auctions split into management/queries to keep files < 700 lines).
const groups: Record<string, string[]> = {
  ...
  bids: ["placeBid", "getBidHistory", "getMyActiveBids", "getMyAuctionParticipation"],
...
const origDecl = origSf.getVariableDeclarationOrThrow("cardMarketRouter");
...
for (const [groupName, names] of Object.entries(groups)) {
  ...
  const decl = sf.getVariableDeclarationOrThrow("cardMarketRouter");
  decl.rename(`cardMarket${cap(groupName)}Router`);
...
console.log("Done. Now create card-market/index.ts (mergeRouters) and delete card-market.ts.");
