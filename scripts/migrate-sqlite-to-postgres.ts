...
const isBooleanField = (key: string, value: any): boolean => {
  ...
  const booleanPrefixes = ['is', 'has', 'allow', 'require', 'notify', 'enable', 'disable', 'can', 'should', 'must', 'uses', 'hide'];
  const booleanWords = ['active', 'deleted', 'published', 'draft', 'featured', 'sticky', 'closed', 'resolved',
                        'urgent', 'critical', 'public', 'private', 'hidden', 'verified', 'approved', 'rejected', 'pending'];

  const lowerKey = key.toLowerCase();
  return booleanPrefixes.some(prefix => lowerKey.startsWith(prefix)) || booleanWords.includes(lowerKey);
};
...
const DATETIME_FIELDS = new Set([
  ...
  'completedAt', 'startedAt', 'endedAt', 'expiresAt', 'lastSeenAt',
  'lastLoginAt', 'lastActiveAt', 'verifiedAt', 'approvedAt', 'rejectedAt',
  'canceledAt', 'closedAt', 'resolvedAt', 'archivedAt', 'lastCalculated',
  'baselineDate', 'timestamp', 'date', 'dateTime', 'sentAt', 'receivedAt',
  'readAt', 'deliveredAt', 'processedAt', 'executedAt', 'installedAt'
]);
    ...
    for (let i = 0; i < items.length; i += batchSize) {
      ...
      for (const item of batch) {
        try {
          ...
          if (skipIfExists && (error.code === 'P2002' || error.message?.includes('Unique constraint'))) {
            skipped++;
          } else {
      ...
      console.log(`  Progress: ${progress}/${items.length} (${migrated} migrated, ${skipped} skipped, ${errors} errors)`);
    }
...
async function main() {
  ...
  try {
    ...
    await migrateTable(
      'Country',
      (data) => postgres.country.create({ data })
    );

    // Migrate Users (no dependencies)
    await migrateTable(
      'User',
      (data) => postgres.user.create({ data })
    );

    // Migrate Roles
    await migrateTable(
      'Role',
      (data) => postgres.role.create({ data })
    );

    // Migrate Permissions
    await migrateTable(
      'Permission',
      (data) => postgres.permission.create({ data })
    );
