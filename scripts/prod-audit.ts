...
async function testDatabaseConnectivity() {
  ...
  try {
    ...
    logResult("DB_CONNECTION", "FAIL", "Failed to connect to database", {
...
async function testSchemaIntegrity() {
  ...
  try {
    ...
    try {
      const crisisEvents =
        await prisma.$queryRaw`SELECT DISTINCT "category" FROM "CrisisEvent" LIMIT 10`;
    ...
    logResult("SCHEMA_INTEGRITY", "FAIL", "Schema integrity check failed", {
      error: error instanceof Error ? error.message : error,
    });
    return false;
  }
...
async function testUserCRUD(testUserId?: string) {
  ...
  try {
    ...
    logResult("USER_CRUD", "FAIL", "User CRUD operations failed", {
      error: error instanceof Error ? error.message : error,
    });
    return false;
  }
  ...
  try {
    ...
    if (!user?.country) {
      logResult(
        "COUNTRY_READ",
        "WARN",
        "No country found for test user - skipping COUNTRY CRUD tests"
      );
      return true;
    }
    ...
    logResult("COUNTRY_CRUD", "FAIL", "Country CRUD operations failed", {
      error: error instanceof Error ? error.message : error,
    });
    return false;
  }
...
async function testNotificationSystem(testUserId?: string) {
  ...
  try {
    ...
    logResult(
      "NOTIFICATION_READ",
      "PASS",
      `Found ${userNotifications} unread notifications for user`
    );
    ...
    logResult("NOTIFICATION_SYSTEM", "FAIL", "Notification system tests failed", {
      error: error instanceof Error ? error.message : error,
    });
    return false;
  }
...
async function testThinkPagesSystem(testUserId?: string) {
  ...
  try {
    ...
    logResult("THINKPAGES_SYSTEM", "FAIL", "ThinkPages system tests failed", {
      error: error instanceof Error ? error.message : error,
    });
    return false;
  }
...
async function testGovernmentSystem(testUserId?: string) {
  ...
  try {
    ...
    logResult(
      ...
      effectiveness ? "PASS" : "WARN",
      effectiveness ? "Atomic effectiveness record exists" : "No atomic effectiveness record"
    );

    return true;
  } catch (error) {
    logResult("GOVERNMENT_SYSTEM", "FAIL", "Government system tests failed", {
      error: error instanceof Error ? error.message : error,
    });
    return false;
  }
...
async function generateSummaryReport() {
  ...
  const health =
    failed === 0 ? (warned === 0 ? "🟢 HEALTHY" : "🟡 HEALTHY (with warnings)") : "🔴 UNHEALTHY";
