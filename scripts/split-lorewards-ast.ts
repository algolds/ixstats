/**
 * One-time AST splitter for the lorewards mega-router (mirrors split-activities-ast.ts).
 *
 ...
 * in lorewards/index.ts, so api.lorewards.* paths are unchanged.
 *
 * Run from the project root:  bun run scripts/split-lorewards-ast.ts
 */
...
const routerFile = "src/server/api/routers/lorewards.ts";
const outDir = "src/server/api/routers/lorewards";
...
const groups: Record<string, string[]> = {
  awards: [
    ...
    "getAwardWinningArticles",
    "isAwardWinningArticle",
    "getAwardFrequency",
    "getRecentWinners",
  ],
  leaderboards: [
  ...
  calendars: [
    "getStreakCalendar",
    "getRollingStreakCalendar",
    "getWinnersCalendar",
  ],
  admin: [
    "crossValidate",
    "getCrossValidationHistory",
    "scoreDay",
    "triggerSync",
    "overrideWinner",
    "getBlacklist",
...
const origDecl = origSf.getVariableDeclarationOrThrow("lorewardsRouter");
...
for (const [groupName, names] of Object.entries(groups)) {
  ...
  const decl = sf.getVariableDeclarationOrThrow("lorewardsRouter");
  decl.rename(`lorewards${cap(groupName)}Router`);
...
console.log("Done. Now create lorewards/index.ts (mergeRouters) and delete lorewards.ts.");
