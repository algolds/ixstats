...
if (hasPublishableKey) {
  console.log(`   Publishable Key Type: ${publishableKeyType === 'LIVE' ? '✅ LIVE' : publishableKeyType === 'TEST' ? '⚠️  TEST' : '❌ INVALID'}`);
}

console.log(`   CLERK_SECRET_KEY: ${hasSecretKey ? '✅ Present' : '❌ Missing'}`);
if (hasSecretKey) {
  console.log(`   Secret Key Type: ${secretKeyType === 'LIVE' ? '✅ LIVE' : secretKeyType === 'TEST' ? '⚠️  TEST' : '❌ INVALID'}`);
}
