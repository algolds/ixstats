/**
 * One-time AST splitter for the thinkpages posts mega-router.
 *
 ...
 * in thinkpages/posts/index.ts, so api.thinkpages.posts.* paths are unchanged.
 *
 * Run from the project root:  bun run scripts/split-thinkpages-posts-ast.ts
 */
...
const routerFile = "src/server/api/routers/thinkpages/posts.ts";
const outDir = "src/server/api/routers/thinkpages/posts";
fs.mkdirSync(outDir, { recursive: true });

const groups: Record<string, string[]> = {
  posts: ["createPost", "updatePost", "deletePost", "pinPost"],
  reactions: ["addReaction", "removeReaction", "getPostReactions"],
  bookmarks: ["getBookmarks", "isBookmarked", "bookmarkPost"],
  flags: ["getFlaggedPosts", "isFlagged", "flagPost", "unflagPost"],
};
...
const origDecl = origSf.getVariableDeclarationOrThrow("thinkpagesPostsRouter");
...
for (const [groupName, names] of Object.entries(groups)) {
  ...
  const decl = sf.getVariableDeclarationOrThrow("thinkpagesPostsRouter");
  decl.rename(`thinkpagesPosts${cap(groupName)}Router`);
...
console.log("Done. Now create thinkpages/posts/index.ts (mergeRouters) and delete posts.ts.");
