...
async function calculateIxEarthMetrics() {
  ...
  try {
    ...
    console.log(
      `   With Geographic Boundaries: ${countriesWithGeography} (${((countriesWithGeography / allCountries.length) * 100).toFixed(1)}%)`
    );
    console.log(
      `   Without Geographic Boundaries: ${countriesWithoutGeography} (${((countriesWithoutGeography / allCountries.length) * 100).toFixed(1)}%)`
    );
    console.log("");
    ...
    console.log(
      `   Percentage Difference: ${((landAreaScaleFactor - 1) * 100).toFixed(1)}% ${landAreaScaleFactor > 1 ? "larger" : "smaller"}`
    );
    console.log("");
    ...
    console.log(
      `   Land Percentage: ${((totalLandArea / estimatedTotalSurfaceArea) * 100).toFixed(1)}%`
    );
    console.log(
      `   Water Percentage: ${((estimatedWaterArea / estimatedTotalSurfaceArea) * 100).toFixed(1)}%`
    );
    console.log("");
    ...
    console.log(
      `   Percentage Difference: ${((totalSurfaceAreaScaleFactor - 1) * 100).toFixed(1)}% ${totalSurfaceAreaScaleFactor > 1 ? "larger" : "smaller"}`
    );
    console.log("");
    ...
    console.log(
      `   Population Density: ${(totalPopulation / totalLandArea).toFixed(2)} people/sq mi`
    );
    console.log("");
    ...
    sortedByArea.forEach((country, index) => {
      ...
      console.log(
        `   ${(index + 1).toString().padStart(2)}. ${hasMap}${country.name.padEnd(30)} ${country.areaSqMi.toLocaleString().padStart(15)} sq mi`
      );
    });
    console.log("");
    ...
    sortedByPopulation.forEach((country, index) => {
      const hasMap = country.geometry ? "🗺️ " : "   ";
      console.log(
        `   ${(index + 1).toString().padStart(2)}. ${hasMap}${country.name.padEnd(30)} ${country.currentPopulation.toLocaleString().padStart(15)}`
      );
    });
    console.log("");
    ...
    sortedByGdp.forEach((country, index) => {
      ...
      console.log(
        `   ${(index + 1).toString().padStart(2)}. ${hasMap}${country.name.padEnd(30)} $${gdpTrillions.toFixed(2).padStart(8)} trillion`
      );
    });
    console.log("");
    ...
    console.log(
      `   making it approximately ${landAreaScaleFactor.toFixed(2)}x the size of Earth's land area.`
    );
    console.log("");
    console.log(`   If IxEarth has a similar land-to-water ratio as Earth (~29% land),`);
    console.log(
      `   its total surface area would be approximately ${estimatedTotalSurfaceArea.toLocaleString()} sq mi,`
    );
