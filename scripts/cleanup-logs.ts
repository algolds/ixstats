    ...
    try {
      ...
      for (const file of files) {
        ...
        if (lines.length > MAX_LINES) {
          ...
          console.log(`  ✂️  Truncated ${file}: ${lines.length} → ${MAX_LINES} lines (${this.formatBytes(spaceSaved)} saved)`);
        }
      }
    } catch (error) {
    ...
    try {
      ...
      try {
        ...
        if (lines.length > MAX_LINES) {
          ...
          this.stats.truncatedFiles++;
          this.stats.totalSpaceSaved += spaceSaved;
          console.log(`  ✂️  Truncated dev.log: ${lines.length} → ${MAX_LINES} lines (${this.formatBytes(spaceSaved)} saved)`);
        }
      } catch (error) {
