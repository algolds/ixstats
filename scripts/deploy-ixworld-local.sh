#!/bin/bash
# deploy-ixworld-local.sh - Build and deploy the IxWorld standalone maps app (maps.ixwiki.com) locally.
# (Git sync block removed to preserve uncommitted local changes during development)
...
log "=== Starting Local IxWorld Deployment ==="
...
# Save the existing IxStats .next so the IxWorld build does not clobber it.
if [ -d ".next" ]; then
...
# Ensure shared dynamic assets are symlinked
