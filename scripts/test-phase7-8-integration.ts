  ...
  try {
    ...
    try {
      ...
      if (scenarioCount === 0) {
        ...
        for (const scenario of scenarios) {
          log(`  • ${scenario.title} (Type: ${scenario.type}, Status: ${scenario.status})`, 'reset');
          log(`    Countries: ${scenario.country1Name} <-> ${scenario.country2Name}`, 'reset');
          log(`    Impact: ${scenario.culturalImpact}, Risk: ${scenario.diplomaticRisk}, Cost: ${scenario.economicCost}`, 'reset');
        }
      }
    ...
    try {
      ...
      if (personalityCount === 0) {
        log('⚠️  No personalities found (empty database - fallback pattern will be tested)', 'yellow');
        results.push({
        ...
        for (const personality of personalities) {
          ...
          log(`    Active: ${personality.isActive}, Usage Count: ${personality.usageCount}`, 'reset');
          log(`    Traits: A:${personality.assertiveness} C:${personality.cooperativeness} E:${personality.economicFocus} M:${personality.militarism}`, 'reset');
        }
      }
    ...
    try {
      ...
      if (sampleScenario) {
        ...
        try {
          ...
          log(`  ✅ responseOptions parsed: ${Array.isArray(responseOptions) ? responseOptions.length : 0} options`, 'green');
          jsonTestsPassed++;
        } catch (error) {
      ...
      if (samplePersonality) {
        ...
        try {
          ...
          log(`  ✅ traitDescriptions parsed: ${Object.keys(traitDescriptions).length} traits`, 'green');
          jsonTestsPassed++;
        } catch (error) {
        ...
        try {
          ...
          log(`  ✅ culturalProfile parsed: ${Object.keys(culturalProfile).length} properties`, 'green');
          jsonTestsPassed++;
        } catch (error) {
        ...
        try {
          ...
          log(`  ✅ responsePatterns parsed: ${Array.isArray(responsePatterns) ? responsePatterns.length : 0} patterns`, 'green');
          jsonTestsPassed++;
        } catch (error) {
        ...
        try {
          ...
          log(`  ✅ scenarioResponses parsed: ${Object.keys(scenarioResponses).length} responses`, 'green');
          jsonTestsPassed++;
        } catch (error) {
        ...
        try {
          ...
          log(`  ✅ eventModifiers parsed: ${Object.keys(eventModifiers).length} modifiers`, 'green');
          jsonTestsPassed++;
        } catch (error) {
      ...
      if (jsonTestsPassed > 0) {
        log(`\nJSON Parsing Summary: ${jsonTestsPassed} passed, ${jsonTestsFailed} failed`, jsonTestsFailed > 0 ? 'yellow' : 'green');
        results.push({
    ...
    try {
      ...
      if (assignmentCount > 0) {
        ...
        for (const assignment of assignmentsWithRelations) {
          ...
          log(`  ${status} Country: ${countryName} -> Personality: ${personalityName}`, status === '✅' ? 'green' : 'red');
          log(`     Assigned: ${assignment.assignedAt.toISOString().split('T')[0]}, By: ${assignment.assignedBy || 'system'}`, 'reset');
        }

        // Check for orphaned assignments
        const orphanedAssignments = assignmentsWithRelations.filter(a => !a.personality || !a.country);
        if (orphanedAssignments.length > 0) {
          log(`\n⚠️  Found ${orphanedAssignments.length} orphaned assignments (missing personality or country)`, 'yellow');
          results.push({
    ...
    try {
      ...
      log(`  ✅ getPlayerScenarioHistory: ${completedScenarios.length} completed scenarios`, 'green');
      ...
      log(`  ✅ getPersonalityByArchetype: ${personalityByArchetype ? 'Found' : 'Not found'}`, personalityByArchetype ? 'green' : 'yellow');
      ...
      log(`  ✅ getCountryPersonality: ${sampleAssignment ? 'Assignment found' : 'No assignments'}`, sampleAssignment ? 'green' : 'yellow');
    ...
    try {
      ...
      if (completedWithoutResolved > 0) {
        log(`  ⚠️  Found ${completedWithoutResolved} completed scenarios without resolvedAt date`, 'yellow');
        qualityIssues++;
      } else {
      ...
      if (invalidTraitPersonalities.length > 0) {
        log(`  ⚠️  Found ${invalidTraitPersonalities.length} personalities with traits outside 0-100 range`, 'yellow');
        qualityIssues++;
      } else {
