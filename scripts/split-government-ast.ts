/**
 * One-time AST splitter for the government mega-router (mirrors split-activities-ast.ts).
 *
 * Strategy (behaviour-preserving): copy the whole router file once per group so every
 * import + module-level helper/schema is retained, rename the exported router, then delete
 * the procedures that don't belong to the group. The groups are recombined with mergeRouters
 * in government/index.ts, so api.government.* paths are unchanged.
 *
 * Run from the project root:  bun run scripts/split-government-ast.ts
 */
...
const routerFile = "src/server/api/routers/government.ts";
const outDir = "src/server/api/routers/government";
...
const origDecl = origSf.getVariableDeclarationOrThrow("governmentRouter");
...
for (const [groupName, names] of Object.entries(groups)) {
  ...
  const decl = sf.getVariableDeclarationOrThrow("governmentRouter");
  decl.rename(`government${cap(groupName)}Router`);
...
console.log("Done. Now create government/index.ts (mergeRouters) and delete government.ts.");
