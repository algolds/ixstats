...
function parsePrismaSchema(): Map<
  string,
  Map<string, { type: string; default?: string; optional: boolean }>
> {
    ...
    for (const file of routerFiles) {
      ...
      while ((match = schemaRegex.exec(content)) !== null) {
        ...
        while ((fieldMatch = fieldRegex.exec(schemaContent)) !== null) {
          ...
          const isOptional =
            schemaContent.includes(`${fieldName}:`) && schemaContent.includes(".optional()");
  ...
  for (const [schemaKey, zodFields] of zodSchemas) {
    ...
    for (const [fieldName, prismaField] of prismaFields) {
      if (!zodFields.has(fieldName)) {
        ...
        const expectedZodTypes = TYPE_MAPPINGS[prismaField.type] || [
          prismaField.type.toLowerCase(),
        ];
