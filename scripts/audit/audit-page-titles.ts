...
function extractMetadata(content: string): { hasMetadata: boolean; hasTitle: boolean; currentTitle: string | null } {
  ...
  function scanDirectory(dir: string) {
    ...
    for (const entry of entries) {
      ...
      if (stat.isDirectory()) {
        ...
        const suggestedTitle = TITLE_MAPPINGS[routePath] || `${entry.replace('.tsx', '')} - IxStats`;
        ...
        results.push({
          ...
          ...metadata,
          suggestedTitle,
          needsUpdate: !metadata.hasTitle || (metadata.currentTitle !== suggestedTitle && metadata.currentTitle !== '(dynamic)')
        });
      }
    }
  }
...
function generateReport(pages: PageInfo[]) {
  ...
  console.log(`With titles: ${withTitle.length} (${Math.round(withTitle.length/pages.length*100)}%)`);
