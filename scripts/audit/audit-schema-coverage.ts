...
function parseRouterOperations(): Map<string, { operations: string[]; models: string[] }> {
  ...
  try {
    const routerFiles = readdirSync(routersDir).filter(
      (file) => file.endsWith(".ts") && !file.startsWith("__")
    );
...
function analyzeModelCoverage(
  models: string[],
  routerOps: Map<string, { operations: string[]; models: string[] }>
): void {
    ...
    for (const [router, data] of routerOps) {
      if (data.models.includes(model)) {
        ...
        if (
          ...
          operations.includes("find")
        )
          coverage.hasRead = true;
...
function analyzeRouterCoverage(
  routerOps: Map<string, { operations: string[]; models: string[] }>
): void {
  for (const [router, data] of routerOps) {
...
function generateRecommendations(): void {
  const criticalModels = report.modelCoverage.filter(
    (m) => m.priority === "critical" && m.userFacing
  );
  const highPriorityModels = report.modelCoverage.filter(
    (m) => m.priority === "high" && m.userFacing
  );

  if (criticalModels.length > 0) {
    report.recommendations.push(
      "🚨 CRITICAL: Add missing CRUD operations for: " +
  ...
  if (highPriorityModels.length > 0) {
    report.recommendations.push(
      "⚠️ HIGH PRIORITY: Improve coverage for: " + highPriorityModels.map((m) => m.model).join(", ")
    );
  }
...
function calculateStatistics(): void {
  ...
  report.partiallyCoveredModels = report.modelCoverage.filter(
...
function printResults(): void {
  ...
  report.routerCoverage
    ...
    .forEach((router) => {
      console.log(
        `  📄 ${router.router}: ${router.operations} operations, ${router.coveredModels} models`
      );
    });
  console.log("");
