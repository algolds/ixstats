/**
 * One-time AST splitter for the embassies/lifecycle router (mirrors split-activities-ast.ts).
 *
 ...
 * in diplomacy/embassies/lifecycle/index.ts, so api.diplomaticEmbassies.lifecycle.* paths
 * are unchanged.
 *
 * Run from the project root:  bun run scripts/split-embassies-lifecycle-ast.ts
 */
...
const routerFile = "src/server/api/routers/diplomacy/embassies/lifecycle.ts";
const outDir = "src/server/api/routers/diplomacy/embassies/lifecycle";
fs.mkdirSync(outDir, { recursive: true });

const groups: Record<string, string[]> = {
  status: ["closeEmbassy", "reopenEmbassy"],
  severance: ["deleteEmbassy"],
};
...
const origDecl = origSf.getVariableDeclarationOrThrow("diplomaticEmbassiesLifecycleRouter");
...
for (const [groupName, names] of Object.entries(groups)) {
  ...
  const decl = sf.getVariableDeclarationOrThrow("diplomaticEmbassiesLifecycleRouter");
  decl.rename(`diplomaticEmbassiesLifecycle${cap(groupName)}Router`);
...
console.log("Done. Now create lifecycle/index.ts (mergeRouters) and delete lifecycle.ts.");
