...
function makeRequest(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    req.on("error", (error) => {
      reject(
        new Error(`Request failed: ${error instanceof Error ? error.message : "Unknown error"}`)
      );
    });
 ...
 * @returns {Promise<FlagCacheStatus>}
 */
async function waitForUpdate(maxWaitTime = 600000) {
  // 10 minutes
...
async function main() {
  ...
  try {
    ...
    if (flagCache.shouldUpdate) {
      ...
      log(
        `Final status: ${finalFlagCache.cachedFlags}/${finalFlagCache.totalCountries} flags cached`
      );
