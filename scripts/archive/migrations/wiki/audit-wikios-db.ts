...
async function main() {
  ...
  const redirects = await prisma.wikiArticle.count({ where: { redirectTargetSlug: { not: null } } });
  ...
  console.log(`   - Published Articles:      ${publishedArticles.toLocaleString()} (${((publishedArticles / (totalArticles || 1)) * 100).toFixed(1)}%)`);
  console.log(`   - With Wikitext Content:   ${withWikitext.toLocaleString()} (${((withWikitext / (totalArticles || 1)) * 100).toFixed(1)}%)`);
  ...
  console.log(`   - Avg Links per Article:   ${(totalLinks / (totalArticles || 1)).toFixed(1)} edges`);
  ...
  console.log(`   - With Dimensions (W x H): ${withDimensions.toLocaleString()} (${((withDimensions / (totalAssets || 1)) * 100).toFixed(1)}%)`);
  console.log(`   - With Thumbnail Variants: ${withThumbnails.toLocaleString()} (${((withThumbnails / (totalAssets || 1)) * 100).toFixed(1)}%)`);
