...
console.log(`   Difference: ${Math.abs(canonicalTotalLand - SCREENSHOT_APPROXIMATION.totalLand).toLocaleString()} sq mi`);
console.log(`   Match: ${((canonicalTotalLand / SCREENSHOT_APPROXIMATION.totalLand) * 100).toFixed(1)}%`);
console.log('\n');
...
console.log(`   Claimed by 82 Countries: ${claimedByCountries.toLocaleString()} sq mi (${claimedPercent.toFixed(1)}%)`);
console.log(`   Unclaimed Territories: ${unclaimedLand.toLocaleString()} sq mi (${(100 - claimedPercent).toFixed(1)}%)`);
console.log('\n');

// Using screenshot approximation's land-to-total ratio to estimate total surface
const screenshotLandPercent = SCREENSHOT_APPROXIMATION.totalLand / SCREENSHOT_APPROXIMATION.totalSurface;
...
console.log(`   Based on screenshot land-to-total ratio (${(screenshotLandPercent * 100).toFixed(1)}% land):`);
...
console.log(`   Match: ${((estimatedTotalSurface / SCREENSHOT_APPROXIMATION.totalSurface) * 100).toFixed(1)}%`);
console.log('\n');
...
console.log(`      - Claimed Countries: ${claimedByCountries.toLocaleString()} sq mi (${claimedPercent.toFixed(1)}%)`);
console.log(`      - Unclaimed Territories: ${unclaimedLand.toLocaleString()} sq mi (${(100 - claimedPercent).toFixed(1)}%)`);
