/**
 * One-time AST splitter for the diplomacy/cultural/lifecycle sub-router
 ...
 * in diplomacy/cultural/lifecycle/index.ts, so api.diplomaticCulturalLifecycle.* paths
 * are unchanged.
 *
 * NOTE: the source file `diplomacy/cultural/lifecycle.ts` and the output dir
 * `diplomacy/cultural/lifecycle/` share the same stem — a file and a directory of the
 * same stem cannot coexist. The caller must `rm` the original file first.
 *
 * Run from the project root:  bun run scripts/split-cultural-lifecycle-ast.ts
 */
...
const routerFile = "src/server/api/routers/diplomacy/cultural/lifecycle.ts";
const outDir = "src/server/api/routers/diplomacy/cultural/lifecycle";
fs.mkdirSync(outDir, { recursive: true });

const groups: Record<string, string[]> = {
  participation: ["voteOnExchange", "uploadCulturalArtifact"],
  management: ["updateCulturalExchange", "cancelCulturalExchange"],
};
...
const origDecl = origSf.getVariableDeclarationOrThrow("diplomaticCulturalLifecycleRouter");
...
for (const [groupName, names] of Object.entries(groups)) {
  ...
  const decl = sf.getVariableDeclarationOrThrow("diplomaticCulturalLifecycleRouter");
  decl.rename(`diplomaticCulturalLifecycle${cap(groupName)}Router`);
...
console.log("Done. Now create lifecycle/index.ts (mergeRouters).");
