/**
 * One-time AST splitter for the taxSystem mega-router (mirrors split-activities-ast.ts).
 *
 * Strategy (behaviour-preserving): copy the whole router file once per group so every
 * import + module-level helper (e.g. validateBracketsState) is retained, rename the
 * exported router, then delete the procedures that don't belong to the group. The groups
 * are recombined with mergeRouters in taxSystem/index.ts, so api.taxSystem.* paths are
 * unchanged.
 *
 * Run from the project root:  bun run scripts/split-taxsystem-ast.ts
 */
...
const routerFile = "src/server/api/routers/taxSystem.ts";
const outDir = "src/server/api/routers/taxSystem";
fs.mkdirSync(outDir, { recursive: true });

const groups: Record<string, string[]> = {
  crud: ["create", "update", "delete", "getByCountryId", "autosave"],
  calculations: [
    "calculateLiveTax",
  ...
  analysis: [
    "calculateTaxEffectiveness",
    "checkConflicts",
    "parseEconomicData",
    "parseEconomicDataForTax",
  ],
};
...
const origDecl = origSf.getVariableDeclarationOrThrow("taxSystemRouter");
...
for (const [groupName, names] of Object.entries(groups)) {
  ...
  const decl = sf.getVariableDeclarationOrThrow("taxSystemRouter");
  decl.rename(`taxSystem${cap(groupName)}Router`);
...
console.log("Done. Now create taxSystem/index.ts (mergeRouters) and delete taxSystem.ts.");
