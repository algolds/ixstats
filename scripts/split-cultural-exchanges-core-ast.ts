/**
 * One-time AST splitter for the diplomacy/cultural/exchanges/core sub-router
 * (mirrors split-cultural-exchanges-ast.ts).
 *
 ...
 * in diplomacy/cultural/exchanges/core/index.ts, so api.diplomaticCulturalExchanges.* paths
 * are unchanged.
 *
 * The source file `exchanges/core.ts` and the output dir `exchanges/core/` share the same
 * stem — a file and a directory of the same stem cannot coexist. The caller must `rm` the
 * original file first.
 *
 * Run from the project root:  bun run scripts/split-cultural-exchanges-core-ast.ts
 */
...
const routerFile = "src/server/api/routers/diplomacy/cultural/exchanges/core.ts";
const outDir = "src/server/api/routers/diplomacy/cultural/exchanges/core";
fs.mkdirSync(outDir, { recursive: true });

const groups: Record<string, string[]> = {
  queries: ["getCulturalExchanges"],
  mutations: ["createCulturalExchange", "joinCulturalExchange"],
};
...
const origDecl = origSf.getVariableDeclarationOrThrow("diplomaticCulturalExchangesCoreRouter");
...
for (const [groupName, names] of Object.entries(groups)) {
  ...
  const decl = sf.getVariableDeclarationOrThrow("diplomaticCulturalExchangesCoreRouter");
  decl.rename(`diplomaticCulturalExchangesCore${cap(groupName)}Router`);
  ...
  // Strip orphan top-level statements (function declarations, etc.) that come AFTER the
  // router variable. The splitter template copies the whole file per group, so dead module-
  // level helpers (e.g. embassy game mechanics scaffolding) are duplicated into every group.
  // Those helpers are not referenced by the kept procedures — they're scaffolding for code
  // that was never written. Removing them shrinks each file under the 700-line cap and is
  // strictly behaviour-preserving: the router's procedure keys and handlers are unchanged.
  const routerEnd = decl.getEnd();
  const orphans = sf.getStatements().filter((s) => s.getStart() > routerEnd);
  for (const s of orphans) s.remove();

  sf.saveSync();
  keptSum += kept;
  console.log(
    `${groupName}.ts: kept ${kept}/${names.length} procedures; stripped ${orphans.length} orphan top-level statements`
  );
}
...
console.log(
  "Done. Now create diplomacy/cultural/exchanges/core/index.ts (mergeRouters) and delete diplomacy/cultural/exchanges/core.ts."
);
