/**
 * One-time AST splitter for the activities mega-router (mirrors split-thinkpages-ast.ts).
 *
 ...
 * in activities/index.ts, so api.activities.* paths are unchanged.
 *
 * Run from the project root:  bun run scripts/split-activities-ast.ts
 */
...
const routerFile = "src/server/api/routers/activities.ts";
const outDir = "src/server/api/routers/activities";
...
const origDecl = origSf.getVariableDeclarationOrThrow("activitiesRouter");
...
for (const [groupName, names] of Object.entries(groups)) {
  ...
  const decl = sf.getVariableDeclarationOrThrow("activitiesRouter");
  decl.rename(`activities${cap(groupName)}Router`);
...
console.log("Done. Now create activities/index.ts (mergeRouters) and delete activities.ts.");
