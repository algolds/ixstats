/**
 * One-time AST splitter for the militaryEquipment mega-router (mirrors split-activities-ast.ts).
 *
 ...
 * in militaryEquipment/index.ts, so api.militaryEquipment.* paths are unchanged.
 *
 * Run from the project root:  bun run scripts/split-militaryequipment-ast.ts
 */
...
const routerFile = "src/server/api/routers/militaryEquipment.ts";
const outDir = "src/server/api/routers/militaryEquipment";
...
const groups: Record<string, string[]> = {
  catalog: [
    ...
    "getAllCatalogEquipment",
    "createCatalogEquipment",
    "updateCatalogEquipment",
    "deleteCatalogEquipment",
    "bulkToggleEquipment",
    "incrementEquipmentUsage",
  ],
  manufacturers: [
    "getManufacturers",
    "createManufacturer",
  ...
  analytics: ["getEquipmentUsageStats"],
  images: [
    "resolveEquipmentImage",
    "batchResolveEquipmentImages",
    "getImageCacheStats",
  ],
};
...
const origDecl = origSf.getVariableDeclarationOrThrow("militaryEquipmentRouter");
...
for (const [groupName, names] of Object.entries(groups)) {
  ...
  const decl = sf.getVariableDeclarationOrThrow("militaryEquipmentRouter");
  decl.rename(`militaryEquipment${cap(groupName)}Router`);
...
console.log(
  "Done. Now create militaryEquipment/index.ts (mergeRouters) and delete militaryEquipment.ts."
);
