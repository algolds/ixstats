...
async function unifyData() {
  ...
  console.log(
    `   Total calculated area (Earth scale): ${totalCalculatedArea.toLocaleString()} sq mi`
  );
  ...
  for (const country of rosterCountries) {
    try {
      ...
      const existing = await db.country.findFirst({
        where: {
          OR: [{ name: country.Country }, { slug: slug }],
        },
      });
      ...
      if (validGeometry) {
        withGeography++;
        console.log(
          `✓ ${country.Country.padEnd(30)} → ${country["Area (sq mi)"].toLocaleString().padStart(12)} sq mi (with map)`
        );
      } else {
        withoutGeography++;
        console.log(
          `○ ${country.Country.padEnd(30)} → ${country["Area (sq mi)"].toLocaleString().padStart(12)} sq mi (no map)`
        );
      }
    } catch (error) {
