/**
 * One-time AST splitter for the diplomacy/cultural/exchanges sub-router
 ...
 * in diplomacy/cultural/exchanges/index.ts, so api.diplomaticCulturalExchanges.* paths
 * are unchanged.
 *
 * NOTE: the source file `diplomacy/cultural/exchanges.ts` and the output dir
 * `diplomacy/cultural/exchanges/` share the same stem — a file and a directory of the
 * same stem cannot coexist. The caller must `rm` the original file first.
 *
 * Run from the project root:  bun run scripts/split-cultural-exchanges-ast.ts
 */
...
const routerFile = "src/server/api/routers/diplomacy/cultural/exchanges.ts";
const outDir = "src/server/api/routers/diplomacy/cultural/exchanges";
fs.mkdirSync(outDir, { recursive: true });

const groups: Record<string, string[]> = {
  core: ["getCulturalExchanges", "createCulturalExchange", "joinCulturalExchange"],
  missions: ["linkExchangeToMission"],
};
...
const origDecl = origSf.getVariableDeclarationOrThrow("diplomaticCulturalExchangesRouter");
...
for (const [groupName, names] of Object.entries(groups)) {
  ...
  const decl = sf.getVariableDeclarationOrThrow("diplomaticCulturalExchangesRouter");
  decl.rename(`diplomaticCulturalExchanges${cap(groupName)}Router`);
...
console.log("Done. Now create exchanges/index.ts (mergeRouters).");
