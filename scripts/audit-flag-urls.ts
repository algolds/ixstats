...
async function checkFlagDirectory(): Promise<void> {
  ...
  const flagFiles = fs.readdirSync(FLAGS_DIR).filter(f =>
    f.endsWith('.svg') || f.endsWith('.png') || f.endsWith('.jpg') || f.endsWith('.webp')
  );
...
async function checkFlagServiceCode(): Promise<void> {
  ...
  if (!serviceContent.includes('${basePath}/flags/metadata.json') &&
...
async function checkFlagUsageInComponents(): Promise<void> {
  ...
  for (const file of files) {
    ...
    if (content.includes('unifiedFlagService') ||
