/**
 * One-time AST splitter for the diplomatic-cultural compatibility router
 ...
 * in compatibility/index.ts, so api.<...>.diplomaticCultural.* paths are unchanged.
 *
 * Because the source file becomes a same-named subdirectory, the original file is
 * removed BEFORE the output directory is created. The file content is read into memory
 * first so the per-group copies can still be written.
 *
 * Run from the project root:  bun run scripts/split-cultural-compatibility-ast.ts
 */
...
const routerFile = "src/server/api/routers/diplomacy/cultural/compatibility.ts";
const outDir = "src/server/api/routers/diplomacy/cultural/compatibility";

const groups: Record<string, string[]> = {
  impact: ["calculateExchangeImpact"],
  scores: ["getCulturalCompatibility", "getRecommendedPartners"],
};
...
// Read source into memory FIRST so we can rm the file before mkdir'ing the same-name dir.
const originalContent = fs.readFileSync(routerFile, "utf8");

// Validate the original router AST in-memory (use a tmp path so ts-morph is happy).
const origSf = project.createSourceFile(routerFile + ".__orig.tmp.ts", originalContent);
const origDecl = origSf.getVariableDeclarationOrThrow("diplomaticCulturalCompatibilityRouter");
...
// Drop the validation source from the project (it was never on disk).
project.removeSourceFile(origSf);

// rm the old file BEFORE creating the dir.
...
for (const [groupName, names] of Object.entries(groups)) {
  ...
  const decl = sf.getVariableDeclarationOrThrow("diplomaticCulturalCompatibilityRouter");
  decl.rename(`diplomaticCulturalCompatibility${cap(groupName)}Router`);
...
console.log("Done. Now create compatibility/index.ts (mergeRouters).");
